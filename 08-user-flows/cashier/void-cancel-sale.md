---
title: "Void or Cancel Sale Flow"
...
permission_anchor: "pos.sale.void"
updated: "2026-05-10"
---

# Void or Cancel Sale Flow
> Cancel an in-progress sale or void a completed sale using permission-controlled rules.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Cancel an in-progress sale or void a completed sale using permission-controlled rules.
...
|---|---|---|
| `sales` | Supports void or cancel sale flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `sale_lines` | Supports void or cancel sale flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `stock_movements` | Supports void or cancel sale flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `audit_logs` | Supports void or cancel sale flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/pos/sales/{saleId}/cancel` | Supports this flow | JWT + tenant context + `pos.sale.void` |
| `POST /api/pos/sales/{saleId}/void` | Supports this flow | JWT + tenant context + `pos.sale.void` |
...
{ "permission": "pos.sale.void", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Void or Cancel Sale Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
