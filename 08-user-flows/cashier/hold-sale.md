...
permission_anchor: "pos.sale.hold"
updated: "2026-05-10"
---

# Hold Sale Flow
> Temporarily save a sale cart under tenant, outlet, session, and cashier context.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Temporarily save a sale cart under tenant, outlet, session, and cashier context.
...
|---|---|---|
| `sales` | Supports hold sale flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `sale_lines` | Supports hold sale flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/pos/sales/hold` | Supports this flow | JWT + tenant context + `pos.sale.hold` |
| `GET /api/pos/sales/held` | Supports this flow | JWT + tenant context + `pos.sale.hold` |
...
{ "permission": "pos.sale.hold", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Hold Sale Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
