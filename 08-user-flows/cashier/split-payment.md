...
permission_anchor: "payment.split.record"
updated: "2026-05-10"
---

# Split Payment Flow
> Allocate multiple payment records to one sale or order while preventing overpayment errors.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Allocate multiple payment records to one sale or order while preventing overpayment errors.
...
|---|---|---|
| `payments` | Supports split payment flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `sale_payment_allocations` | Supports split payment flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `order_payment_allocations` | Supports split payment flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/pos/sales/{saleId}/split-payments` | Supports this flow | JWT + tenant context + `payment.split.record` |
...
## 14. API Example
```http
POST /api/pos/sales/{saleId}/split-payments
...
{ "permission": "payment.split.record", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Split Payment Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
