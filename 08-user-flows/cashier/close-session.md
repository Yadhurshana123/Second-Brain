...
permission_anchor: "pos.till.close"
updated: "2026-05-10"
---

# Close Till Session Flow
> Count cash, calculate variance, request approval where needed, and close the active session.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Count cash, calculate variance, request approval where needed, and close the active session.
...
|---|---|---|
| `till_sessions` | Supports close till session flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `cash_count_denominations` | Supports close till session flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `cash_movements` | Supports close till session flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/pos/till-sessions/{id}/close` | Supports this flow | JWT + tenant context + `pos.till.close` |
...
## 14. API Example
```http
POST /api/pos/till-sessions/{id}/close
...
{ "permission": "pos.till.close", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Close Till Session Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
