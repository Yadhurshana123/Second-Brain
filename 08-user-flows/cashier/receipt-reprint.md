...
permission_anchor: "receipt.reprint"
updated: "2026-05-10"
---

# Receipt Reprint Flow
> Reprint receipt with permission, duplicate label, and print audit log.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Reprint receipt with permission, duplicate label, and print audit log.
...
|---|---|---|
| `receipts` | Supports receipt reprint flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `receipt_print_logs` | Supports receipt reprint flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `audit_logs` | Supports receipt reprint flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/pos/receipts/{id}/reprint` | Supports this flow | JWT + tenant context + `receipt.reprint` |
...
{ "permission": "receipt.reprint", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Receipt Reprint Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
