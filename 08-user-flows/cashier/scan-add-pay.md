...
permission_anchor: "pos.sale.create"
updated: "2026-05-10"
---

# Scan Add Pay Flow
> Scan or search products, build a POS cart, validate totals, accept payment, deduct stock, and issue receipt.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Scan or search products, build a POS cart, validate totals, accept payment, deduct stock, and issue receipt.
...
|---|---|---|
| `sales` | Supports scan add pay flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `sale_lines` | Supports scan add pay flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `payments` | Supports scan add pay flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `sale_payment_allocations` | Supports scan add pay flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `stock_movements` | Supports scan add pay flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `receipts` | Supports scan add pay flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/pos/sales` | Supports this flow | JWT + tenant context + `pos.sale.create` |
| `POST /api/pos/sales/{id}/complete` | Supports this flow | JWT + tenant context + `pos.sale.create` |
...
{ "permission": "pos.sale.create", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Scan Add Pay Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
