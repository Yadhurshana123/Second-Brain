...
permission_anchor: "return.create"
updated: "2026-05-10"
---

# POS Return Flow
> Return eligible sale or order lines, calculate refund, update stock, and audit policy decisions.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Return eligible sale or order lines, calculate refund, update stock, and audit policy decisions.
...
|---|---|---|
| `returns` | Supports pos return flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `return_lines` | Supports pos return flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `refunds` | Supports pos return flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `return_refund_allocations` | Supports pos return flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `stock_movements` | Supports pos return flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/pos/returns` | Supports this flow | JWT + tenant context + `return.create` |
| `POST /api/pos/returns/{id}/complete` | Supports this flow | JWT + tenant context + `return.create` |
...
{ "permission": "return.create", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `POS Return Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
