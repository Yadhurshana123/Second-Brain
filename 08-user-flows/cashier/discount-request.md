...
permission_anchor: "discount.request.create"
updated: "2026-05-10"
---

# Discount Request Flow
> Request line, sale, or order discount and route approval where configured thresholds require it.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Request line, sale, or order discount and route approval where configured thresholds require it.
...
|---|---|---|
| `discount_requests` | Supports discount request flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `discount_policies` | Supports discount request flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `discount_applications` | Supports discount request flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/pos/discount-requests` | Supports this flow | JWT + tenant context + `discount.request.create` |
| `POST /api/manager/discount-requests/{id}/resolve` | Supports this flow | JWT + tenant context + `discount.request.create` |
...
## 14. API Example
```http
POST /api/pos/discount-requests
...
{ "permission": "discount.request.create", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Discount Request Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
