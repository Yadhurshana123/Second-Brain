...
permission_anchor: "exchange.create"
updated: "2026-05-10"
---

# POS Exchange Flow
> Create exchange from accepted return, issue new items, and collect or refund difference.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Create exchange from accepted return, issue new items, and collect or refund difference.
...
|---|---|---|
| `exchanges` | Supports pos exchange flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `exchange_lines` | Supports pos exchange flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `payments` | Supports pos exchange flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `refunds` | Supports pos exchange flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `stock_movements` | Supports pos exchange flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/pos/exchanges` | Supports this flow | JWT + tenant context + `exchange.create` |
| `POST /api/pos/exchanges/{id}/complete` | Supports this flow | JWT + tenant context + `exchange.create` |
...
{ "permission": "exchange.create", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `POS Exchange Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
