...
permission_anchor: "pos.sale.recall"
updated: "2026-05-10"
---

# Recall Sale Flow
> Retrieve a held sale and continue checkout under controlled outlet/session access.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Retrieve a held sale and continue checkout under controlled outlet/session access.
...
|---|---|---|
| `sales` | Supports recall sale flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `sale_lines` | Supports recall sale flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `till_sessions` | Supports recall sale flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/pos/sales/{saleId}/recall` | Supports this flow | JWT + tenant context + `pos.sale.recall` |
...
## 14. API Example
```http
POST /api/pos/sales/{saleId}/recall
...
{ "permission": "pos.sale.recall", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Recall Sale Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
