...
permission_anchor: "payment.cash.record"
updated: "2026-05-10"
---

# Cash Payment Flow
> Record cash payment against POS sale and update expected cash for the active till session.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Record cash payment against POS sale and update expected cash for the active till session.
...
|---|---|---|
| `payments` | Supports cash payment flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `sale_payment_allocations` | Supports cash payment flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `till_sessions` | Supports cash payment flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `receipts` | Supports cash payment flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/pos/payments/cash` | Supports this flow | JWT + tenant context + `payment.cash.record` |
| `POST /api/pos/sales/{saleId}/payments` | Supports this flow | JWT + tenant context + `payment.cash.record` |
...
{ "permission": "payment.cash.record", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Cash Payment Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
