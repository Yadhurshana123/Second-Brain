...
permission_anchor: "pos.till.open"
updated: "2026-05-10"
---

# Start Till Session Flow
> Open a cashier till session with device, outlet, till, and opening cash validation.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Open a cashier till session with device, outlet, till, and opening cash validation.
...
|---|---|---|
| `till_sessions` | Supports start till session flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `tills` | Supports start till session flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `pos_devices` | Supports start till session flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `cash_count_denominations` | Supports start till session flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/pos/till-sessions/open` | Supports this flow | JWT + tenant context + `pos.till.open` |
| `GET /api/pos/tills/{id}/status` | Supports this flow | JWT + tenant context + `pos.till.open` |
...
## 14. API Example
```http
POST /api/pos/till-sessions/open
...
{ "permission": "pos.till.open", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Start Till Session Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
