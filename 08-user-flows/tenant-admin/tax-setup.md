...
permission_anchor: "tax.configure"
updated: "2026-05-10"
---

# Tax Setup Flow
> Configure tax classes, rates, mappings, and effective-date behavior for POS and e-commerce totals.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Configure tax classes, rates, mappings, and effective-date behavior for POS and e-commerce totals.
...
|---|---|---|
| `tax_classes` | Supports tax setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `tax_rates` | Supports tax setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `tax_class_rates` | Supports tax setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `products` | Supports tax setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/tenant/tax/classes` | Supports this flow | JWT + tenant context + `tax.configure` |
| `POST /api/tenant/tax/rates` | Supports this flow | JWT + tenant context + `tax.configure` |
| `PUT /api/tenant/products/{id}/tax-class` | Supports this flow | JWT + tenant context + `tax.configure` |
...
{ "permission": "tax.configure", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Tax Setup Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
