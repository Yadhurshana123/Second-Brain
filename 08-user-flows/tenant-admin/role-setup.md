...
permission_anchor: "role.configure"
updated: "2026-05-10"
---

# Tenant Role Setup Flow
> Create tenant-specific roles and assign configurable permissions and features.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Create tenant-specific roles and assign configurable permissions and features.
...
|---|---|---|
| `roles` | Supports tenant role setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `permissions` | Supports tenant role setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `role_permissions` | Supports tenant role setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `role_feature_assignments` | Supports tenant role setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/tenant/roles` | Supports this flow | JWT + tenant context + `role.configure` |
| `PUT /api/tenant/roles/{id}/permissions` | Supports this flow | JWT + tenant context + `role.configure` |
| `PUT /api/tenant/roles/{id}/features` | Supports this flow | JWT + tenant context + `role.configure` |
...
{ "permission": "role.configure", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Tenant Role Setup Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
