...
permission_anchor: "theme.configure"
updated: "2026-05-10"
---

# Theme Setup Flow
> Configure tenant UI theme tokens used by tenant admin and customer-facing experiences.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Configure tenant UI theme tokens used by tenant admin and customer-facing experiences.
...
|---|---|---|
| `ui_themes` | Supports theme setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `tenant_settings` | Supports theme setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/tenant/themes` | Supports this flow | JWT + tenant context + `theme.configure` |
| `PUT /api/tenant/themes/{id}/default` | Supports this flow | JWT + tenant context + `theme.configure` |
...
{ "permission": "theme.configure", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Theme Setup Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
