...
permission_anchor: "staff.user.create"
updated: "2026-05-10"
---

# Staff User Setup Flow
> Create tenant staff users and assign tenant or outlet roles.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Create tenant staff users and assign tenant or outlet roles.
...
|---|---|---|
| `users` | Supports staff user setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `tenant_user_roles` | Supports staff user setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `outlet_user_roles` | Supports staff user setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `roles` | Supports staff user setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/tenant/users` | Supports this flow | JWT + tenant context + `staff.user.create` |
| `POST /api/tenant/users/{userId}/roles` | Supports this flow | JWT + tenant context + `staff.user.create` |
...
{ "permission": "staff.user.create", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Staff User Setup Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
