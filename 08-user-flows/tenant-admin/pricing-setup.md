...
permission_anchor: "pricing.configure"
updated: "2026-05-10"
---

# Pricing Setup Flow
> Create channel-aware price lists and variant-level prices for POS and e-commerce.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Create channel-aware price lists and variant-level prices for POS and e-commerce.
...
|---|---|---|
| `price_lists` | Supports pricing setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `price_list_items` | Supports pricing setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `product_variants` | Supports pricing setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/tenant/price-lists` | Supports this flow | JWT + tenant context + `pricing.configure` |
| `POST /api/tenant/price-lists/{id}/items` | Supports this flow | JWT + tenant context + `pricing.configure` |
...
{ "permission": "pricing.configure", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Pricing Setup Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
