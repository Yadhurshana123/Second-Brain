---
title: "Delivery Zone Setup Flow"
...
permission_anchor: "delivery.configure"
updated: "2026-05-10"
---

# Delivery Zone Setup Flow
> Define pickup/delivery methods, zones, and zone rates for online fulfillment.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Define pickup/delivery methods, zones, and zone rates for online fulfillment.
...
|---|---|---|
| `delivery_methods` | Supports delivery zone setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `delivery_zones` | Supports delivery zone setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `delivery_zone_rates` | Supports delivery zone setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/tenant/delivery-zones` | Supports this flow | JWT + tenant context + `delivery.configure` |
| `POST /api/tenant/delivery-zone-rates` | Supports this flow | JWT + tenant context + `delivery.configure` |
...
## 14. API Example
```http
POST /api/tenant/delivery-zones
...
{ "permission": "delivery.configure", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Delivery Zone Setup Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
