---
title: "Loyalty Program Setup Flow"
...
permission_anchor: "loyalty.configure"
updated: "2026-05-10"
---

# Loyalty Program Setup Flow
> Configure tenant-owned loyalty programs, membership tiers, and earning/redemption rules.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Configure tenant-owned loyalty programs, membership tiers, and earning/redemption rules.
...
|---|---|---|
| `loyalty_programs` | Supports loyalty program setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `membership_tiers` | Supports loyalty program setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `customer_memberships` | Supports loyalty program setup flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/tenant/loyalty-programs` | Supports this flow | JWT + tenant context + `loyalty.configure` |
| `POST /api/tenant/loyalty-programs/{id}/tiers` | Supports this flow | JWT + tenant context + `loyalty.configure` |
...
## 14. API Example
```http
POST /api/tenant/loyalty-programs
...
{ "permission": "loyalty.configure", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Loyalty Program Setup Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
