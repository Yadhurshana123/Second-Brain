...
permission_anchor: "discount.approve"
updated: "2026-05-10"
---

# Discount Approval Flow
> Approve or reject discount requests according to configurable tenant policies.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Approve or reject discount requests according to configurable tenant policies.
...
|---|---|---|
| `discount_requests` | Supports discount approval flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `discount_policies` | Supports discount approval flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `discount_applications` | Supports discount approval flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `audit_logs` | Supports discount approval flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/manager/discount-requests/{id}/approve` | Supports this flow | JWT + tenant context + `discount.approve` |
| `POST /api/manager/discount-requests/{id}/reject` | Supports this flow | JWT + tenant context + `discount.approve` |
...
## 14. API Example
```http
POST /api/manager/discount-requests/{id}/approve
...
{ "permission": "discount.approve", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Discount Approval Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
