---
title: "Offline Conflict Resolution Flow"
...
permission_anchor: "offline.conflict.resolve"
updated: "2026-05-10"
---

# Offline Conflict Resolution Flow
> Resolve offline sync conflicts without silently corrupting stock, payments, receipts, or sales.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Resolve offline sync conflicts without silently corrupting stock, payments, receipts, or sales.
...
|---|---|---|
| `offline_sync_conflicts` | Supports offline conflict resolution flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `offline_sync_items` | Supports offline conflict resolution flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `offline_sync_audit_logs` | Supports offline conflict resolution flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `audit_logs` | Supports offline conflict resolution flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/manager/offline-conflicts/{id}/resolve` | Supports this flow | JWT + tenant context + `offline.conflict.resolve` |
...
## 14. API Example
```http
POST /api/manager/offline-conflicts/{id}/resolve
...
{ "permission": "offline.conflict.resolve", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Offline Conflict Resolution Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
