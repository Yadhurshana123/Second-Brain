---
title: "Stock Adjustment Approval Flow"
...
permission_anchor: "inventory.adjustment.approve"
updated: "2026-05-10"
---

# Stock Adjustment Approval Flow
> Approve stock adjustment documents before posting inventory movements.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Approve stock adjustment documents before posting inventory movements.
...
|---|---|---|
| `stock_adjustments` | Supports stock adjustment approval flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `stock_adjustment_lines` | Supports stock adjustment approval flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `audit_logs` | Supports stock adjustment approval flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/manager/stock-adjustments/{id}/approve` | Supports this flow | JWT + tenant context + `inventory.adjustment.approve` |
...
## 14. API Example
```http
POST /api/manager/stock-adjustments/{id}/approve
...
{ "permission": "inventory.adjustment.approve", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Stock Adjustment Approval Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
