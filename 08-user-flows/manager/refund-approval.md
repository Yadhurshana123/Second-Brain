...
permission_anchor: "refund.approve"
updated: "2026-05-10"
---

# Refund Approval Flow
> Approve tenant refund requests before payment reversal or cash refund execution.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Approve tenant refund requests before payment reversal or cash refund execution.
...
|---|---|---|
| `refunds` | Supports refund approval flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `payments` | Supports refund approval flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `returns` | Supports refund approval flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `audit_logs` | Supports refund approval flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/manager/refunds/{id}/approve` | Supports this flow | JWT + tenant context + `refund.approve` |
...
{ "permission": "refund.approve", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Refund Approval Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
