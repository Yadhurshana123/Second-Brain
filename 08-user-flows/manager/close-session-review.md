---
title: "Close Session Review Flow"
...
permission_anchor: "till.close.review"
updated: "2026-05-10"
---

# Close Session Review Flow
> Review till close variance and approve or reject close session variance where configured.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Review till close variance and approve or reject close session variance where configured.
...
|---|---|---|
| `till_sessions` | Supports close session review flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `cash_count_denominations` | Supports close session review flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `audit_logs` | Supports close session review flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/manager/till-sessions/{id}/variance-approval` | Supports this flow | JWT + tenant context + `till.close.review` |
...
## 14. API Example
```http
POST /api/manager/till-sessions/{id}/variance-approval
...
{ "permission": "till.close.review", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Close Session Review Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
