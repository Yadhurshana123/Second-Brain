---
title: "Receipt Reprint Approval Flow"
...
permission_anchor: "receipt.reprint.approve"
updated: "2026-05-10"
---

# Receipt Reprint Approval Flow
> Control receipt reprint where tenant policy requires manager approval.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Control receipt reprint where tenant policy requires manager approval.
...
|---|---|---|
| `receipts` | Supports receipt reprint approval flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `receipt_print_logs` | Supports receipt reprint approval flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `audit_logs` | Supports receipt reprint approval flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/manager/receipt-reprints/{id}/approve` | Supports this flow | JWT + tenant context + `receipt.reprint.approve` |
...
## 14. API Example
```http
POST /api/manager/receipt-reprints/{id}/approve
...
{ "permission": "receipt.reprint.approve", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Receipt Reprint Approval Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
