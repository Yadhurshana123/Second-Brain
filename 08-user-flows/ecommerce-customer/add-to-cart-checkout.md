...
permission_anchor: "storefront.checkout.create"
updated: "2026-05-10"
---

# Add to Cart and Checkout Flow
> Create cart, add variants, validate price/stock, place order, and initiate payment/fulfillment status.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Create cart, add variants, validate price/stock, place order, and initiate payment/fulfillment status.
...
|---|---|---|
| `carts` | Supports add to cart and checkout flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `cart_items` | Supports add to cart and checkout flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `orders` | Supports add to cart and checkout flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `order_items` | Supports add to cart and checkout flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `order_addresses` | Supports add to cart and checkout flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `stock_reservations` | Supports add to cart and checkout flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/storefront/carts` | Supports this flow | JWT + tenant context + `storefront.checkout.create` |
| `POST /api/storefront/checkout` | Supports this flow | JWT + tenant context + `storefront.checkout.create` |
...
{ "permission": "storefront.checkout.create", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Add to Cart and Checkout Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
