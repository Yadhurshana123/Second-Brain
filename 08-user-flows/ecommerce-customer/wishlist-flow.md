...
permission_anchor: "wishlist.manage"
updated: "2026-05-10"
---

# Wishlist Flow
> Save, view, and remove tenant-scoped customer wishlist items.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Save, view, and remove tenant-scoped customer wishlist items.
...
|---|---|---|
| `wishlists` | Supports wishlist flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `wishlist_items` | Supports wishlist flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `customers` | Supports wishlist flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/storefront/wishlists/{id}/items` | Supports this flow | JWT + tenant context + `wishlist.manage` |
| `DELETE /api/storefront/wishlist-items/{id}` | Supports this flow | JWT + tenant context + `wishlist.manage` |
...
## 14. API Example
```http
POST /api/storefront/wishlists/{id}/items
...
{ "permission": "wishlist.manage", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Wishlist Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
