---
title: "Customer Register and Login Flow"
...
permission_anchor: "customer.auth.login"
updated: "2026-05-10"
---

# Customer Register and Login Flow
> Register or authenticate customer account with tenant-scoped identity and JWT-backed session.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Register or authenticate customer account with tenant-scoped identity and JWT-backed session.
...
|---|---|---|
| `customers` | Supports customer register and login flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `customer_auth_accounts` | Supports customer register and login flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `customer_auth_identities` | Supports customer register and login flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `otp_verifications` | Supports customer register and login flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/storefront/auth/register` | Supports this flow | JWT + tenant context + `customer.auth.login` |
| `POST /api/storefront/auth/login` | Supports this flow | JWT + tenant context + `customer.auth.login` |
...
{ "permission": "customer.auth.login", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Customer Register and Login Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
