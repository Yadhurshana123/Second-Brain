...
permission_anchor: "review.create"
updated: "2026-05-10"
---

# Product Review Flow
> Submit product review linked to customer and optional order, then route moderation before display.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Submit product review linked to customer and optional order, then route moderation before display.
...
|---|---|---|
| `product_reviews` | Supports product review flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `orders` | Supports product review flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `customers` | Supports product review flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `products` | Supports product review flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/storefront/products/{id}/reviews` | Supports this flow | JWT + tenant context + `review.create` |
...
{ "permission": "review.create", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Product Review Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
