...
permission_anchor: "storefront.product.view"
updated: "2026-05-10"
---

# Product Browse Flow
> Browse tenant-published products and variants using e-commerce visibility, pricing, and stock rules.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Browse tenant-published products and variants using e-commerce visibility, pricing, and stock rules.
...
|---|---|---|
| `products` | Supports product browse flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `product_variants` | Supports product browse flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `product_images` | Supports product browse flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `price_list_items` | Supports product browse flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `inventory_balances` | Supports product browse flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `GET /api/storefront/products` | Supports this flow | JWT + tenant context + `storefront.product.view` |
| `GET /api/storefront/products/{slug}` | Supports this flow | JWT + tenant context + `storefront.product.view` |
...
{ "permission": "storefront.product.view", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Product Browse Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
