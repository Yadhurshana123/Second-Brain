...
permission_anchor: "customer.order.view"
updated: "2026-05-10"
---

# Order Tracking Flow
> Allow customer to view order, payment, fulfillment, delivery, and status history under tenant context.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Allow customer to view order, payment, fulfillment, delivery, and status history under tenant context.
...
|---|---|---|
| `orders` | Supports order tracking flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `order_status_history` | Supports order tracking flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `deliveries` | Supports order tracking flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `delivery_tracking` | Supports order tracking flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `GET /api/storefront/orders/{orderNumber}/tracking` | Supports this flow | JWT + tenant context + `customer.order.view` |
...
## 14. API Example
```http
GET /api/storefront/orders/{orderNumber}/tracking
...
{ "permission": "customer.order.view", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Order Tracking Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
