...
permission_anchor: "fulfillment.manage"
updated: "2026-05-10"
---

# Fulfillment Flow
> Pick, pack, reserve, release, and complete fulfillment for delivery or pickup orders.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Pick, pack, reserve, release, and complete fulfillment for delivery or pickup orders.
...
|---|---|---|
| `deliveries` | Supports fulfillment flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `delivery_items` | Supports fulfillment flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `stock_reservations` | Supports fulfillment flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `stock_movements` | Supports fulfillment flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/ops/orders/{id}/fulfillment` | Supports this flow | JWT + tenant context + `fulfillment.manage` |
| `POST /api/ops/deliveries/{id}/status` | Supports this flow | JWT + tenant context + `fulfillment.manage` |
...
## 14. API Example
```http
POST /api/ops/orders/{id}/fulfillment
...
{ "permission": "fulfillment.manage", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Fulfillment Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
