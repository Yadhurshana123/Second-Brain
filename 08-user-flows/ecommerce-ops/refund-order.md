...
permission_anchor: "order.refund"
updated: "2026-05-10"
---

# Refund Order Flow
> Create refund against captured order payment and update order payment/refund states.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Create refund against captured order payment and update order payment/refund states.
...
|---|---|---|
| `refunds` | Supports refund order flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `payments` | Supports refund order flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `order_payment_allocations` | Supports refund order flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `order_status_history` | Supports refund order flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/ops/orders/{id}/refunds` | Supports this flow | JWT + tenant context + `order.refund` |
...
## 14. API Example
```http
POST /api/ops/orders/{id}/refunds
...
{ "permission": "order.refund", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Refund Order Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
