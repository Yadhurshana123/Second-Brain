...
permission_anchor: "order.process"
updated: "2026-05-10"
---

# Order Processing Flow
> Move online orders from confirmed state into operational processing using valid status transitions.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Move online orders from confirmed state into operational processing using valid status transitions.
...
|---|---|---|
| `orders` | Supports order processing flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `order_status_transitions` | Supports order processing flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `order_status_history` | Supports order processing flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/ops/orders/{id}/process` | Supports this flow | JWT + tenant context + `order.process` |
...
## 14. API Example
```http
POST /api/ops/orders/{id}/process
...
{ "permission": "order.process", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Order Processing Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
