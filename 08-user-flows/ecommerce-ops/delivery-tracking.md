...
permission_anchor: "delivery.tracking.update"
updated: "2026-05-10"
---

# Delivery Tracking Flow
> Record delivery/pickup tracking events and expose customer-safe status updates.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Record delivery/pickup tracking events and expose customer-safe status updates.
...
|---|---|---|
| `deliveries` | Supports delivery tracking flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `delivery_tracking` | Supports delivery tracking flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `order_status_history` | Supports delivery tracking flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/ops/deliveries/{id}/tracking` | Supports this flow | JWT + tenant context + `delivery.tracking.update` |
...
## 14. API Example
```http
POST /api/ops/deliveries/{id}/tracking
...
{ "permission": "delivery.tracking.update", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Delivery Tracking Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
