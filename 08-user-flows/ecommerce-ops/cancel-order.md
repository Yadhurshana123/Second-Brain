...
permission_anchor: "order.cancel"
updated: "2026-05-10"
---

# Cancel Order Flow
> Cancel eligible online order, release stock reservation, and handle payment/refund status correctly.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Cancel eligible online order, release stock reservation, and handle payment/refund status correctly.
...
|---|---|---|
| `orders` | Supports cancel order flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `stock_reservations` | Supports cancel order flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `payments` | Supports cancel order flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `refunds` | Supports cancel order flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `order_status_history` | Supports cancel order flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/ops/orders/{id}/cancel` | Supports this flow | JWT + tenant context + `order.cancel` |
...
{ "permission": "order.cancel", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Cancel Order Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
