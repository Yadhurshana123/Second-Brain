...
permission_anchor: "platform.support.access"
updated: "2026-05-10"
---

# Emergency Support Flow
> Allow controlled platform support investigation without bypassing tenant isolation or audit rules.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Allow controlled platform support investigation without bypassing tenant isolation or audit rules.
...
|---|---|---|
| `audit_logs` | Supports emergency support flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `tenants` | Supports emergency support flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `users` | Supports emergency support flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/platform/support-sessions` | Supports this flow | JWT + tenant context + `platform.support.access` |
| `POST /api/platform/support-sessions/{id}/actions` | Supports this flow | JWT + tenant context + `platform.support.access` |
...
{ "permission": "platform.support.access", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Emergency Support Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
