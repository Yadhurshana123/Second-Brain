...
permission_anchor: "platform.tenant.create"
updated: "2026-05-10"
---

# Tenant Onboarding Flow
> Create a tenant, configure business profile, select operating mode, enable entitled platform features, and prepare initial tenant administration.
...
| Access model | Platform-only administration; tenant data access remains audited and isolated. |
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Create a tenant, configure business profile, select operating mode, enable entitled platform features, and prepare initial tenant administration.
...
|---|---|---|
| `tenants` | Supports tenant onboarding flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `tenant_feature_entitlements` | Supports tenant onboarding flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `ui_themes` | Supports tenant onboarding flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `tenant_settings` | Supports tenant onboarding flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `audit_logs` | Supports tenant onboarding flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/platform/tenants` | Supports this flow | JWT + tenant context + `platform.tenant.create` |
| `POST /api/platform/tenants/{tenantId}/features` | Supports this flow | JWT + tenant context + `platform.tenant.create` |
| `POST /api/platform/tenants/{tenantId}/admin-user` | Supports this flow | JWT + tenant context + `platform.tenant.create` |
...
{ "permission": "platform.tenant.create", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Tenant Onboarding Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
