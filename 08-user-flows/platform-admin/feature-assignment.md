---
title: "Platform Feature Assignment Flow"
...
permission_anchor: "platform.feature.assign"
updated: "2026-05-10"
---

# Platform Feature Assignment Flow
> Enable platform-owned features for a tenant and control which tenant roles can use them.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Enable platform-owned features for a tenant and control which tenant roles can use them.
...
|---|---|---|
| `platform_features` | Supports platform feature assignment flow | Platform-owned; enforce tenant consistency. |
| `tenant_feature_entitlements` | Supports platform feature assignment flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `role_feature_assignments` | Supports platform feature assignment flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `feature_flags` | Supports platform feature assignment flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `PUT /api/platform/tenants/{tenantId}/feature-entitlements` | Supports this flow | JWT + tenant context + `platform.feature.assign` |
| `GET /api/platform/features` | Supports this flow | JWT + tenant context + `platform.feature.assign` |
...
## 14. API Example
```http
PUT /api/platform/tenants/{tenantId}/feature-entitlements
...
{ "permission": "platform.feature.assign", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Platform Feature Assignment Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
