...
permission_anchor: "platform.user.manage"
updated: "2026-05-10"
---

# Platform User Management Flow
> Manage platform administrators separately from tenant staff users.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Manage platform administrators separately from tenant staff users.
...
|---|---|---|
| `platform_users` | Supports platform user management flow | Platform-owned; enforce tenant consistency. |
| `audit_logs` | Supports platform user management flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/platform/users` | Supports this flow | JWT + tenant context + `platform.user.manage` |
| `PATCH /api/platform/users/{id}/status` | Supports this flow | JWT + tenant context + `platform.user.manage` |
...
{ "permission": "platform.user.manage", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Platform User Management Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
