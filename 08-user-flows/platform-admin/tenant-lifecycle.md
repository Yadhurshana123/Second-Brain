...
permission_anchor: "platform.tenant.status.update"
updated: "2026-05-10"
---

# Tenant Lifecycle Flow
> Activate, suspend, reactivate, or retire tenant accounts while preserving data isolation and auditability.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Activate, suspend, reactivate, or retire tenant accounts while preserving data isolation and auditability.
...
|---|---|---|
| `tenants` | Supports tenant lifecycle flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `audit_logs` | Supports tenant lifecycle flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `tenant_settings` | Supports tenant lifecycle flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `PATCH /api/platform/tenants/{tenantId}/status` | Supports this flow | JWT + tenant context + `platform.tenant.status.update` |
| `GET /api/platform/tenants/{tenantId}` | Supports this flow | JWT + tenant context + `platform.tenant.status.update` |
...
{ "permission": "platform.tenant.status.update", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Tenant Lifecycle Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
