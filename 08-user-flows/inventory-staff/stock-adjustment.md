...
permission_anchor: "inventory.adjustment.create"
updated: "2026-05-10"
---

# Stock Adjustment Flow
> Draft, approve, and post manual stock increase or decrease with reason and audit trail.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Draft, approve, and post manual stock increase or decrease with reason and audit trail.
...
|---|---|---|
| `stock_adjustments` | Supports stock adjustment flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `stock_adjustment_lines` | Supports stock adjustment flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `stock_movements` | Supports stock adjustment flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/inventory/stock-adjustments` | Supports this flow | JWT + tenant context + `inventory.adjustment.create` |
| `POST /api/inventory/stock-adjustments/{id}/post` | Supports this flow | JWT + tenant context + `inventory.adjustment.create` |
...
{ "permission": "inventory.adjustment.create", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Stock Adjustment Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
