---
title: "Damaged Stock Handling Flow"
...
permission_anchor: "inventory.damage.handle"
updated: "2026-05-10"
---

# Damaged Stock Handling Flow
> Move damaged returned or adjusted stock out of sellable inventory with traceable reason.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Move damaged returned or adjusted stock out of sellable inventory with traceable reason.
...
|---|---|---|
| `returns` | Supports damaged stock handling flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `return_lines` | Supports damaged stock handling flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `stock_adjustments` | Supports damaged stock handling flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `stock_movements` | Supports damaged stock handling flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/inventory/damaged-stock/actions` | Supports this flow | JWT + tenant context + `inventory.damage.handle` |
...
## 14. API Example
```http
POST /api/inventory/damaged-stock/actions
...
{ "permission": "inventory.damage.handle", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Damaged Stock Handling Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
