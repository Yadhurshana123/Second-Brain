...
permission_anchor: "inventory.receipt.post"
updated: "2026-05-10"
---

# Stock Receipt Flow
> Receive supplier stock into outlet inventory and post purchase receipt stock movements.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Receive supplier stock into outlet inventory and post purchase receipt stock movements.
...
|---|---|---|
| `purchase_receipts` | Supports stock receipt flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `purchase_receipt_lines` | Supports stock receipt flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `stock_movements` | Supports stock receipt flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `inventory_balances` | Supports stock receipt flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/inventory/purchase-receipts` | Supports this flow | JWT + tenant context + `inventory.receipt.post` |
| `POST /api/inventory/purchase-receipts/{id}/post` | Supports this flow | JWT + tenant context + `inventory.receipt.post` |
...
{ "permission": "inventory.receipt.post", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Stock Receipt Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
