...
permission_anchor: "inventory.transfer.manage"
updated: "2026-05-10"
---

# Stock Transfer Flow
> Transfer inventory between outlets using transfer-out and transfer-in movements.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Transfer inventory between outlets using transfer-out and transfer-in movements.
...
|---|---|---|
| `stock_transfers` | Supports stock transfer flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `stock_transfer_lines` | Supports stock transfer flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `stock_movements` | Supports stock transfer flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/inventory/transfers` | Supports this flow | JWT + tenant context + `inventory.transfer.manage` |
| `POST /api/inventory/transfers/{id}/ship` | Supports this flow | JWT + tenant context + `inventory.transfer.manage` |
| `POST /api/inventory/transfers/{id}/receive` | Supports this flow | JWT + tenant context + `inventory.transfer.manage` |
...
{ "permission": "inventory.transfer.manage", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Stock Transfer Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
