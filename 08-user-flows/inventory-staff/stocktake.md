...
permission_anchor: "inventory.stocktake.post"
updated: "2026-05-10"
---

# Stocktake Flow
> Count stock, compare expected vs counted quantity, and post gain/loss movements.
...
## 3. Business Purpose
- Supports this approved Unified Commerce operation: Count stock, compare expected vs counted quantity, and post gain/loss movements.
...
|---|---|---|
| `stocktakes` | Supports stocktake flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `stocktake_lines` | Supports stocktake flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |
| `stock_movements` | Supports stocktake flow | Tenant-scoped or tenant-linked; enforce tenant consistency. |

## 8. API Surface
| API | Purpose | Guard |
|---|---|---|
| `POST /api/inventory/stocktakes` | Supports this flow | JWT + tenant context + `inventory.stocktake.post` |
| `POST /api/inventory/stocktakes/{id}/post` | Supports this flow | JWT + tenant context + `inventory.stocktake.post` |
...
{ "permission": "inventory.stocktake.post", "payload": { "example": "flow-specific data" } }
```
...
## 20. Final Rule
The `Stocktake Flow` must remain tenant-aware, permission-controlled, architecture-aligned, and consistent with the approved scope, database design, frontend architecture, backend architecture, and configurable RBAC model.
