---
title: "Discount Requests Feature Specification"
...
# Discount Requests Feature Specification

## Purpose
This document defines the implementation scope for `discount-requests` inside the `discounts-promotions` module of the Unified Commerce platform.
...
|---|---|
| `discount_types` | Source or supporting table for `discount-requests` behavior. |
| `discount_scopes` | Source or supporting table for `discount-requests` behavior. |
| `discount_policies` | Source or supporting table for `discount-requests` behavior. |
| `discount_requests` | Source or supporting table for `discount-requests` behavior. |
| `coupons` | Source or supporting table for `discount-requests` behavior. |
| `discount_applications` | Source or supporting table for `discount-requests` behavior. |
| `coupon_redemptions` | Source or supporting table for `discount-requests` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `discounts-promotions.discount-requests.read` | View data and search records. | Yes |
| `discounts-promotions.discount-requests.create` | Create new records or workflow drafts. | Yes |
| `discounts-promotions.discount-requests.update` | Modify allowed editable fields. | Yes |
| `discounts-promotions.discount-requests.approve` | Approve sensitive actions where applicable. | Yes |
| `discounts-promotions.discount-requests.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class DiscountRequestsService : IDiscountRequestsService
{
    private readonly IDiscountRequestsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<DiscountRequestsDto>> HandleAsync(DiscountRequestsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "discounts-promotions.discount-requests.manage");
        ...
        return ApiResponse.Success(new DiscountRequestsDto());
    }
}
```
...
```ts
export const discountrequestsQueryKey = (tenantId: string, outletId?: string) =>
  ["discounts-promotions", "discount-requests", tenantId, outletId ?? "tenant"] as const;

export function useDiscountRequests(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: discountrequestsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/discounts-promotions/discount-requests"),
    staleTime: 60_000,
  });
}
```
