---
title: "Discount Applications API Specification"
...
# Discount Applications API Specification

## API Ownership
The `discount-applications` API belongs to the `discounts-promotions` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/discounts-promotions/discount-applications` | `discounts-promotions.discount-applications.read` |
| Detail | GET | `/api/v1/discounts-promotions/discount-applications/{id}` | `discounts-promotions.discount-applications.read` |
| Create | POST | `/api/v1/discounts-promotions/discount-applications` | `discounts-promotions.discount-applications.create` |
| Update | PUT | `/api/v1/discounts-promotions/discount-applications/{id}` | `discounts-promotions.discount-applications.update` |
| State action | POST | `/api/v1/discounts-promotions/discount-applications/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/discounts-promotions/discount-applications
...
{
  "featureKey": "discounts-promotions.discount-applications",
...
- Access consideration 1: Permission `discounts-promotions.discount-applications.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `discounts-promotions.discount-applications.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `discounts-promotions.discount-applications.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `discounts-promotions.discount-applications.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `discounts-promotions.discount-applications.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `discounts-promotions.discount-applications.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `discounts-promotions.discount-applications.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `discounts-promotions.discount-applications.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `discounts-promotions.discount-applications.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `discounts-promotions.discount-applications.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `discounts-promotions.discount-applications.read` may show data, while command permissions must be separately configured per role.
