---
title: "Discount Applications Feature Specification"
...
# Discount Applications Feature Specification

## Purpose
This document defines the implementation scope for `discount-applications` inside the `discounts-promotions` module of the Unified Commerce platform.
...
|---|---|
| `discount_types` | Source or supporting table for `discount-applications` behavior. |
| `discount_scopes` | Source or supporting table for `discount-applications` behavior. |
| `discount_policies` | Source or supporting table for `discount-applications` behavior. |
| `discount_requests` | Source or supporting table for `discount-applications` behavior. |
| `coupons` | Source or supporting table for `discount-applications` behavior. |
| `discount_applications` | Source or supporting table for `discount-applications` behavior. |
| `coupon_redemptions` | Source or supporting table for `discount-applications` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `discounts-promotions.discount-applications.read` | View data and search records. | Yes |
| `discounts-promotions.discount-applications.create` | Create new records or workflow drafts. | Yes |
| `discounts-promotions.discount-applications.update` | Modify allowed editable fields. | Yes |
| `discounts-promotions.discount-applications.approve` | Approve sensitive actions where applicable. | Yes |
| `discounts-promotions.discount-applications.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class DiscountApplicationsService : IDiscountApplicationsService
{
    private readonly IDiscountApplicationsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<DiscountApplicationsDto>> HandleAsync(DiscountApplicationsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "discounts-promotions.discount-applications.manage");
        ...
        return ApiResponse.Success(new DiscountApplicationsDto());
    }
}
```
...
```ts
export const discountapplicationsQueryKey = (tenantId: string, outletId?: string) =>
  ["discounts-promotions", "discount-applications", tenantId, outletId ?? "tenant"] as const;

export function useDiscountApplications(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: discountapplicationsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/discounts-promotions/discount-applications"),
    staleTime: 60_000,
  });
}
```
