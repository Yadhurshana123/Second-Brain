---
title: "Coupon Redemptions API Specification"
...
# Coupon Redemptions API Specification

## API Ownership
The `coupon-redemptions` API belongs to the `discounts-promotions` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/discounts-promotions/coupon-redemptions` | `discounts-promotions.coupon-redemptions.read` |
| Detail | GET | `/api/v1/discounts-promotions/coupon-redemptions/{id}` | `discounts-promotions.coupon-redemptions.read` |
| Create | POST | `/api/v1/discounts-promotions/coupon-redemptions` | `discounts-promotions.coupon-redemptions.create` |
| Update | PUT | `/api/v1/discounts-promotions/coupon-redemptions/{id}` | `discounts-promotions.coupon-redemptions.update` |
| State action | POST | `/api/v1/discounts-promotions/coupon-redemptions/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/discounts-promotions/coupon-redemptions
...
{
  "featureKey": "discounts-promotions.coupon-redemptions",
...
- Access consideration 1: Permission `discounts-promotions.coupon-redemptions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `discounts-promotions.coupon-redemptions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `discounts-promotions.coupon-redemptions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `discounts-promotions.coupon-redemptions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `discounts-promotions.coupon-redemptions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `discounts-promotions.coupon-redemptions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `discounts-promotions.coupon-redemptions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `discounts-promotions.coupon-redemptions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `discounts-promotions.coupon-redemptions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `discounts-promotions.coupon-redemptions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `discounts-promotions.coupon-redemptions.read` may show data, while command permissions must be separately configured per role.
