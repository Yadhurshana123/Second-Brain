---
title: "Coupon Redemptions Feature Specification"
...
# Coupon Redemptions Feature Specification

## Purpose
This document defines the implementation scope for `coupon-redemptions` inside the `discounts-promotions` module of the Unified Commerce platform.
...
|---|---|
| `discount_types` | Source or supporting table for `coupon-redemptions` behavior. |
| `discount_scopes` | Source or supporting table for `coupon-redemptions` behavior. |
| `discount_policies` | Source or supporting table for `coupon-redemptions` behavior. |
| `discount_requests` | Source or supporting table for `coupon-redemptions` behavior. |
| `coupons` | Source or supporting table for `coupon-redemptions` behavior. |
| `discount_applications` | Source or supporting table for `coupon-redemptions` behavior. |
| `coupon_redemptions` | Source or supporting table for `coupon-redemptions` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `discounts-promotions.coupon-redemptions.read` | View data and search records. | Yes |
| `discounts-promotions.coupon-redemptions.create` | Create new records or workflow drafts. | Yes |
| `discounts-promotions.coupon-redemptions.update` | Modify allowed editable fields. | Yes |
| `discounts-promotions.coupon-redemptions.approve` | Approve sensitive actions where applicable. | Yes |
| `discounts-promotions.coupon-redemptions.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class CouponRedemptionsService : ICouponRedemptionsService
{
    private readonly ICouponRedemptionsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<CouponRedemptionsDto>> HandleAsync(CouponRedemptionsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "discounts-promotions.coupon-redemptions.manage");
        ...
        return ApiResponse.Success(new CouponRedemptionsDto());
    }
}
```
...
```ts
export const couponredemptionsQueryKey = (tenantId: string, outletId?: string) =>
  ["discounts-promotions", "coupon-redemptions", tenantId, outletId ?? "tenant"] as const;

export function useCouponRedemptions(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: couponredemptionsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/discounts-promotions/coupon-redemptions"),
    staleTime: 60_000,
  });
}
```
