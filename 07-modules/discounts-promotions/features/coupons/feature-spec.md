---
title: "Coupons Feature Specification"
...
# Coupons Feature Specification

## Purpose
This document defines the implementation scope for `coupons` inside the `discounts-promotions` module of the Unified Commerce platform.
...
|---|---|
| `discount_types` | Source or supporting table for `coupons` behavior. |
| `discount_scopes` | Source or supporting table for `coupons` behavior. |
| `discount_policies` | Source or supporting table for `coupons` behavior. |
| `discount_requests` | Source or supporting table for `coupons` behavior. |
| `coupons` | Source or supporting table for `coupons` behavior. |
| `discount_applications` | Source or supporting table for `coupons` behavior. |
| `coupon_redemptions` | Source or supporting table for `coupons` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `discounts-promotions.coupons.read` | View data and search records. | Yes |
| `discounts-promotions.coupons.create` | Create new records or workflow drafts. | Yes |
| `discounts-promotions.coupons.update` | Modify allowed editable fields. | Yes |
| `discounts-promotions.coupons.approve` | Approve sensitive actions where applicable. | Yes |
| `discounts-promotions.coupons.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class CouponsService : ICouponsService
{
    private readonly ICouponsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<CouponsDto>> HandleAsync(CouponsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "discounts-promotions.coupons.manage");
        ...
        return ApiResponse.Success(new CouponsDto());
    }
}
```
...
```ts
export const couponsQueryKey = (tenantId: string, outletId?: string) =>
  ["discounts-promotions", "coupons", tenantId, outletId ?? "tenant"] as const;

export function useCoupons(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: couponsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/discounts-promotions/coupons"),
    staleTime: 60_000,
  });
}
```
