---
title: "Exchange Payment Refund Allocations API Specification"
...
# Exchange Payment Refund Allocations API Specification

## API Ownership
The `exchange-payment-refund-allocations` API belongs to the `returns-exchanges` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/returns-exchanges/exchange-payment-refund-allocations` | `returns-exchanges.exchange-payment-refund-allocations.read` |
| Detail | GET | `/api/v1/returns-exchanges/exchange-payment-refund-allocations/{id}` | `returns-exchanges.exchange-payment-refund-allocations.read` |
| Create | POST | `/api/v1/returns-exchanges/exchange-payment-refund-allocations` | `returns-exchanges.exchange-payment-refund-allocations.create` |
| Update | PUT | `/api/v1/returns-exchanges/exchange-payment-refund-allocations/{id}` | `returns-exchanges.exchange-payment-refund-allocations.update` |
| State action | POST | `/api/v1/returns-exchanges/exchange-payment-refund-allocations/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/returns-exchanges/exchange-payment-refund-allocations
...
{
  "featureKey": "returns-exchanges.exchange-payment-refund-allocations",
...
- Access consideration 1: Permission `returns-exchanges.exchange-payment-refund-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `returns-exchanges.exchange-payment-refund-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `returns-exchanges.exchange-payment-refund-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `returns-exchanges.exchange-payment-refund-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `returns-exchanges.exchange-payment-refund-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `returns-exchanges.exchange-payment-refund-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `returns-exchanges.exchange-payment-refund-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `returns-exchanges.exchange-payment-refund-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `returns-exchanges.exchange-payment-refund-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `returns-exchanges.exchange-payment-refund-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `returns-exchanges.exchange-payment-refund-allocations.read` may show data, while command permissions must be separately configured per role.
