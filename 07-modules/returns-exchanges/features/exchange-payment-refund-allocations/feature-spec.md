---
title: "Exchange Payment Refund Allocations Feature Specification"
...
# Exchange Payment Refund Allocations Feature Specification

## Purpose
This document defines the implementation scope for `exchange-payment-refund-allocations` inside the `returns-exchanges` module of the Unified Commerce platform.
...
|---|---|
| `return_reason_codes` | Source or supporting table for `exchange-payment-refund-allocations` behavior. |
| `returns` | Source or supporting table for `exchange-payment-refund-allocations` behavior. |
| `return_lines` | Source or supporting table for `exchange-payment-refund-allocations` behavior. |
| `return_refund_allocations` | Source or supporting table for `exchange-payment-refund-allocations` behavior. |
| `exchanges` | Source or supporting table for `exchange-payment-refund-allocations` behavior. |
| `exchange_lines` | Source or supporting table for `exchange-payment-refund-allocations` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `returns-exchanges.exchange-payment-refund-allocations.read` | View data and search records. | Yes |
| `returns-exchanges.exchange-payment-refund-allocations.create` | Create new records or workflow drafts. | Yes |
| `returns-exchanges.exchange-payment-refund-allocations.update` | Modify allowed editable fields. | Yes |
| `returns-exchanges.exchange-payment-refund-allocations.approve` | Approve sensitive actions where applicable. | Yes |
| `returns-exchanges.exchange-payment-refund-allocations.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class ExchangePaymentRefundAllocationsService : IExchangePaymentRefundAllocationsService
{
    private readonly IExchangePaymentRefundAllocationsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<ExchangePaymentRefundAllocationsDto>> HandleAsync(ExchangePaymentRefundAllocationsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "returns-exchanges.exchange-payment-refund-allocations.manage");
        ...
        return ApiResponse.Success(new ExchangePaymentRefundAllocationsDto());
    }
}
```
...
```ts
export const exchangepaymentrefundallocationsQueryKey = (tenantId: string, outletId?: string) =>
  ["returns-exchanges", "exchange-payment-refund-allocations", tenantId, outletId ?? "tenant"] as const;

export function useExchangePaymentRefundAllocations(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: exchangepaymentrefundallocationsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/returns-exchanges/exchange-payment-refund-allocations"),
    staleTime: 60_000,
  });
}
```
...
- Related module overview: [[../README|Returns Exchanges Overview]]
