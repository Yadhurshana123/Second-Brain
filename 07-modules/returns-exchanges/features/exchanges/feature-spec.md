---
title: "Exchanges Feature Specification"
...
# Exchanges Feature Specification

## Purpose
This document defines the implementation scope for `exchanges` inside the `returns-exchanges` module of the Unified Commerce platform.
...
|---|---|
| `return_reason_codes` | Source or supporting table for `exchanges` behavior. |
| `returns` | Source or supporting table for `exchanges` behavior. |
| `return_lines` | Source or supporting table for `exchanges` behavior. |
| `return_refund_allocations` | Source or supporting table for `exchanges` behavior. |
| `exchanges` | Source or supporting table for `exchanges` behavior. |
| `exchange_lines` | Source or supporting table for `exchanges` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `returns-exchanges.exchanges.read` | View data and search records. | Yes |
| `returns-exchanges.exchanges.create` | Create new records or workflow drafts. | Yes |
| `returns-exchanges.exchanges.update` | Modify allowed editable fields. | Yes |
| `returns-exchanges.exchanges.approve` | Approve sensitive actions where applicable. | Yes |
| `returns-exchanges.exchanges.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class ExchangesService : IExchangesService
{
    private readonly IExchangesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<ExchangesDto>> HandleAsync(ExchangesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "returns-exchanges.exchanges.manage");
        ...
        return ApiResponse.Success(new ExchangesDto());
    }
}
```
...
```ts
export const exchangesQueryKey = (tenantId: string, outletId?: string) =>
  ["returns-exchanges", "exchanges", tenantId, outletId ?? "tenant"] as const;

export function useExchanges(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: exchangesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/returns-exchanges/exchanges"),
    staleTime: 60_000,
  });
}
```
