---
title: "Exchanges API Specification"
...
# Exchanges API Specification

## API Ownership
The `exchanges` API belongs to the `returns-exchanges` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/returns-exchanges/exchanges` | `returns-exchanges.exchanges.read` |
| Detail | GET | `/api/v1/returns-exchanges/exchanges/{id}` | `returns-exchanges.exchanges.read` |
| Create | POST | `/api/v1/returns-exchanges/exchanges` | `returns-exchanges.exchanges.create` |
| Update | PUT | `/api/v1/returns-exchanges/exchanges/{id}` | `returns-exchanges.exchanges.update` |
| State action | POST | `/api/v1/returns-exchanges/exchanges/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/returns-exchanges/exchanges
...
{
  "featureKey": "returns-exchanges.exchanges",
...
- Access consideration 1: Permission `returns-exchanges.exchanges.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `returns-exchanges.exchanges.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `returns-exchanges.exchanges.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `returns-exchanges.exchanges.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `returns-exchanges.exchanges.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `returns-exchanges.exchanges.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `returns-exchanges.exchanges.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `returns-exchanges.exchanges.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `returns-exchanges.exchanges.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `returns-exchanges.exchanges.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `returns-exchanges.exchanges.read` may show data, while command permissions must be separately configured per role.
