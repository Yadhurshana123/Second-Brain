---
title: "Returns API Specification"
...
## API Ownership
The `returns` API belongs to the `returns-exchanges` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/returns-exchanges/returns` | `returns-exchanges.returns.read` |
| Detail | GET | `/api/v1/returns-exchanges/returns/{id}` | `returns-exchanges.returns.read` |
| Create | POST | `/api/v1/returns-exchanges/returns` | `returns-exchanges.returns.create` |
| Update | PUT | `/api/v1/returns-exchanges/returns/{id}` | `returns-exchanges.returns.update` |
| State action | POST | `/api/v1/returns-exchanges/returns/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/returns-exchanges/returns
...
{
  "featureKey": "returns-exchanges.returns",
...
- Access consideration 1: Permission `returns-exchanges.returns.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `returns-exchanges.returns.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `returns-exchanges.returns.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `returns-exchanges.returns.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `returns-exchanges.returns.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `returns-exchanges.returns.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `returns-exchanges.returns.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `returns-exchanges.returns.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `returns-exchanges.returns.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `returns-exchanges.returns.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `returns-exchanges.returns.read` may show data, while command permissions must be separately configured per role.
