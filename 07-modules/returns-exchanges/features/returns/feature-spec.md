---
title: "Returns Feature Specification"
...
# Returns Feature Specification

## Purpose
This document defines the implementation scope for `returns` inside the `returns-exchanges` module of the Unified Commerce platform.
...
|---|---|
| `return_reason_codes` | Source or supporting table for `returns` behavior. |
| `returns` | Source or supporting table for `returns` behavior. |
| `return_lines` | Source or supporting table for `returns` behavior. |
| `return_refund_allocations` | Source or supporting table for `returns` behavior. |
| `exchanges` | Source or supporting table for `returns` behavior. |
| `exchange_lines` | Source or supporting table for `returns` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `returns-exchanges.returns.read` | View data and search records. | Yes |
| `returns-exchanges.returns.create` | Create new records or workflow drafts. | Yes |
| `returns-exchanges.returns.update` | Modify allowed editable fields. | Yes |
| `returns-exchanges.returns.approve` | Approve sensitive actions where applicable. | Yes |
| `returns-exchanges.returns.void` | Void/cancel/reverse where business rules allow. | Yes |
...
public sealed class ReturnsService : IReturnsService
{
    ...
    public async Task<ApiResponse<ReturnsDto>> HandleAsync(ReturnsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "returns-exchanges.returns.manage");
        ...
        return ApiResponse.Success(new ReturnsDto());
    }
}
```
...
```ts
export const returnsQueryKey = (tenantId: string, outletId?: string) =>
  ["returns-exchanges", "returns", tenantId, outletId ?? "tenant"] as const;

export function useReturns(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: returnsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/returns-exchanges/returns"),
    staleTime: 60_000,
  });
}
```
