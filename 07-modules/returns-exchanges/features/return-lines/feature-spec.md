---
title: "Return Lines Feature Specification"
...
# Return Lines Feature Specification

## Purpose
This document defines the implementation scope for `return-lines` inside the `returns-exchanges` module of the Unified Commerce platform.
...
|---|---|
| `return_reason_codes` | Source or supporting table for `return-lines` behavior. |
| `returns` | Source or supporting table for `return-lines` behavior. |
| `return_lines` | Source or supporting table for `return-lines` behavior. |
| `return_refund_allocations` | Source or supporting table for `return-lines` behavior. |
| `exchanges` | Source or supporting table for `return-lines` behavior. |
| `exchange_lines` | Source or supporting table for `return-lines` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `returns-exchanges.return-lines.read` | View data and search records. | Yes |
| `returns-exchanges.return-lines.create` | Create new records or workflow drafts. | Yes |
| `returns-exchanges.return-lines.update` | Modify allowed editable fields. | Yes |
| `returns-exchanges.return-lines.approve` | Approve sensitive actions where applicable. | Yes |
| `returns-exchanges.return-lines.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class ReturnLinesService : IReturnLinesService
{
    private readonly IReturnLinesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<ReturnLinesDto>> HandleAsync(ReturnLinesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "returns-exchanges.return-lines.manage");
        ...
        return ApiResponse.Success(new ReturnLinesDto());
    }
}
```
...
```ts
export const returnlinesQueryKey = (tenantId: string, outletId?: string) =>
  ["returns-exchanges", "return-lines", tenantId, outletId ?? "tenant"] as const;

export function useReturnLines(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: returnlinesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/returns-exchanges/return-lines"),
    staleTime: 60_000,
  });
}
```
