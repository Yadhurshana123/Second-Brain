---
title: "Return Reason Codes Feature Specification"
...
# Return Reason Codes Feature Specification

## Purpose
This document defines the implementation scope for `return-reason-codes` inside the `returns-exchanges` module of the Unified Commerce platform.
...
|---|---|
| `return_reason_codes` | Source or supporting table for `return-reason-codes` behavior. |
| `returns` | Source or supporting table for `return-reason-codes` behavior. |
| `return_lines` | Source or supporting table for `return-reason-codes` behavior. |
| `return_refund_allocations` | Source or supporting table for `return-reason-codes` behavior. |
| `exchanges` | Source or supporting table for `return-reason-codes` behavior. |
| `exchange_lines` | Source or supporting table for `return-reason-codes` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `returns-exchanges.return-reason-codes.read` | View data and search records. | Yes |
| `returns-exchanges.return-reason-codes.create` | Create new records or workflow drafts. | Yes |
| `returns-exchanges.return-reason-codes.update` | Modify allowed editable fields. | Yes |
| `returns-exchanges.return-reason-codes.approve` | Approve sensitive actions where applicable. | Yes |
| `returns-exchanges.return-reason-codes.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class ReturnReasonCodesService : IReturnReasonCodesService
{
    private readonly IReturnReasonCodesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<ReturnReasonCodesDto>> HandleAsync(ReturnReasonCodesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "returns-exchanges.return-reason-codes.manage");
        ...
        return ApiResponse.Success(new ReturnReasonCodesDto());
    }
}
```
...
```ts
export const returnreasoncodesQueryKey = (tenantId: string, outletId?: string) =>
  ["returns-exchanges", "return-reason-codes", tenantId, outletId ?? "tenant"] as const;

export function useReturnReasonCodes(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: returnreasoncodesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/returns-exchanges/return-reason-codes"),
    staleTime: 60_000,
  });
}
```
