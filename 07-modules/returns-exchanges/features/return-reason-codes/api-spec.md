---
title: "Return Reason Codes API Specification"
...
# Return Reason Codes API Specification

## API Ownership
The `return-reason-codes` API belongs to the `returns-exchanges` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/returns-exchanges/return-reason-codes` | `returns-exchanges.return-reason-codes.read` |
| Detail | GET | `/api/v1/returns-exchanges/return-reason-codes/{id}` | `returns-exchanges.return-reason-codes.read` |
| Create | POST | `/api/v1/returns-exchanges/return-reason-codes` | `returns-exchanges.return-reason-codes.create` |
| Update | PUT | `/api/v1/returns-exchanges/return-reason-codes/{id}` | `returns-exchanges.return-reason-codes.update` |
| State action | POST | `/api/v1/returns-exchanges/return-reason-codes/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/returns-exchanges/return-reason-codes
...
{
  "featureKey": "returns-exchanges.return-reason-codes",
...
- Access consideration 1: Permission `returns-exchanges.return-reason-codes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `returns-exchanges.return-reason-codes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `returns-exchanges.return-reason-codes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `returns-exchanges.return-reason-codes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `returns-exchanges.return-reason-codes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `returns-exchanges.return-reason-codes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `returns-exchanges.return-reason-codes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `returns-exchanges.return-reason-codes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `returns-exchanges.return-reason-codes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `returns-exchanges.return-reason-codes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `returns-exchanges.return-reason-codes.read` may show data, while command permissions must be separately configured per role.
