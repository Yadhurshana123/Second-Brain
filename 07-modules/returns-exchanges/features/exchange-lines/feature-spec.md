---
title: "Exchange Lines Feature Specification"
...
# Exchange Lines Feature Specification

## Purpose
This document defines the implementation scope for `exchange-lines` inside the `returns-exchanges` module of the Unified Commerce platform.
...
|---|---|
| `return_reason_codes` | Source or supporting table for `exchange-lines` behavior. |
| `returns` | Source or supporting table for `exchange-lines` behavior. |
| `return_lines` | Source or supporting table for `exchange-lines` behavior. |
| `return_refund_allocations` | Source or supporting table for `exchange-lines` behavior. |
| `exchanges` | Source or supporting table for `exchange-lines` behavior. |
| `exchange_lines` | Source or supporting table for `exchange-lines` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `returns-exchanges.exchange-lines.read` | View data and search records. | Yes |
| `returns-exchanges.exchange-lines.create` | Create new records or workflow drafts. | Yes |
| `returns-exchanges.exchange-lines.update` | Modify allowed editable fields. | Yes |
| `returns-exchanges.exchange-lines.approve` | Approve sensitive actions where applicable. | Yes |
| `returns-exchanges.exchange-lines.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class ExchangeLinesService : IExchangeLinesService
{
    private readonly IExchangeLinesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<ExchangeLinesDto>> HandleAsync(ExchangeLinesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "returns-exchanges.exchange-lines.manage");
        ...
        return ApiResponse.Success(new ExchangeLinesDto());
    }
}
```
...
```ts
export const exchangelinesQueryKey = (tenantId: string, outletId?: string) =>
  ["returns-exchanges", "exchange-lines", tenantId, outletId ?? "tenant"] as const;

export function useExchangeLines(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: exchangelinesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/returns-exchanges/exchange-lines"),
    staleTime: 60_000,
  });
}
```
