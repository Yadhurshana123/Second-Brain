---
title: "Exchange Lines API Specification"
...
# Exchange Lines API Specification

## API Ownership
The `exchange-lines` API belongs to the `returns-exchanges` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/returns-exchanges/exchange-lines` | `returns-exchanges.exchange-lines.read` |
| Detail | GET | `/api/v1/returns-exchanges/exchange-lines/{id}` | `returns-exchanges.exchange-lines.read` |
| Create | POST | `/api/v1/returns-exchanges/exchange-lines` | `returns-exchanges.exchange-lines.create` |
| Update | PUT | `/api/v1/returns-exchanges/exchange-lines/{id}` | `returns-exchanges.exchange-lines.update` |
| State action | POST | `/api/v1/returns-exchanges/exchange-lines/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/returns-exchanges/exchange-lines
...
{
  "featureKey": "returns-exchanges.exchange-lines",
...
- Access consideration 1: Permission `returns-exchanges.exchange-lines.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `returns-exchanges.exchange-lines.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `returns-exchanges.exchange-lines.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `returns-exchanges.exchange-lines.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `returns-exchanges.exchange-lines.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `returns-exchanges.exchange-lines.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `returns-exchanges.exchange-lines.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `returns-exchanges.exchange-lines.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `returns-exchanges.exchange-lines.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `returns-exchanges.exchange-lines.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `returns-exchanges.exchange-lines.read` may show data, while command permissions must be separately configured per role.
