---
title: "Return Refund Allocations Feature Specification"
...
# Return Refund Allocations Feature Specification

## Purpose
This document defines the implementation scope for `return-refund-allocations` inside the `returns-exchanges` module of the Unified Commerce platform.
...
|---|---|
| `return_reason_codes` | Source or supporting table for `return-refund-allocations` behavior. |
| `returns` | Source or supporting table for `return-refund-allocations` behavior. |
| `return_lines` | Source or supporting table for `return-refund-allocations` behavior. |
| `return_refund_allocations` | Source or supporting table for `return-refund-allocations` behavior. |
| `exchanges` | Source or supporting table for `return-refund-allocations` behavior. |
| `exchange_lines` | Source or supporting table for `return-refund-allocations` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `returns-exchanges.return-refund-allocations.read` | View data and search records. | Yes |
| `returns-exchanges.return-refund-allocations.create` | Create new records or workflow drafts. | Yes |
| `returns-exchanges.return-refund-allocations.update` | Modify allowed editable fields. | Yes |
| `returns-exchanges.return-refund-allocations.approve` | Approve sensitive actions where applicable. | Yes |
| `returns-exchanges.return-refund-allocations.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class ReturnRefundAllocationsService : IReturnRefundAllocationsService
{
    private readonly IReturnRefundAllocationsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<ReturnRefundAllocationsDto>> HandleAsync(ReturnRefundAllocationsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "returns-exchanges.return-refund-allocations.manage");
        ...
        return ApiResponse.Success(new ReturnRefundAllocationsDto());
    }
}
```
...
```ts
export const returnrefundallocationsQueryKey = (tenantId: string, outletId?: string) =>
  ["returns-exchanges", "return-refund-allocations", tenantId, outletId ?? "tenant"] as const;

export function useReturnRefundAllocations(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: returnrefundallocationsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/returns-exchanges/return-refund-allocations"),
    staleTime: 60_000,
  });
}
```
