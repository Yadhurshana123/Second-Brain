---
title: "Offline Sync Conflicts Feature Specification"
...
# Offline Sync Conflicts Feature Specification

## Purpose
This document defines the implementation scope for `offline-sync-conflicts` inside the `offline-sync` module of the Unified Commerce platform.
...
|---|---|
| `offline_sync_batches` | Source or supporting table for `offline-sync-conflicts` behavior. |
| `offline_sync_items` | Source or supporting table for `offline-sync-conflicts` behavior. |
| `offline_sale_sync_queue` | Source or supporting table for `offline-sync-conflicts` behavior. |
| `offline_payment_sync_queue` | Source or supporting table for `offline-sync-conflicts` behavior. |
| `offline_sync_conflicts` | Source or supporting table for `offline-sync-conflicts` behavior. |
| `offline_sync_audit_logs` | Source or supporting table for `offline-sync-conflicts` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `offline-sync.offline-sync-conflicts.read` | View data and search records. | Yes |
| `offline-sync.offline-sync-conflicts.create` | Create new records or workflow drafts. | Yes |
| `offline-sync.offline-sync-conflicts.update` | Modify allowed editable fields. | Yes |
| `offline-sync.offline-sync-conflicts.approve` | Approve sensitive actions where applicable. | Yes |
| `offline-sync.offline-sync-conflicts.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class OfflineSyncConflictsService : IOfflineSyncConflictsService
{
    private readonly IOfflineSyncConflictsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<OfflineSyncConflictsDto>> HandleAsync(OfflineSyncConflictsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "offline-sync.offline-sync-conflicts.manage");
        ...
        return ApiResponse.Success(new OfflineSyncConflictsDto());
    }
}
```
...
```ts
export const offlinesyncconflictsQueryKey = (tenantId: string, outletId?: string) =>
  ["offline-sync", "offline-sync-conflicts", tenantId, outletId ?? "tenant"] as const;

export function useOfflineSyncConflicts(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: offlinesyncconflictsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/offline-sync/offline-sync-conflicts"),
    staleTime: 60_000,
  });
}
```
