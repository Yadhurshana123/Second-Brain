---
title: "Offline Sync Audit Logs Feature Specification"
...
# Offline Sync Audit Logs Feature Specification

## Purpose
This document defines the implementation scope for `offline-sync-audit-logs` inside the `offline-sync` module of the Unified Commerce platform.
...
|---|---|
| `offline_sync_batches` | Source or supporting table for `offline-sync-audit-logs` behavior. |
| `offline_sync_items` | Source or supporting table for `offline-sync-audit-logs` behavior. |
| `offline_sale_sync_queue` | Source or supporting table for `offline-sync-audit-logs` behavior. |
| `offline_payment_sync_queue` | Source or supporting table for `offline-sync-audit-logs` behavior. |
| `offline_sync_conflicts` | Source or supporting table for `offline-sync-audit-logs` behavior. |
| `offline_sync_audit_logs` | Source or supporting table for `offline-sync-audit-logs` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `offline-sync.offline-sync-audit-logs.read` | View data and search records. | Yes |
| `offline-sync.offline-sync-audit-logs.create` | Create new records or workflow drafts. | Yes |
| `offline-sync.offline-sync-audit-logs.update` | Modify allowed editable fields. | Yes |
| `offline-sync.offline-sync-audit-logs.approve` | Approve sensitive actions where applicable. | Yes |
| `offline-sync.offline-sync-audit-logs.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class OfflineSyncAuditLogsService : IOfflineSyncAuditLogsService
{
    private readonly IOfflineSyncAuditLogsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<OfflineSyncAuditLogsDto>> HandleAsync(OfflineSyncAuditLogsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "offline-sync.offline-sync-audit-logs.manage");
        ...
        return ApiResponse.Success(new OfflineSyncAuditLogsDto());
    }
}
```
...
```ts
export const offlinesyncauditlogsQueryKey = (tenantId: string, outletId?: string) =>
  ["offline-sync", "offline-sync-audit-logs", tenantId, outletId ?? "tenant"] as const;

export function useOfflineSyncAuditLogs(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: offlinesyncauditlogsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/offline-sync/offline-sync-audit-logs"),
    staleTime: 60_000,
  });
}
```
