---
title: "Offline Sync Batches API Specification"
...
# Offline Sync Batches API Specification

## API Ownership
The `offline-sync-batches` API belongs to the `offline-sync` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/offline-sync/offline-sync-batches` | `offline-sync.offline-sync-batches.read` |
| Detail | GET | `/api/v1/offline-sync/offline-sync-batches/{id}` | `offline-sync.offline-sync-batches.read` |
| Create | POST | `/api/v1/offline-sync/offline-sync-batches` | `offline-sync.offline-sync-batches.create` |
| Update | PUT | `/api/v1/offline-sync/offline-sync-batches/{id}` | `offline-sync.offline-sync-batches.update` |
| State action | POST | `/api/v1/offline-sync/offline-sync-batches/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/offline-sync/offline-sync-batches
...
{
  "featureKey": "offline-sync.offline-sync-batches",
...
- Access consideration 1: Permission `offline-sync.offline-sync-batches.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `offline-sync.offline-sync-batches.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `offline-sync.offline-sync-batches.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `offline-sync.offline-sync-batches.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `offline-sync.offline-sync-batches.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `offline-sync.offline-sync-batches.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `offline-sync.offline-sync-batches.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `offline-sync.offline-sync-batches.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `offline-sync.offline-sync-batches.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `offline-sync.offline-sync-batches.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `offline-sync.offline-sync-batches.read` may show data, while command permissions must be separately configured per role.
