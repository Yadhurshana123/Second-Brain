---
title: "Offline Sync Batches Feature Specification"
...
# Offline Sync Batches Feature Specification

## Purpose
This document defines the implementation scope for `offline-sync-batches` inside the `offline-sync` module of the Unified Commerce platform.
...
|---|---|
| `offline_sync_batches` | Source or supporting table for `offline-sync-batches` behavior. |
| `offline_sync_items` | Source or supporting table for `offline-sync-batches` behavior. |
| `offline_sale_sync_queue` | Source or supporting table for `offline-sync-batches` behavior. |
| `offline_payment_sync_queue` | Source or supporting table for `offline-sync-batches` behavior. |
| `offline_sync_conflicts` | Source or supporting table for `offline-sync-batches` behavior. |
| `offline_sync_audit_logs` | Source or supporting table for `offline-sync-batches` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `offline-sync.offline-sync-batches.read` | View data and search records. | Yes |
| `offline-sync.offline-sync-batches.create` | Create new records or workflow drafts. | Yes |
| `offline-sync.offline-sync-batches.update` | Modify allowed editable fields. | Yes |
| `offline-sync.offline-sync-batches.approve` | Approve sensitive actions where applicable. | Yes |
| `offline-sync.offline-sync-batches.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class OfflineSyncBatchesService : IOfflineSyncBatchesService
{
    private readonly IOfflineSyncBatchesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<OfflineSyncBatchesDto>> HandleAsync(OfflineSyncBatchesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "offline-sync.offline-sync-batches.manage");
        ...
        return ApiResponse.Success(new OfflineSyncBatchesDto());
    }
}
```
...
```ts
export const offlinesyncbatchesQueryKey = (tenantId: string, outletId?: string) =>
  ["offline-sync", "offline-sync-batches", tenantId, outletId ?? "tenant"] as const;

export function useOfflineSyncBatches(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: offlinesyncbatchesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/offline-sync/offline-sync-batches"),
    staleTime: 60_000,
  });
}
```
