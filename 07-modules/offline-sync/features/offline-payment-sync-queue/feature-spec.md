---
title: "Offline Payment Sync Queue Feature Specification"
...
# Offline Payment Sync Queue Feature Specification

## Purpose
This document defines the implementation scope for `offline-payment-sync-queue` inside the `offline-sync` module of the Unified Commerce platform.
...
|---|---|
| `offline_sync_batches` | Source or supporting table for `offline-payment-sync-queue` behavior. |
| `offline_sync_items` | Source or supporting table for `offline-payment-sync-queue` behavior. |
| `offline_sale_sync_queue` | Source or supporting table for `offline-payment-sync-queue` behavior. |
| `offline_payment_sync_queue` | Source or supporting table for `offline-payment-sync-queue` behavior. |
| `offline_sync_conflicts` | Source or supporting table for `offline-payment-sync-queue` behavior. |
| `offline_sync_audit_logs` | Source or supporting table for `offline-payment-sync-queue` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `offline-sync.offline-payment-sync-queue.read` | View data and search records. | Yes |
| `offline-sync.offline-payment-sync-queue.create` | Create new records or workflow drafts. | Yes |
| `offline-sync.offline-payment-sync-queue.update` | Modify allowed editable fields. | Yes |
| `offline-sync.offline-payment-sync-queue.approve` | Approve sensitive actions where applicable. | Yes |
| `offline-sync.offline-payment-sync-queue.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class OfflinePaymentSyncQueueService : IOfflinePaymentSyncQueueService
{
    private readonly IOfflinePaymentSyncQueueRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<OfflinePaymentSyncQueueDto>> HandleAsync(OfflinePaymentSyncQueueRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "offline-sync.offline-payment-sync-queue.manage");
        ...
        return ApiResponse.Success(new OfflinePaymentSyncQueueDto());
    }
}
```
...
```ts
export const offlinepaymentsyncqueueQueryKey = (tenantId: string, outletId?: string) =>
  ["offline-sync", "offline-payment-sync-queue", tenantId, outletId ?? "tenant"] as const;

export function useOfflinePaymentSyncQueue(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: offlinepaymentsyncqueueQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/offline-sync/offline-payment-sync-queue"),
    staleTime: 60_000,
  });
}
```
