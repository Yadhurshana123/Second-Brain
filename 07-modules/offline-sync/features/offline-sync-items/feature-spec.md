---
title: "Offline Sync Items Feature Specification"
...
# Offline Sync Items Feature Specification

## Purpose
This document defines the implementation scope for `offline-sync-items` inside the `offline-sync` module of the Unified Commerce platform.
...
|---|---|
| `offline_sync_batches` | Source or supporting table for `offline-sync-items` behavior. |
| `offline_sync_items` | Source or supporting table for `offline-sync-items` behavior. |
| `offline_sale_sync_queue` | Source or supporting table for `offline-sync-items` behavior. |
| `offline_payment_sync_queue` | Source or supporting table for `offline-sync-items` behavior. |
| `offline_sync_conflicts` | Source or supporting table for `offline-sync-items` behavior. |
| `offline_sync_audit_logs` | Source or supporting table for `offline-sync-items` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `offline-sync.offline-sync-items.read` | View data and search records. | Yes |
| `offline-sync.offline-sync-items.create` | Create new records or workflow drafts. | Yes |
| `offline-sync.offline-sync-items.update` | Modify allowed editable fields. | Yes |
| `offline-sync.offline-sync-items.approve` | Approve sensitive actions where applicable. | Yes |
| `offline-sync.offline-sync-items.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class OfflineSyncItemsService : IOfflineSyncItemsService
{
    private readonly IOfflineSyncItemsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<OfflineSyncItemsDto>> HandleAsync(OfflineSyncItemsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "offline-sync.offline-sync-items.manage");
        ...
        return ApiResponse.Success(new OfflineSyncItemsDto());
    }
}
```
...
```ts
export const offlinesyncitemsQueryKey = (tenantId: string, outletId?: string) =>
  ["offline-sync", "offline-sync-items", tenantId, outletId ?? "tenant"] as const;

export function useOfflineSyncItems(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: offlinesyncitemsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/offline-sync/offline-sync-items"),
    staleTime: 60_000,
  });
}
```
