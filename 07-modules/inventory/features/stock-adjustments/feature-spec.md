---
title: "Stock Adjustments Feature Specification"
...
# Stock Adjustments Feature Specification

## Purpose
This document defines the implementation scope for `stock-adjustments` inside the `inventory` module of the Unified Commerce platform.
...
|---|---|
| `inventory_balances` | Source or supporting table for `stock-adjustments` behavior. |
| `inventory_channel_allocations` | Source or supporting table for `stock-adjustments` behavior. |
| `stock_movements` | Source or supporting table for `stock-adjustments` behavior. |
| `stock_reservations` | Source or supporting table for `stock-adjustments` behavior. |
| `stock_transfers` | Source or supporting table for `stock-adjustments` behavior. |
| `stocktakes` | Source or supporting table for `stock-adjustments` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `inventory.stock-adjustments.read` | View data and search records. | Yes |
| `inventory.stock-adjustments.create` | Create new records or workflow drafts. | Yes |
| `inventory.stock-adjustments.update` | Modify allowed editable fields. | Yes |
| `inventory.stock-adjustments.approve` | Approve sensitive actions where applicable. | Yes |
| `inventory.stock-adjustments.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class StockAdjustmentsService : IStockAdjustmentsService
{
    private readonly IStockAdjustmentsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<StockAdjustmentsDto>> HandleAsync(StockAdjustmentsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "inventory.stock-adjustments.manage");
        ...
        return ApiResponse.Success(new StockAdjustmentsDto());
    }
}
```
...
```ts
export const stockadjustmentsQueryKey = (tenantId: string, outletId?: string) =>
  ["inventory", "stock-adjustments", tenantId, outletId ?? "tenant"] as const;

export function useStockAdjustments(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: stockadjustmentsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/inventory/stock-adjustments"),
    staleTime: 60_000,
  });
}
```
