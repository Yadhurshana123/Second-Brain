---
title: "Stock Adjustments API Specification"
...
# Stock Adjustments API Specification

## API Ownership
The `stock-adjustments` API belongs to the `inventory` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/inventory/stock-adjustments` | `inventory.stock-adjustments.read` |
| Detail | GET | `/api/v1/inventory/stock-adjustments/{id}` | `inventory.stock-adjustments.read` |
| Create | POST | `/api/v1/inventory/stock-adjustments` | `inventory.stock-adjustments.create` |
| Update | PUT | `/api/v1/inventory/stock-adjustments/{id}` | `inventory.stock-adjustments.update` |
| State action | POST | `/api/v1/inventory/stock-adjustments/{id}/actions/{action}` | Action-specific permission |
...
{
  "featureKey": "inventory.stock-adjustments",
...
- Access consideration 1: Permission `inventory.stock-adjustments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `inventory.stock-adjustments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `inventory.stock-adjustments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `inventory.stock-adjustments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `inventory.stock-adjustments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `inventory.stock-adjustments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `inventory.stock-adjustments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `inventory.stock-adjustments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `inventory.stock-adjustments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `inventory.stock-adjustments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `inventory.stock-adjustments.read` may show data, while command permissions must be separately configured per role.
