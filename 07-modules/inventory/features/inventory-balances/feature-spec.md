---
title: "Inventory Balances Feature Specification"
...
# Inventory Balances Feature Specification

## Purpose
This document defines the implementation scope for `inventory-balances` inside the `inventory` module of the Unified Commerce platform.
...
|---|---|
| `inventory_balances` | Source or supporting table for `inventory-balances` behavior. |
| `inventory_channel_allocations` | Source or supporting table for `inventory-balances` behavior. |
| `stock_movements` | Source or supporting table for `inventory-balances` behavior. |
| `stock_reservations` | Source or supporting table for `inventory-balances` behavior. |
| `stock_transfers` | Source or supporting table for `inventory-balances` behavior. |
| `stocktakes` | Source or supporting table for `inventory-balances` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `inventory.inventory-balances.read` | View data and search records. | Yes |
| `inventory.inventory-balances.create` | Create new records or workflow drafts. | Yes |
| `inventory.inventory-balances.update` | Modify allowed editable fields. | Yes |
| `inventory.inventory-balances.approve` | Approve sensitive actions where applicable. | Yes |
| `inventory.inventory-balances.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class InventoryBalancesService : IInventoryBalancesService
{
    private readonly IInventoryBalancesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<InventoryBalancesDto>> HandleAsync(InventoryBalancesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "inventory.inventory-balances.manage");
        ...
        return ApiResponse.Success(new InventoryBalancesDto());
    }
}
```
...
```ts
export const inventorybalancesQueryKey = (tenantId: string, outletId?: string) =>
  ["inventory", "inventory-balances", tenantId, outletId ?? "tenant"] as const;

export function useInventoryBalances(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: inventorybalancesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/inventory/inventory-balances"),
    staleTime: 60_000,
  });
}
```
