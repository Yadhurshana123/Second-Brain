---
title: "Inventory Balances API Specification"
...
# Inventory Balances API Specification

## API Ownership
The `inventory-balances` API belongs to the `inventory` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/inventory/inventory-balances` | `inventory.inventory-balances.read` |
| Detail | GET | `/api/v1/inventory/inventory-balances/{id}` | `inventory.inventory-balances.read` |
| Create | POST | `/api/v1/inventory/inventory-balances` | `inventory.inventory-balances.create` |
| Update | PUT | `/api/v1/inventory/inventory-balances/{id}` | `inventory.inventory-balances.update` |
| State action | POST | `/api/v1/inventory/inventory-balances/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/inventory/inventory-balances
...
{
  "featureKey": "inventory.inventory-balances",
...
- Access consideration 1: Permission `inventory.inventory-balances.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `inventory.inventory-balances.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `inventory.inventory-balances.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `inventory.inventory-balances.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `inventory.inventory-balances.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `inventory.inventory-balances.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `inventory.inventory-balances.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `inventory.inventory-balances.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `inventory.inventory-balances.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `inventory.inventory-balances.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `inventory.inventory-balances.read` may show data, while command permissions must be separately configured per role.
