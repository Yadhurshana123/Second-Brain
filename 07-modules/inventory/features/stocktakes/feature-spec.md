---
title: "Stocktakes Feature Specification"
...
# Stocktakes Feature Specification

## Purpose
This document defines the implementation scope for `stocktakes` inside the `inventory` module of the Unified Commerce platform.
...
|---|---|
| `inventory_balances` | Source or supporting table for `stocktakes` behavior. |
| `inventory_channel_allocations` | Source or supporting table for `stocktakes` behavior. |
| `stock_movements` | Source or supporting table for `stocktakes` behavior. |
| `stock_reservations` | Source or supporting table for `stocktakes` behavior. |
| `stock_transfers` | Source or supporting table for `stocktakes` behavior. |
| `stocktakes` | Source or supporting table for `stocktakes` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `inventory.stocktakes.read` | View data and search records. | Yes |
| `inventory.stocktakes.create` | Create new records or workflow drafts. | Yes |
| `inventory.stocktakes.update` | Modify allowed editable fields. | Yes |
| `inventory.stocktakes.approve` | Approve sensitive actions where applicable. | Yes |
| `inventory.stocktakes.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class StocktakesService : IStocktakesService
{
    private readonly IStocktakesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<StocktakesDto>> HandleAsync(StocktakesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "inventory.stocktakes.manage");
        ...
        return ApiResponse.Success(new StocktakesDto());
    }
}
```
...
```ts
export const stocktakesQueryKey = (tenantId: string, outletId?: string) =>
  ["inventory", "stocktakes", tenantId, outletId ?? "tenant"] as const;

export function useStocktakes(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: stocktakesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/inventory/stocktakes"),
    staleTime: 60_000,
  });
}
```
