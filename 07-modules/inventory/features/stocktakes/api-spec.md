---
title: "Stocktakes API Specification"
...
# Stocktakes API Specification

## API Ownership
The `stocktakes` API belongs to the `inventory` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/inventory/stocktakes` | `inventory.stocktakes.read` |
| Detail | GET | `/api/v1/inventory/stocktakes/{id}` | `inventory.stocktakes.read` |
| Create | POST | `/api/v1/inventory/stocktakes` | `inventory.stocktakes.create` |
| Update | PUT | `/api/v1/inventory/stocktakes/{id}` | `inventory.stocktakes.update` |
| State action | POST | `/api/v1/inventory/stocktakes/{id}/actions/{action}` | Action-specific permission |
...
{
  "featureKey": "inventory.stocktakes",
...
- Access consideration 1: Permission `inventory.stocktakes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `inventory.stocktakes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `inventory.stocktakes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `inventory.stocktakes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `inventory.stocktakes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `inventory.stocktakes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `inventory.stocktakes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `inventory.stocktakes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `inventory.stocktakes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `inventory.stocktakes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `inventory.stocktakes.read` may show data, while command permissions must be separately configured per role.
