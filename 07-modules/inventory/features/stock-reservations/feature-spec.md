---
title: "Stock Reservations Feature Specification"
...
# Stock Reservations Feature Specification

## Purpose
This document defines the implementation scope for `stock-reservations` inside the `inventory` module of the Unified Commerce platform.
...
|---|---|
| `inventory_balances` | Source or supporting table for `stock-reservations` behavior. |
| `inventory_channel_allocations` | Source or supporting table for `stock-reservations` behavior. |
| `stock_movements` | Source or supporting table for `stock-reservations` behavior. |
| `stock_reservations` | Source or supporting table for `stock-reservations` behavior. |
| `stock_transfers` | Source or supporting table for `stock-reservations` behavior. |
| `stocktakes` | Source or supporting table for `stock-reservations` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `inventory.stock-reservations.read` | View data and search records. | Yes |
| `inventory.stock-reservations.create` | Create new records or workflow drafts. | Yes |
| `inventory.stock-reservations.update` | Modify allowed editable fields. | Yes |
| `inventory.stock-reservations.approve` | Approve sensitive actions where applicable. | Yes |
| `inventory.stock-reservations.void` | Void/cancel/reverse where business rules allow. | Yes |
...
{
    private readonly IStockReservationsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<StockReservationsDto>> HandleAsync(StockReservationsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "inventory.stock-reservations.manage");
        ...
        return ApiResponse.Success(new StockReservationsDto());
    }
}
```
...
```ts
export const stockreservationsQueryKey = (tenantId: string, outletId?: string) =>
  ["inventory", "stock-reservations", tenantId, outletId ?? "tenant"] as const;

export function useStockReservations(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: stockreservationsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/inventory/stock-reservations"),
    staleTime: 60_000,
  });
}
```
