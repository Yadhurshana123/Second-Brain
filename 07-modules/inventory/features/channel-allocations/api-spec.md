---
title: "Channel Allocations API Specification"
...
# Channel Allocations API Specification

## API Ownership
The `channel-allocations` API belongs to the `inventory` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/inventory/channel-allocations` | `inventory.channel-allocations.read` |
| Detail | GET | `/api/v1/inventory/channel-allocations/{id}` | `inventory.channel-allocations.read` |
| Create | POST | `/api/v1/inventory/channel-allocations` | `inventory.channel-allocations.create` |
| Update | PUT | `/api/v1/inventory/channel-allocations/{id}` | `inventory.channel-allocations.update` |
| State action | POST | `/api/v1/inventory/channel-allocations/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/inventory/channel-allocations
...
{
  "featureKey": "inventory.channel-allocations",
...
- Access consideration 1: Permission `inventory.channel-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `inventory.channel-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `inventory.channel-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `inventory.channel-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `inventory.channel-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `inventory.channel-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `inventory.channel-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `inventory.channel-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `inventory.channel-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `inventory.channel-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `inventory.channel-allocations.read` may show data, while command permissions must be separately configured per role.
