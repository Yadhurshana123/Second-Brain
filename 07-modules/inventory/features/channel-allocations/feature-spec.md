---
title: "Channel Allocations Feature Specification"
...
# Channel Allocations Feature Specification

## Purpose
This document defines the implementation scope for `channel-allocations` inside the `inventory` module of the Unified Commerce platform.
...
|---|---|
| `inventory_balances` | Source or supporting table for `channel-allocations` behavior. |
| `inventory_channel_allocations` | Source or supporting table for `channel-allocations` behavior. |
| `stock_movements` | Source or supporting table for `channel-allocations` behavior. |
| `stock_reservations` | Source or supporting table for `channel-allocations` behavior. |
| `stock_transfers` | Source or supporting table for `channel-allocations` behavior. |
| `stocktakes` | Source or supporting table for `channel-allocations` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `inventory.channel-allocations.read` | View data and search records. | Yes |
| `inventory.channel-allocations.create` | Create new records or workflow drafts. | Yes |
| `inventory.channel-allocations.update` | Modify allowed editable fields. | Yes |
| `inventory.channel-allocations.approve` | Approve sensitive actions where applicable. | Yes |
| `inventory.channel-allocations.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class ChannelAllocationsService : IChannelAllocationsService
{
    private readonly IChannelAllocationsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<ChannelAllocationsDto>> HandleAsync(ChannelAllocationsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "inventory.channel-allocations.manage");
        ...
        return ApiResponse.Success(new ChannelAllocationsDto());
    }
}
```
...
```ts
export const channelallocationsQueryKey = (tenantId: string, outletId?: string) =>
  ["inventory", "channel-allocations", tenantId, outletId ?? "tenant"] as const;

export function useChannelAllocations(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: channelallocationsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/inventory/channel-allocations"),
    staleTime: 60_000,
  });
}
```
...
- Related module overview: [[../README|Inventory Overview]]
