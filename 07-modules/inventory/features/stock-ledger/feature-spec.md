---
title: "Stock Ledger Feature Specification"
...
# Stock Ledger Feature Specification

## Purpose
This document defines the implementation scope for `stock-ledger` inside the `inventory` module of the Unified Commerce platform.
...
|---|---|
| `inventory_balances` | Source or supporting table for `stock-ledger` behavior. |
| `inventory_channel_allocations` | Source or supporting table for `stock-ledger` behavior. |
| `stock_movements` | Source or supporting table for `stock-ledger` behavior. |
| `stock_reservations` | Source or supporting table for `stock-ledger` behavior. |
| `stock_transfers` | Source or supporting table for `stock-ledger` behavior. |
| `stocktakes` | Source or supporting table for `stock-ledger` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `inventory.stock-ledger.read` | View data and search records. | Yes |
| `inventory.stock-ledger.create` | Create new records or workflow drafts. | Yes |
| `inventory.stock-ledger.update` | Modify allowed editable fields. | Yes |
| `inventory.stock-ledger.approve` | Approve sensitive actions where applicable. | Yes |
| `inventory.stock-ledger.void` | Void/cancel/reverse where business rules allow. | Yes |
...
public sealed class StockLedgerService : IStockLedgerService
{
    ...
    public async Task<ApiResponse<StockLedgerDto>> HandleAsync(StockLedgerRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "inventory.stock-ledger.manage");
        ...
        return ApiResponse.Success(new StockLedgerDto());
    }
}
```
...
```ts
export const stockledgerQueryKey = (tenantId: string, outletId?: string) =>
  ["inventory", "stock-ledger", tenantId, outletId ?? "tenant"] as const;

export function useStockLedger(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: stockledgerQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/inventory/stock-ledger"),
    staleTime: 60_000,
  });
}
```
