---
title: "Stock Ledger API Specification"
...
# Stock Ledger API Specification

## API Ownership
The `stock-ledger` API belongs to the `inventory` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/inventory/stock-ledger` | `inventory.stock-ledger.read` |
| Detail | GET | `/api/v1/inventory/stock-ledger/{id}` | `inventory.stock-ledger.read` |
| Create | POST | `/api/v1/inventory/stock-ledger` | `inventory.stock-ledger.create` |
| Update | PUT | `/api/v1/inventory/stock-ledger/{id}` | `inventory.stock-ledger.update` |
| State action | POST | `/api/v1/inventory/stock-ledger/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/inventory/stock-ledger
...
{
  "featureKey": "inventory.stock-ledger",
...
- Access consideration 1: Permission `inventory.stock-ledger.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `inventory.stock-ledger.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `inventory.stock-ledger.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `inventory.stock-ledger.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `inventory.stock-ledger.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `inventory.stock-ledger.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `inventory.stock-ledger.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `inventory.stock-ledger.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `inventory.stock-ledger.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `inventory.stock-ledger.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `inventory.stock-ledger.read` may show data, while command permissions must be separately configured per role.
