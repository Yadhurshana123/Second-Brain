---
title: "Stock Transfers Feature Specification"
...
# Stock Transfers Feature Specification

## Purpose
This document defines the implementation scope for `stock-transfers` inside the `inventory` module of the Unified Commerce platform.
...
|---|---|
| `inventory_balances` | Source or supporting table for `stock-transfers` behavior. |
| `inventory_channel_allocations` | Source or supporting table for `stock-transfers` behavior. |
| `stock_movements` | Source or supporting table for `stock-transfers` behavior. |
| `stock_reservations` | Source or supporting table for `stock-transfers` behavior. |
| `stock_transfers` | Source or supporting table for `stock-transfers` behavior. |
| `stocktakes` | Source or supporting table for `stock-transfers` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `inventory.stock-transfers.read` | View data and search records. | Yes |
| `inventory.stock-transfers.create` | Create new records or workflow drafts. | Yes |
| `inventory.stock-transfers.update` | Modify allowed editable fields. | Yes |
| `inventory.stock-transfers.approve` | Approve sensitive actions where applicable. | Yes |
| `inventory.stock-transfers.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class StockTransfersService : IStockTransfersService
{
    private readonly IStockTransfersRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<StockTransfersDto>> HandleAsync(StockTransfersRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "inventory.stock-transfers.manage");
        ...
        return ApiResponse.Success(new StockTransfersDto());
    }
}
```
...
```ts
export const stocktransfersQueryKey = (tenantId: string, outletId?: string) =>
  ["inventory", "stock-transfers", tenantId, outletId ?? "tenant"] as const;

export function useStockTransfers(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: stocktransfersQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/inventory/stock-transfers"),
    staleTime: 60_000,
  });
}
```
