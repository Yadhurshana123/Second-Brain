---
title: "Purchase Receipts API Specification"
...
# Purchase Receipts API Specification

## API Ownership
The `purchase-receipts` API belongs to the `inventory` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/inventory/purchase-receipts` | `inventory.purchase-receipts.read` |
| Detail | GET | `/api/v1/inventory/purchase-receipts/{id}` | `inventory.purchase-receipts.read` |
| Create | POST | `/api/v1/inventory/purchase-receipts` | `inventory.purchase-receipts.create` |
| Update | PUT | `/api/v1/inventory/purchase-receipts/{id}` | `inventory.purchase-receipts.update` |
| State action | POST | `/api/v1/inventory/purchase-receipts/{id}/actions/{action}` | Action-specific permission |
...
{
  "featureKey": "inventory.purchase-receipts",
...
- Access consideration 1: Permission `inventory.purchase-receipts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `inventory.purchase-receipts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `inventory.purchase-receipts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `inventory.purchase-receipts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `inventory.purchase-receipts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `inventory.purchase-receipts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `inventory.purchase-receipts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `inventory.purchase-receipts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `inventory.purchase-receipts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `inventory.purchase-receipts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `inventory.purchase-receipts.read` may show data, while command permissions must be separately configured per role.
