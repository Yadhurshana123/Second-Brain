---
title: "Purchase Receipts Feature Specification"
...
# Purchase Receipts Feature Specification

## Purpose
This document defines the implementation scope for `purchase-receipts` inside the `inventory` module of the Unified Commerce platform.
...
|---|---|
| `inventory_balances` | Source or supporting table for `purchase-receipts` behavior. |
| `inventory_channel_allocations` | Source or supporting table for `purchase-receipts` behavior. |
| `stock_movements` | Source or supporting table for `purchase-receipts` behavior. |
| `stock_reservations` | Source or supporting table for `purchase-receipts` behavior. |
| `stock_transfers` | Source or supporting table for `purchase-receipts` behavior. |
| `stocktakes` | Source or supporting table for `purchase-receipts` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `inventory.purchase-receipts.read` | View data and search records. | Yes |
| `inventory.purchase-receipts.create` | Create new records or workflow drafts. | Yes |
| `inventory.purchase-receipts.update` | Modify allowed editable fields. | Yes |
| `inventory.purchase-receipts.approve` | Approve sensitive actions where applicable. | Yes |
| `inventory.purchase-receipts.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class PurchaseReceiptsService : IPurchaseReceiptsService
{
    private readonly IPurchaseReceiptsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<PurchaseReceiptsDto>> HandleAsync(PurchaseReceiptsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "inventory.purchase-receipts.manage");
        ...
        return ApiResponse.Success(new PurchaseReceiptsDto());
    }
}
```
...
```ts
export const purchasereceiptsQueryKey = (tenantId: string, outletId?: string) =>
  ["inventory", "purchase-receipts", tenantId, outletId ?? "tenant"] as const;

export function usePurchaseReceipts(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: purchasereceiptsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/inventory/purchase-receipts"),
    staleTime: 60_000,
  });
}
```
