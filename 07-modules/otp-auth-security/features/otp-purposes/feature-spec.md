---
title: "Otp Purposes Feature Specification"
...
# Otp Purposes Feature Specification

## Purpose
This document defines the implementation scope for `otp-purposes` inside the `otp-auth-security` module of the Unified Commerce platform.
...
|---|---|
| `otp_channels` | Source or supporting table for `otp-purposes` behavior. |
| `otp_purposes` | Source or supporting table for `otp-purposes` behavior. |
| `otp_verifications` | Source or supporting table for `otp-purposes` behavior. |
| `users` | Source or supporting table for `otp-purposes` behavior. |
| `customer_auth_accounts` | Source or supporting table for `otp-purposes` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `otp-auth-security.otp-purposes.read` | View data and search records. | Yes |
| `otp-auth-security.otp-purposes.create` | Create new records or workflow drafts. | Yes |
| `otp-auth-security.otp-purposes.update` | Modify allowed editable fields. | Yes |
| `otp-auth-security.otp-purposes.approve` | Approve sensitive actions where applicable. | Yes |
| `otp-auth-security.otp-purposes.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class OtpPurposesService : IOtpPurposesService
{
    private readonly IOtpPurposesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<OtpPurposesDto>> HandleAsync(OtpPurposesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "otp-auth-security.otp-purposes.manage");
        ...
        return ApiResponse.Success(new OtpPurposesDto());
    }
}
```
...
```ts
export const otppurposesQueryKey = (tenantId: string, outletId?: string) =>
  ["otp-auth-security", "otp-purposes", tenantId, outletId ?? "tenant"] as const;

export function useOtpPurposes(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: otppurposesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/otp-auth-security/otp-purposes"),
    staleTime: 60_000,
  });
}
```
