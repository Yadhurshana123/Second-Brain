---
title: "Otp Channels Feature Specification"
...
# Otp Channels Feature Specification

## Purpose
This document defines the implementation scope for `otp-channels` inside the `otp-auth-security` module of the Unified Commerce platform.
...
|---|---|
| `otp_channels` | Source or supporting table for `otp-channels` behavior. |
| `otp_purposes` | Source or supporting table for `otp-channels` behavior. |
| `otp_verifications` | Source or supporting table for `otp-channels` behavior. |
| `users` | Source or supporting table for `otp-channels` behavior. |
| `customer_auth_accounts` | Source or supporting table for `otp-channels` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `otp-auth-security.otp-channels.read` | View data and search records. | Yes |
| `otp-auth-security.otp-channels.create` | Create new records or workflow drafts. | Yes |
| `otp-auth-security.otp-channels.update` | Modify allowed editable fields. | Yes |
| `otp-auth-security.otp-channels.approve` | Approve sensitive actions where applicable. | Yes |
| `otp-auth-security.otp-channels.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class OtpChannelsService : IOtpChannelsService
{
    private readonly IOtpChannelsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<OtpChannelsDto>> HandleAsync(OtpChannelsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "otp-auth-security.otp-channels.manage");
        ...
        return ApiResponse.Success(new OtpChannelsDto());
    }
}
```
...
```ts
export const otpchannelsQueryKey = (tenantId: string, outletId?: string) =>
  ["otp-auth-security", "otp-channels", tenantId, outletId ?? "tenant"] as const;

export function useOtpChannels(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: otpchannelsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/otp-auth-security/otp-channels"),
    staleTime: 60_000,
  });
}
```
