---
title: "Otp Channels API Specification"
...
# Otp Channels API Specification

## API Ownership
The `otp-channels` API belongs to the `otp-auth-security` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/otp-auth-security/otp-channels` | `otp-auth-security.otp-channels.read` |
| Detail | GET | `/api/v1/otp-auth-security/otp-channels/{id}` | `otp-auth-security.otp-channels.read` |
| Create | POST | `/api/v1/otp-auth-security/otp-channels` | `otp-auth-security.otp-channels.create` |
| Update | PUT | `/api/v1/otp-auth-security/otp-channels/{id}` | `otp-auth-security.otp-channels.update` |
| State action | POST | `/api/v1/otp-auth-security/otp-channels/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/otp-auth-security/otp-channels
...
{
  "featureKey": "otp-auth-security.otp-channels",
...
- Access consideration 1: Permission `otp-auth-security.otp-channels.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `otp-auth-security.otp-channels.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `otp-auth-security.otp-channels.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `otp-auth-security.otp-channels.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `otp-auth-security.otp-channels.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `otp-auth-security.otp-channels.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `otp-auth-security.otp-channels.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `otp-auth-security.otp-channels.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `otp-auth-security.otp-channels.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `otp-auth-security.otp-channels.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `otp-auth-security.otp-channels.read` may show data, while command permissions must be separately configured per role.
