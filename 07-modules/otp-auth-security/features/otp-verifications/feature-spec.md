---
title: "Otp Verifications Feature Specification"
...
# Otp Verifications Feature Specification

## Purpose
This document defines the implementation scope for `otp-verifications` inside the `otp-auth-security` module of the Unified Commerce platform.
...
|---|---|
| `otp_channels` | Source or supporting table for `otp-verifications` behavior. |
| `otp_purposes` | Source or supporting table for `otp-verifications` behavior. |
| `otp_verifications` | Source or supporting table for `otp-verifications` behavior. |
| `users` | Source or supporting table for `otp-verifications` behavior. |
| `customer_auth_accounts` | Source or supporting table for `otp-verifications` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `otp-auth-security.otp-verifications.read` | View data and search records. | Yes |
| `otp-auth-security.otp-verifications.create` | Create new records or workflow drafts. | Yes |
| `otp-auth-security.otp-verifications.update` | Modify allowed editable fields. | Yes |
| `otp-auth-security.otp-verifications.approve` | Approve sensitive actions where applicable. | Yes |
| `otp-auth-security.otp-verifications.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class OtpVerificationsService : IOtpVerificationsService
{
    private readonly IOtpVerificationsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<OtpVerificationsDto>> HandleAsync(OtpVerificationsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "otp-auth-security.otp-verifications.manage");
        ...
        return ApiResponse.Success(new OtpVerificationsDto());
    }
}
```
...
```ts
export const otpverificationsQueryKey = (tenantId: string, outletId?: string) =>
  ["otp-auth-security", "otp-verifications", tenantId, outletId ?? "tenant"] as const;

export function useOtpVerifications(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: otpverificationsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/otp-auth-security/otp-verifications"),
    staleTime: 60_000,
  });
}
```
...
- Related module overview: [[../README|Otp Auth Security Overview]]
