---
title: "Otp Verifications API Specification"
...
# Otp Verifications API Specification

## API Ownership
The `otp-verifications` API belongs to the `otp-auth-security` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/otp-auth-security/otp-verifications` | `otp-auth-security.otp-verifications.read` |
| Detail | GET | `/api/v1/otp-auth-security/otp-verifications/{id}` | `otp-auth-security.otp-verifications.read` |
| Create | POST | `/api/v1/otp-auth-security/otp-verifications` | `otp-auth-security.otp-verifications.create` |
| Update | PUT | `/api/v1/otp-auth-security/otp-verifications/{id}` | `otp-auth-security.otp-verifications.update` |
| State action | POST | `/api/v1/otp-auth-security/otp-verifications/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/otp-auth-security/otp-verifications
...
{
  "featureKey": "otp-auth-security.otp-verifications",
...
- Access consideration 1: Permission `otp-auth-security.otp-verifications.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `otp-auth-security.otp-verifications.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `otp-auth-security.otp-verifications.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `otp-auth-security.otp-verifications.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `otp-auth-security.otp-verifications.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `otp-auth-security.otp-verifications.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `otp-auth-security.otp-verifications.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `otp-auth-security.otp-verifications.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `otp-auth-security.otp-verifications.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `otp-auth-security.otp-verifications.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `otp-auth-security.otp-verifications.read` may show data, while command permissions must be separately configured per role.
