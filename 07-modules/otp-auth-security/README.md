---
title: "Otp Auth Security Module Overview"
...
# Otp Auth Security Module Overview

## Module Purpose
The `otp-auth-security` module groups related Unified Commerce capabilities and documents how they are implemented across database, backend, API, frontend, security, and cache/storage layers.
...
|---|---|---|---|
| `otp-channels` | [[features/otp-channels/feature-spec|Spec]] | [[features/otp-channels/api-spec|API]] | [[features/otp-channels/feature-history|History]] |
| `otp-purposes` | [[features/otp-purposes/feature-spec|Spec]] | [[features/otp-purposes/api-spec|API]] | [[features/otp-purposes/feature-history|History]] |
| `otp-verifications` | [[features/otp-verifications/feature-spec|Spec]] | [[features/otp-verifications/api-spec|API]] | [[features/otp-verifications/feature-history|History]] |

## Database Tables Commonly Used
| Table | Purpose in module |
|---|---|
| `otp_channels` | Supports `otp-auth-security` module workflows and tenant-scoped persistence. |
| `otp_purposes` | Supports `otp-auth-security` module workflows and tenant-scoped persistence. |
| `otp_verifications` | Supports `otp-auth-security` module workflows and tenant-scoped persistence. |
| `users` | Supports `otp-auth-security` module workflows and tenant-scoped persistence. |
| `customer_auth_accounts` | Supports `otp-auth-security` module workflows and tenant-scoped persistence. |
...
|---|---|---|
| API | `POS.API/Modules/OtpAuthSecurity` | Controllers, request models, response models. |
| Application | `POS.Application/Modules/OtpAuthSecurity` | Services, validators, interfaces, and `Dtos/`. |
| Domain | `POS.Domain/Modules/OtpAuthSecurity` | Pure rules and entities where needed. |
| Infrastructure | `POS.Infrastructure/Repositories/OtpAuthSecurity` | EF Core repositories and persistence logic. |
...
- Access consideration 33: Permission `otp-auth-security.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 34: Permission `otp-auth-security.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 35: Permission `otp-auth-security.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 36: Permission `otp-auth-security.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 37: Permission `otp-auth-security.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 38: Permission `otp-auth-security.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 39: Permission `otp-auth-security.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 40: Permission `otp-auth-security.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 41: Permission `otp-auth-security.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 42: Permission `otp-auth-security.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 43: Permission `otp-auth-security.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 44: Permission `otp-auth-security.module.read` may show data, while command permissions must be separately configured per role.
