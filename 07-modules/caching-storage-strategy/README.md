---
title: "Caching Storage Strategy Module Overview"
...
# Caching Storage Strategy Module Overview

## Module Purpose
The `caching-storage-strategy` module groups related Unified Commerce capabilities and documents how they are implemented across database, backend, API, frontend, security, and cache/storage layers.
...
|---|---|---|---|
| `backend-postgresql-cache-strategy` | [[features/backend-postgresql-cache-strategy/feature-spec|Spec]] | [[features/backend-postgresql-cache-strategy/api-spec|API]] | [[features/backend-postgresql-cache-strategy/feature-history|History]] |
| `cache-invalidation-and-consistency` | [[features/cache-invalidation-and-consistency/feature-spec|Spec]] | [[features/cache-invalidation-and-consistency/api-spec|API]] | [[features/cache-invalidation-and-consistency/feature-history|History]] |
| `frontend-tanstack-query-cache` | [[features/frontend-tanstack-query-cache/feature-spec|Spec]] | [[features/frontend-tanstack-query-cache/api-spec|API]] | [[features/frontend-tanstack-query-cache/feature-history|History]] |
| `indexeddb-offline-pos-storage` | [[features/indexeddb-offline-pos-storage/feature-spec|Spec]] | [[features/indexeddb-offline-pos-storage/api-spec|API]] | [[features/indexeddb-offline-pos-storage/feature-history|History]] |

## Database Tables Commonly Used
| Table | Purpose in module |
|---|---|
| `daily_sales_summaries` | Supports `caching-storage-strategy` module workflows and tenant-scoped persistence. |
| `daily_payment_summaries` | Supports `caching-storage-strategy` module workflows and tenant-scoped persistence. |
| `daily_inventory_summaries` | Supports `caching-storage-strategy` module workflows and tenant-scoped persistence. |
| `feature_flags` | Supports `caching-storage-strategy` module workflows and tenant-scoped persistence. |
| `tenant_settings` | Supports `caching-storage-strategy` module workflows and tenant-scoped persistence. |
| `offline_sync_items` | Supports `caching-storage-strategy` module workflows and tenant-scoped persistence. |
...
|---|---|---|
| API | `POS.API/Modules/CachingStorageStrategy` | Controllers, request models, response models. |
| Application | `POS.Application/Modules/CachingStorageStrategy` | Services, validators, interfaces, and `Dtos/`. |
| Domain | `POS.Domain/Modules/CachingStorageStrategy` | Pure rules and entities where needed. |
| Infrastructure | `POS.Infrastructure/Repositories/CachingStorageStrategy` | EF Core repositories and persistence logic. |
...
- Access consideration 31: Permission `caching-storage-strategy.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 32: Permission `caching-storage-strategy.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 33: Permission `caching-storage-strategy.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 34: Permission `caching-storage-strategy.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 35: Permission `caching-storage-strategy.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 36: Permission `caching-storage-strategy.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 37: Permission `caching-storage-strategy.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 38: Permission `caching-storage-strategy.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 39: Permission `caching-storage-strategy.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 40: Permission `caching-storage-strategy.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 41: Permission `caching-storage-strategy.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 42: Permission `caching-storage-strategy.module.read` may show data, while command permissions must be separately configured per role.
