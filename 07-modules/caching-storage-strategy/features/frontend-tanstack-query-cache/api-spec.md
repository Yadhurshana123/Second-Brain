---
title: "Frontend Tanstack Query Cache API Specification"
...
# Frontend Tanstack Query Cache API Specification

## API Ownership
The `frontend-tanstack-query-cache` API belongs to the `caching-storage-strategy` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/caching-storage-strategy/frontend-tanstack-query-cache` | `caching-storage-strategy.frontend-tanstack-query-cache.read` |
| Detail | GET | `/api/v1/caching-storage-strategy/frontend-tanstack-query-cache/{id}` | `caching-storage-strategy.frontend-tanstack-query-cache.read` |
| Create | POST | `/api/v1/caching-storage-strategy/frontend-tanstack-query-cache` | `caching-storage-strategy.frontend-tanstack-query-cache.create` |
| Update | PUT | `/api/v1/caching-storage-strategy/frontend-tanstack-query-cache/{id}` | `caching-storage-strategy.frontend-tanstack-query-cache.update` |
| State action | POST | `/api/v1/caching-storage-strategy/frontend-tanstack-query-cache/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/caching-storage-strategy/frontend-tanstack-query-cache
...
{
  "featureKey": "caching-storage-strategy.frontend-tanstack-query-cache",
...
- Access consideration 1: Permission `caching-storage-strategy.frontend-tanstack-query-cache.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `caching-storage-strategy.frontend-tanstack-query-cache.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `caching-storage-strategy.frontend-tanstack-query-cache.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `caching-storage-strategy.frontend-tanstack-query-cache.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `caching-storage-strategy.frontend-tanstack-query-cache.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `caching-storage-strategy.frontend-tanstack-query-cache.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `caching-storage-strategy.frontend-tanstack-query-cache.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `caching-storage-strategy.frontend-tanstack-query-cache.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `caching-storage-strategy.frontend-tanstack-query-cache.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `caching-storage-strategy.frontend-tanstack-query-cache.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `caching-storage-strategy.frontend-tanstack-query-cache.read` may show data, while command permissions must be separately configured per role.
