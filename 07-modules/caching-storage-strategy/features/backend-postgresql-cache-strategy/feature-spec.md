---
title: "Backend Postgresql Cache Strategy Feature Specification"
...
# Backend Postgresql Cache Strategy Feature Specification

## Purpose
This document defines the implementation scope for `backend-postgresql-cache-strategy` inside the `caching-storage-strategy` module of the Unified Commerce platform.
...
|---|---|
| `daily_sales_summaries` | Source or supporting table for `backend-postgresql-cache-strategy` behavior. |
| `daily_payment_summaries` | Source or supporting table for `backend-postgresql-cache-strategy` behavior. |
| `daily_inventory_summaries` | Source or supporting table for `backend-postgresql-cache-strategy` behavior. |
| `feature_flags` | Source or supporting table for `backend-postgresql-cache-strategy` behavior. |
| `tenant_settings` | Source or supporting table for `backend-postgresql-cache-strategy` behavior. |
| `offline_sync_items` | Source or supporting table for `backend-postgresql-cache-strategy` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `caching-storage-strategy.backend-postgresql-cache-strategy.read` | View data and search records. | Yes |
| `caching-storage-strategy.backend-postgresql-cache-strategy.create` | Create new records or workflow drafts. | Yes |
| `caching-storage-strategy.backend-postgresql-cache-strategy.update` | Modify allowed editable fields. | Yes |
| `caching-storage-strategy.backend-postgresql-cache-strategy.approve` | Approve sensitive actions where applicable. | Yes |
| `caching-storage-strategy.backend-postgresql-cache-strategy.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class BackendPostgresqlCacheStrategyService : IBackendPostgresqlCacheStrategyService
{
    private readonly IBackendPostgresqlCacheStrategyRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<BackendPostgresqlCacheStrategyDto>> HandleAsync(BackendPostgresqlCacheStrategyRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "caching-storage-strategy.backend-postgresql-cache-strategy.manage");
        ...
        return ApiResponse.Success(new BackendPostgresqlCacheStrategyDto());
    }
}
```
...
```ts
export const backendpostgresqlcachestrategyQueryKey = (tenantId: string, outletId?: string) =>
  ["caching-storage-strategy", "backend-postgresql-cache-strategy", tenantId, outletId ?? "tenant"] as const;

export function useBackendPostgresqlCacheStrategy(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: backendpostgresqlcachestrategyQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/caching-storage-strategy/backend-postgresql-cache-strategy"),
    staleTime: 60_000,
  });
}
```
