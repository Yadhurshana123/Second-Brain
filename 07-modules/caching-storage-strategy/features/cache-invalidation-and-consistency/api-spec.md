---
title: "Cache Invalidation And Consistency API Specification"
...
# Cache Invalidation And Consistency API Specification

## API Ownership
The `cache-invalidation-and-consistency` API belongs to the `caching-storage-strategy` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/caching-storage-strategy/cache-invalidation-and-consistency` | `caching-storage-strategy.cache-invalidation-and-consistency.read` |
| Detail | GET | `/api/v1/caching-storage-strategy/cache-invalidation-and-consistency/{id}` | `caching-storage-strategy.cache-invalidation-and-consistency.read` |
| Create | POST | `/api/v1/caching-storage-strategy/cache-invalidation-and-consistency` | `caching-storage-strategy.cache-invalidation-and-consistency.create` |
| Update | PUT | `/api/v1/caching-storage-strategy/cache-invalidation-and-consistency/{id}` | `caching-storage-strategy.cache-invalidation-and-consistency.update` |
| State action | POST | `/api/v1/caching-storage-strategy/cache-invalidation-and-consistency/{id}/actions/{action}` | Action-specific permission |
...
- Access consideration 1: Permission `caching-storage-strategy.cache-invalidation-and-consistency.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `caching-storage-strategy.cache-invalidation-and-consistency.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `caching-storage-strategy.cache-invalidation-and-consistency.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `caching-storage-strategy.cache-invalidation-and-consistency.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `caching-storage-strategy.cache-invalidation-and-consistency.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `caching-storage-strategy.cache-invalidation-and-consistency.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `caching-storage-strategy.cache-invalidation-and-consistency.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `caching-storage-strategy.cache-invalidation-and-consistency.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `caching-storage-strategy.cache-invalidation-and-consistency.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `caching-storage-strategy.cache-invalidation-and-consistency.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `caching-storage-strategy.cache-invalidation-and-consistency.read` may show data, while command permissions must be separately configured per role.
