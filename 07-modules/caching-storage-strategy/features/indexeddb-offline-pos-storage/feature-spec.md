---
title: "Indexeddb Offline Pos Storage Feature Specification"
...
# Indexeddb Offline Pos Storage Feature Specification

## Purpose
This document defines the implementation scope for `indexeddb-offline-pos-storage` inside the `caching-storage-strategy` module of the Unified Commerce platform.
...
|---|---|
| `daily_sales_summaries` | Source or supporting table for `indexeddb-offline-pos-storage` behavior. |
| `daily_payment_summaries` | Source or supporting table for `indexeddb-offline-pos-storage` behavior. |
| `daily_inventory_summaries` | Source or supporting table for `indexeddb-offline-pos-storage` behavior. |
| `feature_flags` | Source or supporting table for `indexeddb-offline-pos-storage` behavior. |
| `tenant_settings` | Source or supporting table for `indexeddb-offline-pos-storage` behavior. |
| `offline_sync_items` | Source or supporting table for `indexeddb-offline-pos-storage` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `caching-storage-strategy.indexeddb-offline-pos-storage.read` | View data and search records. | Yes |
| `caching-storage-strategy.indexeddb-offline-pos-storage.create` | Create new records or workflow drafts. | Yes |
| `caching-storage-strategy.indexeddb-offline-pos-storage.update` | Modify allowed editable fields. | Yes |
| `caching-storage-strategy.indexeddb-offline-pos-storage.approve` | Approve sensitive actions where applicable. | Yes |
| `caching-storage-strategy.indexeddb-offline-pos-storage.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class IndexeddbOfflinePosStorageService : IIndexeddbOfflinePosStorageService
{
    private readonly IIndexeddbOfflinePosStorageRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<IndexeddbOfflinePosStorageDto>> HandleAsync(IndexeddbOfflinePosStorageRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "caching-storage-strategy.indexeddb-offline-pos-storage.manage");
        ...
        return ApiResponse.Success(new IndexeddbOfflinePosStorageDto());
    }
}
```
...
```ts
export const indexeddbofflineposstorageQueryKey = (tenantId: string, outletId?: string) =>
  ["caching-storage-strategy", "indexeddb-offline-pos-storage", tenantId, outletId ?? "tenant"] as const;

export function useIndexeddbOfflinePosStorage(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: indexeddbofflineposstorageQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/caching-storage-strategy/indexeddb-offline-pos-storage"),
    staleTime: 60_000,
  });
}
```
