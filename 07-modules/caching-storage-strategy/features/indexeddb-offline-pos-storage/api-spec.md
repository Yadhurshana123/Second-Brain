---
title: "Indexeddb Offline Pos Storage API Specification"
...
# Indexeddb Offline Pos Storage API Specification

## API Ownership
The `indexeddb-offline-pos-storage` API belongs to the `caching-storage-strategy` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/caching-storage-strategy/indexeddb-offline-pos-storage` | `caching-storage-strategy.indexeddb-offline-pos-storage.read` |
| Detail | GET | `/api/v1/caching-storage-strategy/indexeddb-offline-pos-storage/{id}` | `caching-storage-strategy.indexeddb-offline-pos-storage.read` |
| Create | POST | `/api/v1/caching-storage-strategy/indexeddb-offline-pos-storage` | `caching-storage-strategy.indexeddb-offline-pos-storage.create` |
| Update | PUT | `/api/v1/caching-storage-strategy/indexeddb-offline-pos-storage/{id}` | `caching-storage-strategy.indexeddb-offline-pos-storage.update` |
| State action | POST | `/api/v1/caching-storage-strategy/indexeddb-offline-pos-storage/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/caching-storage-strategy/indexeddb-offline-pos-storage
...
{
  "featureKey": "caching-storage-strategy.indexeddb-offline-pos-storage",
...
- Access consideration 1: Permission `caching-storage-strategy.indexeddb-offline-pos-storage.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `caching-storage-strategy.indexeddb-offline-pos-storage.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `caching-storage-strategy.indexeddb-offline-pos-storage.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `caching-storage-strategy.indexeddb-offline-pos-storage.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `caching-storage-strategy.indexeddb-offline-pos-storage.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `caching-storage-strategy.indexeddb-offline-pos-storage.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `caching-storage-strategy.indexeddb-offline-pos-storage.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `caching-storage-strategy.indexeddb-offline-pos-storage.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `caching-storage-strategy.indexeddb-offline-pos-storage.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `caching-storage-strategy.indexeddb-offline-pos-storage.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `caching-storage-strategy.indexeddb-offline-pos-storage.read` may show data, while command permissions must be separately configured per role.
