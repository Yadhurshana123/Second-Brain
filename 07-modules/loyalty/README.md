...
## Module Purpose
The `loyalty` module groups related Unified Commerce capabilities and documents how they are implemented across database, backend, API, frontend, security, and cache/storage layers.
...
|---|---|---|---|
| `customer-memberships` | [[features/customer-memberships/feature-spec|Spec]] | [[features/customer-memberships/api-spec|API]] | [[features/customer-memberships/feature-history|History]] |
| `loyalty-programs` | [[features/loyalty-programs/feature-spec|Spec]] | [[features/loyalty-programs/api-spec|API]] | [[features/loyalty-programs/feature-history|History]] |
| `loyalty-transactions` | [[features/loyalty-transactions/feature-spec|Spec]] | [[features/loyalty-transactions/api-spec|API]] | [[features/loyalty-transactions/feature-history|History]] |
| `membership-tiers` | [[features/membership-tiers/feature-spec|Spec]] | [[features/membership-tiers/api-spec|API]] | [[features/membership-tiers/feature-history|History]] |

## Database Tables Commonly Used
| Table | Purpose in module |
|---|---|
| `loyalty_programs` | Supports `loyalty` module workflows and tenant-scoped persistence. |
| `membership_tiers` | Supports `loyalty` module workflows and tenant-scoped persistence. |
| `customer_memberships` | Supports `loyalty` module workflows and tenant-scoped persistence. |
| `loyalty_transactions` | Supports `loyalty` module workflows and tenant-scoped persistence. |
| `customers` | Supports `loyalty` module workflows and tenant-scoped persistence. |
...
|---|---|---|
| API | `POS.API/Modules/Loyalty` | Controllers, request models, response models. |
| Application | `POS.Application/Modules/Loyalty` | Services, validators, interfaces, and `Dtos/`. |
| Domain | `POS.Domain/Modules/Loyalty` | Pure rules and entities where needed. |
| Infrastructure | `POS.Infrastructure/Repositories/Loyalty` | EF Core repositories and persistence logic. |
...
- Access consideration 32: Permission `loyalty.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 33: Permission `loyalty.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 34: Permission `loyalty.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 35: Permission `loyalty.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 36: Permission `loyalty.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 37: Permission `loyalty.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 38: Permission `loyalty.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 39: Permission `loyalty.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 40: Permission `loyalty.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 41: Permission `loyalty.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 42: Permission `loyalty.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 43: Permission `loyalty.module.read` may show data, while command permissions must be separately configured per role.
