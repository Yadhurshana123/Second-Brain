---
title: "Loyalty Programs API Specification"
...
# Loyalty Programs API Specification

## API Ownership
The `loyalty-programs` API belongs to the `loyalty` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/loyalty/loyalty-programs` | `loyalty.loyalty-programs.read` |
| Detail | GET | `/api/v1/loyalty/loyalty-programs/{id}` | `loyalty.loyalty-programs.read` |
| Create | POST | `/api/v1/loyalty/loyalty-programs` | `loyalty.loyalty-programs.create` |
| Update | PUT | `/api/v1/loyalty/loyalty-programs/{id}` | `loyalty.loyalty-programs.update` |
| State action | POST | `/api/v1/loyalty/loyalty-programs/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/loyalty/loyalty-programs
...
{
  "featureKey": "loyalty.loyalty-programs",
...
- Access consideration 1: Permission `loyalty.loyalty-programs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `loyalty.loyalty-programs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `loyalty.loyalty-programs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `loyalty.loyalty-programs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `loyalty.loyalty-programs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `loyalty.loyalty-programs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `loyalty.loyalty-programs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `loyalty.loyalty-programs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `loyalty.loyalty-programs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `loyalty.loyalty-programs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `loyalty.loyalty-programs.read` may show data, while command permissions must be separately configured per role.
