---
title: "Loyalty Programs Feature Specification"
...
# Loyalty Programs Feature Specification

## Purpose
This document defines the implementation scope for `loyalty-programs` inside the `loyalty` module of the Unified Commerce platform.
...
|---|---|
| `loyalty_programs` | Source or supporting table for `loyalty-programs` behavior. |
| `membership_tiers` | Source or supporting table for `loyalty-programs` behavior. |
| `customer_memberships` | Source or supporting table for `loyalty-programs` behavior. |
| `loyalty_transactions` | Source or supporting table for `loyalty-programs` behavior. |
| `customers` | Source or supporting table for `loyalty-programs` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `loyalty.loyalty-programs.read` | View data and search records. | Yes |
| `loyalty.loyalty-programs.create` | Create new records or workflow drafts. | Yes |
| `loyalty.loyalty-programs.update` | Modify allowed editable fields. | Yes |
| `loyalty.loyalty-programs.approve` | Approve sensitive actions where applicable. | Yes |
| `loyalty.loyalty-programs.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class LoyaltyProgramsService : ILoyaltyProgramsService
{
    private readonly ILoyaltyProgramsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<LoyaltyProgramsDto>> HandleAsync(LoyaltyProgramsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "loyalty.loyalty-programs.manage");
        ...
        return ApiResponse.Success(new LoyaltyProgramsDto());
    }
}
```
...
```ts
export const loyaltyprogramsQueryKey = (tenantId: string, outletId?: string) =>
  ["loyalty", "loyalty-programs", tenantId, outletId ?? "tenant"] as const;

export function useLoyaltyPrograms(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: loyaltyprogramsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/loyalty/loyalty-programs"),
    staleTime: 60_000,
  });
}
```
