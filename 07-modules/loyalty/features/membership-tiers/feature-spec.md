---
title: "Membership Tiers Feature Specification"
...
# Membership Tiers Feature Specification

## Purpose
This document defines the implementation scope for `membership-tiers` inside the `loyalty` module of the Unified Commerce platform.
...
|---|---|
| `loyalty_programs` | Source or supporting table for `membership-tiers` behavior. |
| `membership_tiers` | Source or supporting table for `membership-tiers` behavior. |
| `customer_memberships` | Source or supporting table for `membership-tiers` behavior. |
| `loyalty_transactions` | Source or supporting table for `membership-tiers` behavior. |
| `customers` | Source or supporting table for `membership-tiers` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `loyalty.membership-tiers.read` | View data and search records. | Yes |
| `loyalty.membership-tiers.create` | Create new records or workflow drafts. | Yes |
| `loyalty.membership-tiers.update` | Modify allowed editable fields. | Yes |
| `loyalty.membership-tiers.approve` | Approve sensitive actions where applicable. | Yes |
| `loyalty.membership-tiers.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class MembershipTiersService : IMembershipTiersService
{
    private readonly IMembershipTiersRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<MembershipTiersDto>> HandleAsync(MembershipTiersRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "loyalty.membership-tiers.manage");
        ...
        return ApiResponse.Success(new MembershipTiersDto());
    }
}
```
...
```ts
export const membershiptiersQueryKey = (tenantId: string, outletId?: string) =>
  ["loyalty", "membership-tiers", tenantId, outletId ?? "tenant"] as const;

export function useMembershipTiers(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: membershiptiersQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/loyalty/membership-tiers"),
    staleTime: 60_000,
  });
}
```
