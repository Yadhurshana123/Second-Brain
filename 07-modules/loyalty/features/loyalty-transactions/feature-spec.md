---
title: "Loyalty Transactions Feature Specification"
...
# Loyalty Transactions Feature Specification

## Purpose
This document defines the implementation scope for `loyalty-transactions` inside the `loyalty` module of the Unified Commerce platform.
...
|---|---|
| `loyalty_programs` | Source or supporting table for `loyalty-transactions` behavior. |
| `membership_tiers` | Source or supporting table for `loyalty-transactions` behavior. |
| `customer_memberships` | Source or supporting table for `loyalty-transactions` behavior. |
| `loyalty_transactions` | Source or supporting table for `loyalty-transactions` behavior. |
| `customers` | Source or supporting table for `loyalty-transactions` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `loyalty.loyalty-transactions.read` | View data and search records. | Yes |
| `loyalty.loyalty-transactions.create` | Create new records or workflow drafts. | Yes |
| `loyalty.loyalty-transactions.update` | Modify allowed editable fields. | Yes |
| `loyalty.loyalty-transactions.approve` | Approve sensitive actions where applicable. | Yes |
| `loyalty.loyalty-transactions.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class LoyaltyTransactionsService : ILoyaltyTransactionsService
{
    private readonly ILoyaltyTransactionsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<LoyaltyTransactionsDto>> HandleAsync(LoyaltyTransactionsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "loyalty.loyalty-transactions.manage");
        ...
        return ApiResponse.Success(new LoyaltyTransactionsDto());
    }
}
```
...
```ts
export const loyaltytransactionsQueryKey = (tenantId: string, outletId?: string) =>
  ["loyalty", "loyalty-transactions", tenantId, outletId ?? "tenant"] as const;

export function useLoyaltyTransactions(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: loyaltytransactionsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/loyalty/loyalty-transactions"),
    staleTime: 60_000,
  });
}
```
