---
title: "Sensitive Action Audit Feature History"
folder: "07-modules/audit-compliance"
document_type: "feature-history"
module: "audit-compliance"
feature: "sensitive-action-audit"
...
# Sensitive Action Audit Feature History
...
## Related Documents
- Feature spec: [[feature-spec|Sensitive Action Audit Feature Spec]]
- API spec: [[api-spec|Sensitive Action Audit API Spec]]
...
- Implementation consideration 18: Keep `sensitive-action-audit` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 19: Keep `sensitive-action-audit` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 20: Keep `sensitive-action-audit` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 21: Keep `sensitive-action-audit` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 22: Keep `sensitive-action-audit` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 23: Keep `sensitive-action-audit` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 24: Keep `sensitive-action-audit` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 25: Keep `sensitive-action-audit` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 26: Keep `sensitive-action-audit` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 27: Keep `sensitive-action-audit` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 28: Keep `sensitive-action-audit` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 29: Keep `sensitive-action-audit` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Access consideration 30: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
- Access consideration 31: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
- Access consideration 32: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
- Access consideration 33: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
- Access consideration 34: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
- Access consideration 35: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
- Access consideration 36: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
- Access consideration 37: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
- Access consideration 38: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
- Access consideration 39: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
- Access consideration 40: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
- Access consideration 41: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
