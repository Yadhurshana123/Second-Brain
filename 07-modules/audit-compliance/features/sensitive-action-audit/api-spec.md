---
title: "Sensitive Action Audit API Specification"
...
# Sensitive Action Audit API Specification

## API Ownership
The `sensitive-action-audit` API belongs to the `audit-compliance` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/audit-compliance/sensitive-action-audit` | `audit-compliance.sensitive-action-audit.read` |
| Detail | GET | `/api/v1/audit-compliance/sensitive-action-audit/{id}` | `audit-compliance.sensitive-action-audit.read` |
| Create | POST | `/api/v1/audit-compliance/sensitive-action-audit` | `audit-compliance.sensitive-action-audit.create` |
| Update | PUT | `/api/v1/audit-compliance/sensitive-action-audit/{id}` | `audit-compliance.sensitive-action-audit.update` |
| State action | POST | `/api/v1/audit-compliance/sensitive-action-audit/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/audit-compliance/sensitive-action-audit
...
{
  "featureKey": "audit-compliance.sensitive-action-audit",
...
- Access consideration 1: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `audit-compliance.sensitive-action-audit.read` may show data, while command permissions must be separately configured per role.
