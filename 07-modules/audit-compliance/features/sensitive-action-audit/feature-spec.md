---
title: "Sensitive Action Audit Feature Specification"
...
# Sensitive Action Audit Feature Specification

## Purpose
This document defines the implementation scope for `sensitive-action-audit` inside the `audit-compliance` module of the Unified Commerce platform.
...
|---|---|
| `audit_logs` | Source or supporting table for `sensitive-action-audit` behavior. |
| `offline_sync_audit_logs` | Source or supporting table for `sensitive-action-audit` behavior. |
| `receipt_print_logs` | Source or supporting table for `sensitive-action-audit` behavior. |
| `discount_requests` | Source or supporting table for `sensitive-action-audit` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `audit-compliance.sensitive-action-audit.read` | View data and search records. | Yes |
| `audit-compliance.sensitive-action-audit.create` | Create new records or workflow drafts. | Yes |
| `audit-compliance.sensitive-action-audit.update` | Modify allowed editable fields. | Yes |
| `audit-compliance.sensitive-action-audit.approve` | Approve sensitive actions where applicable. | Yes |
| `audit-compliance.sensitive-action-audit.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class SensitiveActionAuditService : ISensitiveActionAuditService
{
    private readonly ISensitiveActionAuditRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<SensitiveActionAuditDto>> HandleAsync(SensitiveActionAuditRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "audit-compliance.sensitive-action-audit.manage");
        ...
        return ApiResponse.Success(new SensitiveActionAuditDto());
    }
}
```
...
```ts
export const sensitiveactionauditQueryKey = (tenantId: string, outletId?: string) =>
  ["audit-compliance", "sensitive-action-audit", tenantId, outletId ?? "tenant"] as const;

export function useSensitiveActionAudit(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: sensitiveactionauditQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/audit-compliance/sensitive-action-audit"),
    staleTime: 60_000,
  });
}
```
...
- Related module overview: [[../README|Audit Compliance Overview]]
