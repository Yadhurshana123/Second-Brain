---
title: "Audit Logs API Specification"
...
## API Ownership
The `audit-logs` API belongs to the `audit-compliance` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/audit-compliance/audit-logs` | `audit-compliance.audit-logs.read` |
| Detail | GET | `/api/v1/audit-compliance/audit-logs/{id}` | `audit-compliance.audit-logs.read` |
| Create | POST | `/api/v1/audit-compliance/audit-logs` | `audit-compliance.audit-logs.create` |
| Update | PUT | `/api/v1/audit-compliance/audit-logs/{id}` | `audit-compliance.audit-logs.update` |
| State action | POST | `/api/v1/audit-compliance/audit-logs/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/audit-compliance/audit-logs
...
{
  "featureKey": "audit-compliance.audit-logs",
...
- Access consideration 1: Permission `audit-compliance.audit-logs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `audit-compliance.audit-logs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `audit-compliance.audit-logs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `audit-compliance.audit-logs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `audit-compliance.audit-logs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `audit-compliance.audit-logs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `audit-compliance.audit-logs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `audit-compliance.audit-logs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `audit-compliance.audit-logs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `audit-compliance.audit-logs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `audit-compliance.audit-logs.read` may show data, while command permissions must be separately configured per role.
