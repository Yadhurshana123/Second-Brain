---
title: "Audit Logs Feature Specification"
...
# Audit Logs Feature Specification

## Purpose
This document defines the implementation scope for `audit-logs` inside the `audit-compliance` module of the Unified Commerce platform.
...
|---|---|
| `audit_logs` | Source or supporting table for `audit-logs` behavior. |
| `offline_sync_audit_logs` | Source or supporting table for `audit-logs` behavior. |
| `receipt_print_logs` | Source or supporting table for `audit-logs` behavior. |
| `discount_requests` | Source or supporting table for `audit-logs` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `audit-compliance.audit-logs.read` | View data and search records. | Yes |
| `audit-compliance.audit-logs.create` | Create new records or workflow drafts. | Yes |
| `audit-compliance.audit-logs.update` | Modify allowed editable fields. | Yes |
| `audit-compliance.audit-logs.approve` | Approve sensitive actions where applicable. | Yes |
| `audit-compliance.audit-logs.void` | Void/cancel/reverse where business rules allow. | Yes |
...
public sealed class AuditLogsService : IAuditLogsService
{
    ...
    public async Task<ApiResponse<AuditLogsDto>> HandleAsync(AuditLogsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "audit-compliance.audit-logs.manage");
        ...
        return ApiResponse.Success(new AuditLogsDto());
    }
}
```
...
```ts
export const auditlogsQueryKey = (tenantId: string, outletId?: string) =>
  ["audit-compliance", "audit-logs", tenantId, outletId ?? "tenant"] as const;

export function useAuditLogs(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: auditlogsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/audit-compliance/audit-logs"),
    staleTime: 60_000,
  });
}
```
