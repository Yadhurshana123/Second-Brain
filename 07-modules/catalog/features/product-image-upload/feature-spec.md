---
title: "Product Image Upload Feature Specification"
...
# Product Image Upload Feature Specification

## Purpose
This document defines the implementation scope for `product-image-upload` inside the `catalog` module of the Unified Commerce platform.
...
|---|---|
| `brands` | Source or supporting table for `product-image-upload` behavior. |
| `suppliers` | Source or supporting table for `product-image-upload` behavior. |
| `categories` | Source or supporting table for `product-image-upload` behavior. |
| `products` | Source or supporting table for `product-image-upload` behavior. |
| `product_variants` | Source or supporting table for `product-image-upload` behavior. |
| `product_attributes` | Source or supporting table for `product-image-upload` behavior. |
| `attribute_values` | Source or supporting table for `product-image-upload` behavior. |
| `product_images` | Source or supporting table for `product-image-upload` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `catalog.product-image-upload.read` | View data and search records. | Yes |
| `catalog.product-image-upload.create` | Create new records or workflow drafts. | Yes |
| `catalog.product-image-upload.update` | Modify allowed editable fields. | Yes |
| `catalog.product-image-upload.approve` | Approve sensitive actions where applicable. | Yes |
| `catalog.product-image-upload.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class ProductImageUploadService : IProductImageUploadService
{
    private readonly IProductImageUploadRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<ProductImageUploadDto>> HandleAsync(ProductImageUploadRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "catalog.product-image-upload.manage");
        ...
        return ApiResponse.Success(new ProductImageUploadDto());
    }
}
```
...
```ts
export const productimageuploadQueryKey = (tenantId: string, outletId?: string) =>
  ["catalog", "product-image-upload", tenantId, outletId ?? "tenant"] as const;

export function useProductImageUpload(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: productimageuploadQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/catalog/product-image-upload"),
    staleTime: 60_000,
  });
}
```
