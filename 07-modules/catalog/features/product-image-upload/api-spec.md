---
title: "Product Image Upload API Specification"
...
# Product Image Upload API Specification

## API Ownership
The `product-image-upload` API belongs to the `catalog` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/catalog/product-image-upload` | `catalog.product-image-upload.read` |
| Detail | GET | `/api/v1/catalog/product-image-upload/{id}` | `catalog.product-image-upload.read` |
| Create | POST | `/api/v1/catalog/product-image-upload` | `catalog.product-image-upload.create` |
| Update | PUT | `/api/v1/catalog/product-image-upload/{id}` | `catalog.product-image-upload.update` |
| State action | POST | `/api/v1/catalog/product-image-upload/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/catalog/product-image-upload
...
{
  "featureKey": "catalog.product-image-upload",
...
- Access consideration 1: Permission `catalog.product-image-upload.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `catalog.product-image-upload.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `catalog.product-image-upload.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `catalog.product-image-upload.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `catalog.product-image-upload.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `catalog.product-image-upload.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `catalog.product-image-upload.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `catalog.product-image-upload.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `catalog.product-image-upload.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `catalog.product-image-upload.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `catalog.product-image-upload.read` may show data, while command permissions must be separately configured per role.
