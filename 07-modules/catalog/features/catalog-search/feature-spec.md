---
title: "Catalog Search Feature Specification"
...
# Catalog Search Feature Specification

## Purpose
This document defines the implementation scope for `catalog-search` inside the `catalog` module of the Unified Commerce platform.
...
|---|---|
| `brands` | Source or supporting table for `catalog-search` behavior. |
| `suppliers` | Source or supporting table for `catalog-search` behavior. |
| `categories` | Source or supporting table for `catalog-search` behavior. |
| `products` | Source or supporting table for `catalog-search` behavior. |
| `product_variants` | Source or supporting table for `catalog-search` behavior. |
| `product_attributes` | Source or supporting table for `catalog-search` behavior. |
| `attribute_values` | Source or supporting table for `catalog-search` behavior. |
| `product_images` | Source or supporting table for `catalog-search` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `catalog.catalog-search.read` | View data and search records. | Yes |
| `catalog.catalog-search.create` | Create new records or workflow drafts. | Yes |
| `catalog.catalog-search.update` | Modify allowed editable fields. | Yes |
| `catalog.catalog-search.approve` | Approve sensitive actions where applicable. | Yes |
| `catalog.catalog-search.void` | Void/cancel/reverse where business rules allow. | Yes |
...
public sealed class CatalogSearchService : ICatalogSearchService
{
    ...
    public async Task<ApiResponse<CatalogSearchDto>> HandleAsync(CatalogSearchRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "catalog.catalog-search.manage");
        ...
        return ApiResponse.Success(new CatalogSearchDto());
    }
}
```
...
```ts
export const catalogsearchQueryKey = (tenantId: string, outletId?: string) =>
  ["catalog", "catalog-search", tenantId, outletId ?? "tenant"] as const;

export function useCatalogSearch(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: catalogsearchQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/catalog/catalog-search"),
    staleTime: 60_000,
  });
}
```
