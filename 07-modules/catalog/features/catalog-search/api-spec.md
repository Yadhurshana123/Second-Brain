---
title: "Catalog Search API Specification"
...
## API Ownership
The `catalog-search` API belongs to the `catalog` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/catalog/catalog-search` | `catalog.catalog-search.read` |
| Detail | GET | `/api/v1/catalog/catalog-search/{id}` | `catalog.catalog-search.read` |
| Create | POST | `/api/v1/catalog/catalog-search` | `catalog.catalog-search.create` |
| Update | PUT | `/api/v1/catalog/catalog-search/{id}` | `catalog.catalog-search.update` |
| State action | POST | `/api/v1/catalog/catalog-search/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/catalog/catalog-search
...
{
  "featureKey": "catalog.catalog-search",
...
- Access consideration 1: Permission `catalog.catalog-search.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `catalog.catalog-search.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `catalog.catalog-search.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `catalog.catalog-search.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `catalog.catalog-search.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `catalog.catalog-search.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `catalog.catalog-search.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `catalog.catalog-search.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `catalog.catalog-search.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `catalog.catalog-search.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `catalog.catalog-search.read` may show data, while command permissions must be separately configured per role.
