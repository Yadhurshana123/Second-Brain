---
title: "Product Publishing Feature Specification"
...
# Product Publishing Feature Specification

## Purpose
This document defines the implementation scope for `product-publishing` inside the `catalog` module of the Unified Commerce platform.
...
|---|---|
| `brands` | Source or supporting table for `product-publishing` behavior. |
| `suppliers` | Source or supporting table for `product-publishing` behavior. |
| `categories` | Source or supporting table for `product-publishing` behavior. |
| `products` | Source or supporting table for `product-publishing` behavior. |
| `product_variants` | Source or supporting table for `product-publishing` behavior. |
| `product_attributes` | Source or supporting table for `product-publishing` behavior. |
| `attribute_values` | Source or supporting table for `product-publishing` behavior. |
| `product_images` | Source or supporting table for `product-publishing` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `catalog.product-publishing.read` | View data and search records. | Yes |
| `catalog.product-publishing.create` | Create new records or workflow drafts. | Yes |
| `catalog.product-publishing.update` | Modify allowed editable fields. | Yes |
| `catalog.product-publishing.approve` | Approve sensitive actions where applicable. | Yes |
| `catalog.product-publishing.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class ProductPublishingService : IProductPublishingService
{
    private readonly IProductPublishingRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<ProductPublishingDto>> HandleAsync(ProductPublishingRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "catalog.product-publishing.manage");
        ...
        return ApiResponse.Success(new ProductPublishingDto());
    }
}
```
...
```ts
export const productpublishingQueryKey = (tenantId: string, outletId?: string) =>
  ["catalog", "product-publishing", tenantId, outletId ?? "tenant"] as const;

export function useProductPublishing(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: productpublishingQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/catalog/product-publishing"),
    staleTime: 60_000,
  });
}
```
