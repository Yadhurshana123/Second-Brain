---
title: "Product Publishing API Specification"
...
# Product Publishing API Specification

## API Ownership
The `product-publishing` API belongs to the `catalog` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/catalog/product-publishing` | `catalog.product-publishing.read` |
| Detail | GET | `/api/v1/catalog/product-publishing/{id}` | `catalog.product-publishing.read` |
| Create | POST | `/api/v1/catalog/product-publishing` | `catalog.product-publishing.create` |
| Update | PUT | `/api/v1/catalog/product-publishing/{id}` | `catalog.product-publishing.update` |
| State action | POST | `/api/v1/catalog/product-publishing/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/catalog/product-publishing
...
{
  "featureKey": "catalog.product-publishing",
...
- Access consideration 1: Permission `catalog.product-publishing.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `catalog.product-publishing.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `catalog.product-publishing.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `catalog.product-publishing.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `catalog.product-publishing.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `catalog.product-publishing.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `catalog.product-publishing.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `catalog.product-publishing.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `catalog.product-publishing.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `catalog.product-publishing.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `catalog.product-publishing.read` may show data, while command permissions must be separately configured per role.
