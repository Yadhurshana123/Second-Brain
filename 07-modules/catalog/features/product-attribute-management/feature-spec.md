---
title: "Product Attribute Management Feature Specification"
...
# Product Attribute Management Feature Specification

## Purpose
This document defines the implementation scope for `product-attribute-management` inside the `catalog` module of the Unified Commerce platform.
...
|---|---|
| `brands` | Source or supporting table for `product-attribute-management` behavior. |
| `suppliers` | Source or supporting table for `product-attribute-management` behavior. |
| `categories` | Source or supporting table for `product-attribute-management` behavior. |
| `products` | Source or supporting table for `product-attribute-management` behavior. |
| `product_variants` | Source or supporting table for `product-attribute-management` behavior. |
| `product_attributes` | Source or supporting table for `product-attribute-management` behavior. |
| `attribute_values` | Source or supporting table for `product-attribute-management` behavior. |
| `product_images` | Source or supporting table for `product-attribute-management` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `catalog.product-attribute-management.read` | View data and search records. | Yes |
| `catalog.product-attribute-management.create` | Create new records or workflow drafts. | Yes |
| `catalog.product-attribute-management.update` | Modify allowed editable fields. | Yes |
| `catalog.product-attribute-management.approve` | Approve sensitive actions where applicable. | Yes |
| `catalog.product-attribute-management.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class ProductAttributeManagementService : IProductAttributeManagementService
{
    private readonly IProductAttributeManagementRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<ProductAttributeManagementDto>> HandleAsync(ProductAttributeManagementRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "catalog.product-attribute-management.manage");
        ...
        return ApiResponse.Success(new ProductAttributeManagementDto());
    }
}
```
...
```ts
export const productattributemanagementQueryKey = (tenantId: string, outletId?: string) =>
  ["catalog", "product-attribute-management", tenantId, outletId ?? "tenant"] as const;

export function useProductAttributeManagement(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: productattributemanagementQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/catalog/product-attribute-management"),
    staleTime: 60_000,
  });
}
```
