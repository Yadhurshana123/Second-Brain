---
title: "Supplier Management API Specification"
...
## API Ownership
The `supplier-management` API belongs to the `catalog` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/catalog/supplier-management` | `catalog.supplier-management.read` |
| Detail | GET | `/api/v1/catalog/supplier-management/{id}` | `catalog.supplier-management.read` |
| Create | POST | `/api/v1/catalog/supplier-management` | `catalog.supplier-management.create` |
| Update | PUT | `/api/v1/catalog/supplier-management/{id}` | `catalog.supplier-management.update` |
| State action | POST | `/api/v1/catalog/supplier-management/{id}/actions/{action}` | Action-specific permission |
...
- Access consideration 1: Permission `catalog.supplier-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `catalog.supplier-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `catalog.supplier-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `catalog.supplier-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `catalog.supplier-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `catalog.supplier-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `catalog.supplier-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `catalog.supplier-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `catalog.supplier-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `catalog.supplier-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `catalog.supplier-management.read` may show data, while command permissions must be separately configured per role.
