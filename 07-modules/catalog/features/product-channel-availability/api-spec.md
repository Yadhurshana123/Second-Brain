---
title: "Product Channel Availability API Specification"
...
# Product Channel Availability API Specification

## API Ownership
The `product-channel-availability` API belongs to the `catalog` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/catalog/product-channel-availability` | `catalog.product-channel-availability.read` |
| Detail | GET | `/api/v1/catalog/product-channel-availability/{id}` | `catalog.product-channel-availability.read` |
| Create | POST | `/api/v1/catalog/product-channel-availability` | `catalog.product-channel-availability.create` |
| Update | PUT | `/api/v1/catalog/product-channel-availability/{id}` | `catalog.product-channel-availability.update` |
| State action | POST | `/api/v1/catalog/product-channel-availability/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/catalog/product-channel-availability
...
{
  "featureKey": "catalog.product-channel-availability",
...
- Access consideration 1: Permission `catalog.product-channel-availability.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `catalog.product-channel-availability.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `catalog.product-channel-availability.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `catalog.product-channel-availability.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `catalog.product-channel-availability.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `catalog.product-channel-availability.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `catalog.product-channel-availability.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `catalog.product-channel-availability.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `catalog.product-channel-availability.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `catalog.product-channel-availability.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `catalog.product-channel-availability.read` may show data, while command permissions must be separately configured per role.
