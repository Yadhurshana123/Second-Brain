---
title: "Product Channel Availability Feature Specification"
...
# Product Channel Availability Feature Specification

## Purpose
This document defines the implementation scope for `product-channel-availability` inside the `catalog` module of the Unified Commerce platform.
...
|---|---|
| `brands` | Source or supporting table for `product-channel-availability` behavior. |
| `suppliers` | Source or supporting table for `product-channel-availability` behavior. |
| `categories` | Source or supporting table for `product-channel-availability` behavior. |
| `products` | Source or supporting table for `product-channel-availability` behavior. |
| `product_variants` | Source or supporting table for `product-channel-availability` behavior. |
| `product_attributes` | Source or supporting table for `product-channel-availability` behavior. |
| `attribute_values` | Source or supporting table for `product-channel-availability` behavior. |
| `product_images` | Source or supporting table for `product-channel-availability` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `catalog.product-channel-availability.read` | View data and search records. | Yes |
| `catalog.product-channel-availability.create` | Create new records or workflow drafts. | Yes |
| `catalog.product-channel-availability.update` | Modify allowed editable fields. | Yes |
| `catalog.product-channel-availability.approve` | Approve sensitive actions where applicable. | Yes |
| `catalog.product-channel-availability.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class ProductChannelAvailabilityService : IProductChannelAvailabilityService
{
    private readonly IProductChannelAvailabilityRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<ProductChannelAvailabilityDto>> HandleAsync(ProductChannelAvailabilityRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "catalog.product-channel-availability.manage");
        ...
        return ApiResponse.Success(new ProductChannelAvailabilityDto());
    }
}
```
...
```ts
export const productchannelavailabilityQueryKey = (tenantId: string, outletId?: string) =>
  ["catalog", "product-channel-availability", tenantId, outletId ?? "tenant"] as const;

export function useProductChannelAvailability(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: productchannelavailabilityQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/catalog/product-channel-availability"),
    staleTime: 60_000,
  });
}
```
