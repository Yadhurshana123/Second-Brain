---
title: "Product Pricing Feature Specification"
...
# Product Pricing Feature Specification

## Purpose
This document defines the implementation scope for `product-pricing` inside the `catalog` module of the Unified Commerce platform.
...
|---|---|
| `brands` | Source or supporting table for `product-pricing` behavior. |
| `suppliers` | Source or supporting table for `product-pricing` behavior. |
| `categories` | Source or supporting table for `product-pricing` behavior. |
| `products` | Source or supporting table for `product-pricing` behavior. |
| `product_variants` | Source or supporting table for `product-pricing` behavior. |
| `product_attributes` | Source or supporting table for `product-pricing` behavior. |
| `attribute_values` | Source or supporting table for `product-pricing` behavior. |
| `product_images` | Source or supporting table for `product-pricing` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `catalog.product-pricing.read` | View data and search records. | Yes |
| `catalog.product-pricing.create` | Create new records or workflow drafts. | Yes |
| `catalog.product-pricing.update` | Modify allowed editable fields. | Yes |
| `catalog.product-pricing.approve` | Approve sensitive actions where applicable. | Yes |
| `catalog.product-pricing.void` | Void/cancel/reverse where business rules allow. | Yes |
...
public sealed class ProductPricingService : IProductPricingService
{
    ...
    public async Task<ApiResponse<ProductPricingDto>> HandleAsync(ProductPricingRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "catalog.product-pricing.manage");
        ...
        return ApiResponse.Success(new ProductPricingDto());
    }
}
```
...
```ts
export const productpricingQueryKey = (tenantId: string, outletId?: string) =>
  ["catalog", "product-pricing", tenantId, outletId ?? "tenant"] as const;

export function useProductPricing(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: productpricingQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/catalog/product-pricing"),
    staleTime: 60_000,
  });
}
```
