---
title: "Product Pricing API Specification"
...
# Product Pricing API Specification

## API Ownership
The `product-pricing` API belongs to the `catalog` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/catalog/product-pricing` | `catalog.product-pricing.read` |
| Detail | GET | `/api/v1/catalog/product-pricing/{id}` | `catalog.product-pricing.read` |
| Create | POST | `/api/v1/catalog/product-pricing` | `catalog.product-pricing.create` |
| Update | PUT | `/api/v1/catalog/product-pricing/{id}` | `catalog.product-pricing.update` |
| State action | POST | `/api/v1/catalog/product-pricing/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/catalog/product-pricing
...
{
  "featureKey": "catalog.product-pricing",
...
- Access consideration 1: Permission `catalog.product-pricing.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `catalog.product-pricing.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `catalog.product-pricing.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `catalog.product-pricing.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `catalog.product-pricing.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `catalog.product-pricing.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `catalog.product-pricing.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `catalog.product-pricing.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `catalog.product-pricing.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `catalog.product-pricing.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `catalog.product-pricing.read` may show data, while command permissions must be separately configured per role.
