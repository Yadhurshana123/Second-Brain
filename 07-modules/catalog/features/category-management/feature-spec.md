---
title: "Category Management Feature Specification"
...
# Category Management Feature Specification

## Purpose
This document defines the implementation scope for `category-management` inside the `catalog` module of the Unified Commerce platform.
...
|---|---|
| `brands` | Source or supporting table for `category-management` behavior. |
| `suppliers` | Source or supporting table for `category-management` behavior. |
| `categories` | Source or supporting table for `category-management` behavior. |
| `products` | Source or supporting table for `category-management` behavior. |
| `product_variants` | Source or supporting table for `category-management` behavior. |
| `product_attributes` | Source or supporting table for `category-management` behavior. |
| `attribute_values` | Source or supporting table for `category-management` behavior. |
| `product_images` | Source or supporting table for `category-management` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `catalog.category-management.read` | View data and search records. | Yes |
| `catalog.category-management.create` | Create new records or workflow drafts. | Yes |
| `catalog.category-management.update` | Modify allowed editable fields. | Yes |
| `catalog.category-management.approve` | Approve sensitive actions where applicable. | Yes |
| `catalog.category-management.void` | Void/cancel/reverse where business rules allow. | Yes |
...
public sealed class CategoryManagementService : ICategoryManagementService
{
    ...
    public async Task<ApiResponse<CategoryManagementDto>> HandleAsync(CategoryManagementRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "catalog.category-management.manage");
        ...
        return ApiResponse.Success(new CategoryManagementDto());
    }
}
```
...
```ts
export const categorymanagementQueryKey = (tenantId: string, outletId?: string) =>
  ["catalog", "category-management", tenantId, outletId ?? "tenant"] as const;

export function useCategoryManagement(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: categorymanagementQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/catalog/category-management"),
    staleTime: 60_000,
  });
}
```
