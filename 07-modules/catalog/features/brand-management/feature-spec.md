---
title: "Brand Management Feature Specification"
...
# Brand Management Feature Specification

## Purpose
This document defines the implementation scope for `brand-management` inside the `catalog` module of the Unified Commerce platform.
...
|---|---|
| `brands` | Source or supporting table for `brand-management` behavior. |
| `suppliers` | Source or supporting table for `brand-management` behavior. |
| `categories` | Source or supporting table for `brand-management` behavior. |
| `products` | Source or supporting table for `brand-management` behavior. |
| `product_variants` | Source or supporting table for `brand-management` behavior. |
| `product_attributes` | Source or supporting table for `brand-management` behavior. |
| `attribute_values` | Source or supporting table for `brand-management` behavior. |
| `product_images` | Source or supporting table for `brand-management` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `catalog.brand-management.read` | View data and search records. | Yes |
| `catalog.brand-management.create` | Create new records or workflow drafts. | Yes |
| `catalog.brand-management.update` | Modify allowed editable fields. | Yes |
| `catalog.brand-management.approve` | Approve sensitive actions where applicable. | Yes |
| `catalog.brand-management.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class BrandManagementService : IBrandManagementService
{
    private readonly IBrandManagementRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<BrandManagementDto>> HandleAsync(BrandManagementRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "catalog.brand-management.manage");
        ...
        return ApiResponse.Success(new BrandManagementDto());
    }
}
```
...
```ts
export const brandmanagementQueryKey = (tenantId: string, outletId?: string) =>
  ["catalog", "brand-management", tenantId, outletId ?? "tenant"] as const;

export function useBrandManagement(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: brandmanagementQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/catalog/brand-management"),
    staleTime: 60_000,
  });
}
```
