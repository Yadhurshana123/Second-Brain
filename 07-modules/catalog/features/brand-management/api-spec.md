---
title: "Brand Management API Specification"
...
# Brand Management API Specification

## API Ownership
The `brand-management` API belongs to the `catalog` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/catalog/brand-management` | `catalog.brand-management.read` |
| Detail | GET | `/api/v1/catalog/brand-management/{id}` | `catalog.brand-management.read` |
| Create | POST | `/api/v1/catalog/brand-management` | `catalog.brand-management.create` |
| Update | PUT | `/api/v1/catalog/brand-management/{id}` | `catalog.brand-management.update` |
| State action | POST | `/api/v1/catalog/brand-management/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/catalog/brand-management
...
{
  "featureKey": "catalog.brand-management",
...
- Access consideration 1: Permission `catalog.brand-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `catalog.brand-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `catalog.brand-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `catalog.brand-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `catalog.brand-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `catalog.brand-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `catalog.brand-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `catalog.brand-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `catalog.brand-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `catalog.brand-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `catalog.brand-management.read` may show data, while command permissions must be separately configured per role.
