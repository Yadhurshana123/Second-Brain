---
title: "Product Management Feature Specification"
...
## Purpose
This document defines the implementation scope for `product-management` inside the `catalog` module of the Unified Commerce platform.
...
|---|---|
| `brands` | Source or supporting table for `product-management` behavior. |
| `suppliers` | Source or supporting table for `product-management` behavior. |
| `categories` | Source or supporting table for `product-management` behavior. |
| `products` | Source or supporting table for `product-management` behavior. |
| `product_variants` | Source or supporting table for `product-management` behavior. |
| `product_attributes` | Source or supporting table for `product-management` behavior. |
| `attribute_values` | Source or supporting table for `product-management` behavior. |
| `product_images` | Source or supporting table for `product-management` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `catalog.product-management.read` | View data and search records. | Yes |
| `catalog.product-management.create` | Create new records or workflow drafts. | Yes |
| `catalog.product-management.update` | Modify allowed editable fields. | Yes |
| `catalog.product-management.approve` | Approve sensitive actions where applicable. | Yes |
| `catalog.product-management.void` | Void/cancel/reverse where business rules allow. | Yes |
...
public sealed class ProductManagementService : IProductManagementService
{
    ...
    public async Task<ApiResponse<ProductManagementDto>> HandleAsync(ProductManagementRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "catalog.product-management.manage");
        ...
        return ApiResponse.Success(new ProductManagementDto());
    }
}
```
...
```ts
export const productmanagementQueryKey = (tenantId: string, outletId?: string) =>
  ["catalog", "product-management", tenantId, outletId ?? "tenant"] as const;

export function useProductManagement(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: productmanagementQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/catalog/product-management"),
    staleTime: 60_000,
  });
}
```
