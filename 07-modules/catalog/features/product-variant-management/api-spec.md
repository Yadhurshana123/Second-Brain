---
title: "Product Variant Management API Specification"
...
# Product Variant Management API Specification

## API Ownership
The `product-variant-management` API belongs to the `catalog` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/catalog/product-variant-management` | `catalog.product-variant-management.read` |
| Detail | GET | `/api/v1/catalog/product-variant-management/{id}` | `catalog.product-variant-management.read` |
| Create | POST | `/api/v1/catalog/product-variant-management` | `catalog.product-variant-management.create` |
| Update | PUT | `/api/v1/catalog/product-variant-management/{id}` | `catalog.product-variant-management.update` |
| State action | POST | `/api/v1/catalog/product-variant-management/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/catalog/product-variant-management
...
{
  "featureKey": "catalog.product-variant-management",
...
- Access consideration 1: Permission `catalog.product-variant-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `catalog.product-variant-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `catalog.product-variant-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `catalog.product-variant-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `catalog.product-variant-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `catalog.product-variant-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `catalog.product-variant-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `catalog.product-variant-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `catalog.product-variant-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `catalog.product-variant-management.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `catalog.product-variant-management.read` may show data, while command permissions must be separately configured per role.
