---
title: "Product Variant Management Feature Specification"
...
# Product Variant Management Feature Specification

## Purpose
This document defines the implementation scope for `product-variant-management` inside the `catalog` module of the Unified Commerce platform.
...
|---|---|
| `brands` | Source or supporting table for `product-variant-management` behavior. |
| `suppliers` | Source or supporting table for `product-variant-management` behavior. |
| `categories` | Source or supporting table for `product-variant-management` behavior. |
| `products` | Source or supporting table for `product-variant-management` behavior. |
| `product_variants` | Source or supporting table for `product-variant-management` behavior. |
| `product_attributes` | Source or supporting table for `product-variant-management` behavior. |
| `attribute_values` | Source or supporting table for `product-variant-management` behavior. |
| `product_images` | Source or supporting table for `product-variant-management` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `catalog.product-variant-management.read` | View data and search records. | Yes |
| `catalog.product-variant-management.create` | Create new records or workflow drafts. | Yes |
| `catalog.product-variant-management.update` | Modify allowed editable fields. | Yes |
| `catalog.product-variant-management.approve` | Approve sensitive actions where applicable. | Yes |
| `catalog.product-variant-management.void` | Void/cancel/reverse where business rules allow. | Yes |
...
{
    private readonly IProductVariantManagementRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<ProductVariantManagementDto>> HandleAsync(ProductVariantManagementRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "catalog.product-variant-management.manage");
        ...
        return ApiResponse.Success(new ProductVariantManagementDto());
    }
}
```
...
```ts
export const productvariantmanagementQueryKey = (tenantId: string, outletId?: string) =>
  ["catalog", "product-variant-management", tenantId, outletId ?? "tenant"] as const;

export function useProductVariantManagement(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: productvariantmanagementQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/catalog/product-variant-management"),
    staleTime: 60_000,
  });
}
```
