---
title: "Product Return Policy API Specification"
...
# Product Return Policy API Specification

## API Ownership
The `product-return-policy` API belongs to the `catalog` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/catalog/product-return-policy` | `catalog.product-return-policy.read` |
| Detail | GET | `/api/v1/catalog/product-return-policy/{id}` | `catalog.product-return-policy.read` |
| Create | POST | `/api/v1/catalog/product-return-policy` | `catalog.product-return-policy.create` |
| Update | PUT | `/api/v1/catalog/product-return-policy/{id}` | `catalog.product-return-policy.update` |
| State action | POST | `/api/v1/catalog/product-return-policy/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/catalog/product-return-policy
...
{
  "featureKey": "catalog.product-return-policy",
...
- Access consideration 1: Permission `catalog.product-return-policy.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `catalog.product-return-policy.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `catalog.product-return-policy.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `catalog.product-return-policy.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `catalog.product-return-policy.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `catalog.product-return-policy.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `catalog.product-return-policy.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `catalog.product-return-policy.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `catalog.product-return-policy.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `catalog.product-return-policy.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `catalog.product-return-policy.read` may show data, while command permissions must be separately configured per role.
