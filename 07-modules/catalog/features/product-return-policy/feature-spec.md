---
title: "Product Return Policy Feature Specification"
...
# Product Return Policy Feature Specification

## Purpose
This document defines the implementation scope for `product-return-policy` inside the `catalog` module of the Unified Commerce platform.
...
|---|---|
| `brands` | Source or supporting table for `product-return-policy` behavior. |
| `suppliers` | Source or supporting table for `product-return-policy` behavior. |
| `categories` | Source or supporting table for `product-return-policy` behavior. |
| `products` | Source or supporting table for `product-return-policy` behavior. |
| `product_variants` | Source or supporting table for `product-return-policy` behavior. |
| `product_attributes` | Source or supporting table for `product-return-policy` behavior. |
| `attribute_values` | Source or supporting table for `product-return-policy` behavior. |
| `product_images` | Source or supporting table for `product-return-policy` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `catalog.product-return-policy.read` | View data and search records. | Yes |
| `catalog.product-return-policy.create` | Create new records or workflow drafts. | Yes |
| `catalog.product-return-policy.update` | Modify allowed editable fields. | Yes |
| `catalog.product-return-policy.approve` | Approve sensitive actions where applicable. | Yes |
| `catalog.product-return-policy.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class ProductReturnPolicyService : IProductReturnPolicyService
{
    private readonly IProductReturnPolicyRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<ProductReturnPolicyDto>> HandleAsync(ProductReturnPolicyRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "catalog.product-return-policy.manage");
        ...
        return ApiResponse.Success(new ProductReturnPolicyDto());
    }
}
```
...
```ts
export const productreturnpolicyQueryKey = (tenantId: string, outletId?: string) =>
  ["catalog", "product-return-policy", tenantId, outletId ?? "tenant"] as const;

export function useProductReturnPolicy(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: productreturnpolicyQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/catalog/product-return-policy"),
    staleTime: 60_000,
  });
}
```
