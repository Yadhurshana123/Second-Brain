---
title: "Cash Count Denominations Feature Specification"
...
# Cash Count Denominations Feature Specification

## Purpose
This document defines the implementation scope for `cash-count-denominations` inside the `sales-pos` module of the Unified Commerce platform.
...
|---|---|
| `tills` | Source or supporting table for `cash-count-denominations` behavior. |
| `pos_devices` | Source or supporting table for `cash-count-denominations` behavior. |
| `till_sessions` | Source or supporting table for `cash-count-denominations` behavior. |
| `sales` | Source or supporting table for `cash-count-denominations` behavior. |
| `sale_lines` | Source or supporting table for `cash-count-denominations` behavior. |
| `cash_movements` | Source or supporting table for `cash-count-denominations` behavior. |
| `receipts` | Source or supporting table for `cash-count-denominations` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `sales-pos.cash-count-denominations.read` | View data and search records. | Yes |
| `sales-pos.cash-count-denominations.create` | Create new records or workflow drafts. | Yes |
| `sales-pos.cash-count-denominations.update` | Modify allowed editable fields. | Yes |
| `sales-pos.cash-count-denominations.approve` | Approve sensitive actions where applicable. | Yes |
| `sales-pos.cash-count-denominations.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class CashCountDenominationsService : ICashCountDenominationsService
{
    private readonly ICashCountDenominationsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<CashCountDenominationsDto>> HandleAsync(CashCountDenominationsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "sales-pos.cash-count-denominations.manage");
        ...
        return ApiResponse.Success(new CashCountDenominationsDto());
    }
}
```
...
```ts
export const cashcountdenominationsQueryKey = (tenantId: string, outletId?: string) =>
  ["sales-pos", "cash-count-denominations", tenantId, outletId ?? "tenant"] as const;

export function useCashCountDenominations(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: cashcountdenominationsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/sales-pos/cash-count-denominations"),
    staleTime: 60_000,
  });
}
```
