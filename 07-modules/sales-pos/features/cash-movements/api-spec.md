---
title: "Cash Movements API Specification"
...
# Cash Movements API Specification

## API Ownership
The `cash-movements` API belongs to the `sales-pos` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/sales-pos/cash-movements` | `sales-pos.cash-movements.read` |
| Detail | GET | `/api/v1/sales-pos/cash-movements/{id}` | `sales-pos.cash-movements.read` |
| Create | POST | `/api/v1/sales-pos/cash-movements` | `sales-pos.cash-movements.create` |
| Update | PUT | `/api/v1/sales-pos/cash-movements/{id}` | `sales-pos.cash-movements.update` |
| State action | POST | `/api/v1/sales-pos/cash-movements/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/sales-pos/cash-movements
...
{
  "featureKey": "sales-pos.cash-movements",
...
- Access consideration 1: Permission `sales-pos.cash-movements.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `sales-pos.cash-movements.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `sales-pos.cash-movements.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `sales-pos.cash-movements.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `sales-pos.cash-movements.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `sales-pos.cash-movements.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `sales-pos.cash-movements.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `sales-pos.cash-movements.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `sales-pos.cash-movements.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `sales-pos.cash-movements.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `sales-pos.cash-movements.read` may show data, while command permissions must be separately configured per role.
