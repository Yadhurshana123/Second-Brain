---
title: "Cash Movements Feature Specification"
...
# Cash Movements Feature Specification

## Purpose
This document defines the implementation scope for `cash-movements` inside the `sales-pos` module of the Unified Commerce platform.
...
|---|---|
| `tills` | Source or supporting table for `cash-movements` behavior. |
| `pos_devices` | Source or supporting table for `cash-movements` behavior. |
| `till_sessions` | Source or supporting table for `cash-movements` behavior. |
| `sales` | Source or supporting table for `cash-movements` behavior. |
| `sale_lines` | Source or supporting table for `cash-movements` behavior. |
| `cash_movements` | Source or supporting table for `cash-movements` behavior. |
| `receipts` | Source or supporting table for `cash-movements` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `sales-pos.cash-movements.read` | View data and search records. | Yes |
| `sales-pos.cash-movements.create` | Create new records or workflow drafts. | Yes |
| `sales-pos.cash-movements.update` | Modify allowed editable fields. | Yes |
| `sales-pos.cash-movements.approve` | Approve sensitive actions where applicable. | Yes |
| `sales-pos.cash-movements.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class CashMovementsService : ICashMovementsService
{
    private readonly ICashMovementsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<CashMovementsDto>> HandleAsync(CashMovementsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "sales-pos.cash-movements.manage");
        ...
        return ApiResponse.Success(new CashMovementsDto());
    }
}
```
...
```ts
export const cashmovementsQueryKey = (tenantId: string, outletId?: string) =>
  ["sales-pos", "cash-movements", tenantId, outletId ?? "tenant"] as const;

export function useCashMovements(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: cashmovementsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/sales-pos/cash-movements"),
    staleTime: 60_000,
  });
}
```
