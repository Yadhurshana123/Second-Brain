---
title: "Till Sessions Feature Specification"
...
# Till Sessions Feature Specification

## Purpose
This document defines the implementation scope for `till-sessions` inside the `sales-pos` module of the Unified Commerce platform.
...
|---|---|
| `tills` | Source or supporting table for `till-sessions` behavior. |
| `pos_devices` | Source or supporting table for `till-sessions` behavior. |
| `till_sessions` | Source or supporting table for `till-sessions` behavior. |
| `sales` | Source or supporting table for `till-sessions` behavior. |
| `sale_lines` | Source or supporting table for `till-sessions` behavior. |
| `cash_movements` | Source or supporting table for `till-sessions` behavior. |
| `receipts` | Source or supporting table for `till-sessions` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `sales-pos.till-sessions.read` | View data and search records. | Yes |
| `sales-pos.till-sessions.create` | Create new records or workflow drafts. | Yes |
| `sales-pos.till-sessions.update` | Modify allowed editable fields. | Yes |
| `sales-pos.till-sessions.approve` | Approve sensitive actions where applicable. | Yes |
| `sales-pos.till-sessions.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class TillSessionsService : ITillSessionsService
{
    private readonly ITillSessionsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<TillSessionsDto>> HandleAsync(TillSessionsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "sales-pos.till-sessions.manage");
        ...
        return ApiResponse.Success(new TillSessionsDto());
    }
}
```
...
```ts
export const tillsessionsQueryKey = (tenantId: string, outletId?: string) =>
  ["sales-pos", "till-sessions", tenantId, outletId ?? "tenant"] as const;

export function useTillSessions(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: tillsessionsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/sales-pos/till-sessions"),
    staleTime: 60_000,
  });
}
```
