---
title: "Till Sessions API Specification"
...
# Till Sessions API Specification

## API Ownership
The `till-sessions` API belongs to the `sales-pos` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/sales-pos/till-sessions` | `sales-pos.till-sessions.read` |
| Detail | GET | `/api/v1/sales-pos/till-sessions/{id}` | `sales-pos.till-sessions.read` |
| Create | POST | `/api/v1/sales-pos/till-sessions` | `sales-pos.till-sessions.create` |
| Update | PUT | `/api/v1/sales-pos/till-sessions/{id}` | `sales-pos.till-sessions.update` |
| State action | POST | `/api/v1/sales-pos/till-sessions/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/sales-pos/till-sessions
...
{
  "featureKey": "sales-pos.till-sessions",
...
- Access consideration 1: Permission `sales-pos.till-sessions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `sales-pos.till-sessions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `sales-pos.till-sessions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `sales-pos.till-sessions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `sales-pos.till-sessions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `sales-pos.till-sessions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `sales-pos.till-sessions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `sales-pos.till-sessions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `sales-pos.till-sessions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `sales-pos.till-sessions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `sales-pos.till-sessions.read` may show data, while command permissions must be separately configured per role.
