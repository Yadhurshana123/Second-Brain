---
title: "Hold Recall Sale Feature Specification"
...
# Hold Recall Sale Feature Specification

## Purpose
This document defines the implementation scope for `hold-recall-sale` inside the `sales-pos` module of the Unified Commerce platform.
...
|---|---|
| `tills` | Source or supporting table for `hold-recall-sale` behavior. |
| `pos_devices` | Source or supporting table for `hold-recall-sale` behavior. |
| `till_sessions` | Source or supporting table for `hold-recall-sale` behavior. |
| `sales` | Source or supporting table for `hold-recall-sale` behavior. |
| `sale_lines` | Source or supporting table for `hold-recall-sale` behavior. |
| `cash_movements` | Source or supporting table for `hold-recall-sale` behavior. |
| `receipts` | Source or supporting table for `hold-recall-sale` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `sales-pos.hold-recall-sale.read` | View data and search records. | Yes |
| `sales-pos.hold-recall-sale.create` | Create new records or workflow drafts. | Yes |
| `sales-pos.hold-recall-sale.update` | Modify allowed editable fields. | Yes |
| `sales-pos.hold-recall-sale.approve` | Approve sensitive actions where applicable. | Yes |
| `sales-pos.hold-recall-sale.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class HoldRecallSaleService : IHoldRecallSaleService
{
    private readonly IHoldRecallSaleRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<HoldRecallSaleDto>> HandleAsync(HoldRecallSaleRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "sales-pos.hold-recall-sale.manage");
        ...
        return ApiResponse.Success(new HoldRecallSaleDto());
    }
}
```
...
```ts
export const holdrecallsaleQueryKey = (tenantId: string, outletId?: string) =>
  ["sales-pos", "hold-recall-sale", tenantId, outletId ?? "tenant"] as const;

export function useHoldRecallSale(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: holdrecallsaleQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/sales-pos/hold-recall-sale"),
    staleTime: 60_000,
  });
}
```
