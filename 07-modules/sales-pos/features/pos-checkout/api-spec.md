---
title: "Pos Checkout API Specification"
...
## API Ownership
The `pos-checkout` API belongs to the `sales-pos` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/sales-pos/pos-checkout` | `sales-pos.pos-checkout.read` |
| Detail | GET | `/api/v1/sales-pos/pos-checkout/{id}` | `sales-pos.pos-checkout.read` |
| Create | POST | `/api/v1/sales-pos/pos-checkout` | `sales-pos.pos-checkout.create` |
| Update | PUT | `/api/v1/sales-pos/pos-checkout/{id}` | `sales-pos.pos-checkout.update` |
| State action | POST | `/api/v1/sales-pos/pos-checkout/{id}/actions/{action}` | Action-specific permission |
...
- Access consideration 1: Permission `sales-pos.pos-checkout.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `sales-pos.pos-checkout.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `sales-pos.pos-checkout.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `sales-pos.pos-checkout.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `sales-pos.pos-checkout.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `sales-pos.pos-checkout.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `sales-pos.pos-checkout.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `sales-pos.pos-checkout.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `sales-pos.pos-checkout.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `sales-pos.pos-checkout.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `sales-pos.pos-checkout.read` may show data, while command permissions must be separately configured per role.
