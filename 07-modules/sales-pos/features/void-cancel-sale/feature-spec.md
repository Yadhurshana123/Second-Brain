---
title: "Void Cancel Sale Feature Specification"
...
# Void Cancel Sale Feature Specification

## Purpose
This document defines the implementation scope for `void-cancel-sale` inside the `sales-pos` module of the Unified Commerce platform.
...
|---|---|
| `tills` | Source or supporting table for `void-cancel-sale` behavior. |
| `pos_devices` | Source or supporting table for `void-cancel-sale` behavior. |
| `till_sessions` | Source or supporting table for `void-cancel-sale` behavior. |
| `sales` | Source or supporting table for `void-cancel-sale` behavior. |
| `sale_lines` | Source or supporting table for `void-cancel-sale` behavior. |
| `cash_movements` | Source or supporting table for `void-cancel-sale` behavior. |
| `receipts` | Source or supporting table for `void-cancel-sale` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `sales-pos.void-cancel-sale.read` | View data and search records. | Yes |
| `sales-pos.void-cancel-sale.create` | Create new records or workflow drafts. | Yes |
| `sales-pos.void-cancel-sale.update` | Modify allowed editable fields. | Yes |
| `sales-pos.void-cancel-sale.approve` | Approve sensitive actions where applicable. | Yes |
| `sales-pos.void-cancel-sale.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class VoidCancelSaleService : IVoidCancelSaleService
{
    private readonly IVoidCancelSaleRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<VoidCancelSaleDto>> HandleAsync(VoidCancelSaleRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "sales-pos.void-cancel-sale.manage");
        ...
        return ApiResponse.Success(new VoidCancelSaleDto());
    }
}
```
...
```ts
export const voidcancelsaleQueryKey = (tenantId: string, outletId?: string) =>
  ["sales-pos", "void-cancel-sale", tenantId, outletId ?? "tenant"] as const;

export function useVoidCancelSale(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: voidcancelsaleQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/sales-pos/void-cancel-sale"),
    staleTime: 60_000,
  });
}
```
