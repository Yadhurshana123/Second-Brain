---
title: "Staff Users Feature Specification"
...
# Staff Users Feature Specification

## Purpose
This document defines the implementation scope for `staff-users` inside the `identity-access` module of the Unified Commerce platform.
...
|---|---|
| `users` | Source or supporting table for `staff-users` behavior. |
| `roles` | Source or supporting table for `staff-users` behavior. |
| `permissions` | Source or supporting table for `staff-users` behavior. |
| `role_permissions` | Source or supporting table for `staff-users` behavior. |
| `tenant_user_roles` | Source or supporting table for `staff-users` behavior. |
| `outlet_user_roles` | Source or supporting table for `staff-users` behavior. |
| `role_feature_assignments` | Source or supporting table for `staff-users` behavior. |
| `feature_flags` | Source or supporting table for `staff-users` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `identity-access.staff-users.read` | View data and search records. | Yes |
| `identity-access.staff-users.create` | Create new records or workflow drafts. | Yes |
| `identity-access.staff-users.update` | Modify allowed editable fields. | Yes |
| `identity-access.staff-users.approve` | Approve sensitive actions where applicable. | Yes |
| `identity-access.staff-users.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class StaffUsersService : IStaffUsersService
{
    private readonly IStaffUsersRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<StaffUsersDto>> HandleAsync(StaffUsersRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "identity-access.staff-users.manage");
        ...
        return ApiResponse.Success(new StaffUsersDto());
    }
}
```
...
```ts
export const staffusersQueryKey = (tenantId: string, outletId?: string) =>
  ["identity-access", "staff-users", tenantId, outletId ?? "tenant"] as const;

export function useStaffUsers(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: staffusersQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/identity-access/staff-users"),
    staleTime: 60_000,
  });
}
```
