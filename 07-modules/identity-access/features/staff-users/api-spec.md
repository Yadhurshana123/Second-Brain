---
title: "Staff Users API Specification"
...
# Staff Users API Specification

## API Ownership
The `staff-users` API belongs to the `identity-access` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/identity-access/staff-users` | `identity-access.staff-users.read` |
| Detail | GET | `/api/v1/identity-access/staff-users/{id}` | `identity-access.staff-users.read` |
| Create | POST | `/api/v1/identity-access/staff-users` | `identity-access.staff-users.create` |
| Update | PUT | `/api/v1/identity-access/staff-users/{id}` | `identity-access.staff-users.update` |
| State action | POST | `/api/v1/identity-access/staff-users/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/identity-access/staff-users
...
{
  "featureKey": "identity-access.staff-users",
...
- Access consideration 1: Permission `identity-access.staff-users.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `identity-access.staff-users.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `identity-access.staff-users.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `identity-access.staff-users.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `identity-access.staff-users.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `identity-access.staff-users.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `identity-access.staff-users.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `identity-access.staff-users.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `identity-access.staff-users.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `identity-access.staff-users.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `identity-access.staff-users.read` may show data, while command permissions must be separately configured per role.
