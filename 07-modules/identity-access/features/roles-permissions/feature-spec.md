---
title: "Roles Permissions Feature Specification"
...
## Purpose
This document defines the implementation scope for `roles-permissions` inside the `identity-access` module of the Unified Commerce platform.
...
|---|---|
| `users` | Source or supporting table for `roles-permissions` behavior. |
| `roles` | Source or supporting table for `roles-permissions` behavior. |
| `permissions` | Source or supporting table for `roles-permissions` behavior. |
| `role_permissions` | Source or supporting table for `roles-permissions` behavior. |
| `tenant_user_roles` | Source or supporting table for `roles-permissions` behavior. |
| `outlet_user_roles` | Source or supporting table for `roles-permissions` behavior. |
| `role_feature_assignments` | Source or supporting table for `roles-permissions` behavior. |
| `feature_flags` | Source or supporting table for `roles-permissions` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `identity-access.roles-permissions.read` | View data and search records. | Yes |
| `identity-access.roles-permissions.create` | Create new records or workflow drafts. | Yes |
| `identity-access.roles-permissions.update` | Modify allowed editable fields. | Yes |
| `identity-access.roles-permissions.approve` | Approve sensitive actions where applicable. | Yes |
| `identity-access.roles-permissions.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class RolesPermissionsService : IRolesPermissionsService
{
    private readonly IRolesPermissionsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<RolesPermissionsDto>> HandleAsync(RolesPermissionsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "identity-access.roles-permissions.manage");
        ...
        return ApiResponse.Success(new RolesPermissionsDto());
    }
}
```
...
```ts
export const rolespermissionsQueryKey = (tenantId: string, outletId?: string) =>
  ["identity-access", "roles-permissions", tenantId, outletId ?? "tenant"] as const;

export function useRolesPermissions(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: rolespermissionsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/identity-access/roles-permissions"),
    staleTime: 60_000,
  });
}
```
