---
title: "Role Feature Assignments API Specification"
...
# Role Feature Assignments API Specification

## API Ownership
The `role-feature-assignments` API belongs to the `identity-access` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/identity-access/role-feature-assignments` | `identity-access.role-feature-assignments.read` |
| Detail | GET | `/api/v1/identity-access/role-feature-assignments/{id}` | `identity-access.role-feature-assignments.read` |
| Create | POST | `/api/v1/identity-access/role-feature-assignments` | `identity-access.role-feature-assignments.create` |
| Update | PUT | `/api/v1/identity-access/role-feature-assignments/{id}` | `identity-access.role-feature-assignments.update` |
| State action | POST | `/api/v1/identity-access/role-feature-assignments/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/identity-access/role-feature-assignments
...
{
  "featureKey": "identity-access.role-feature-assignments",
...
- Access consideration 1: Permission `identity-access.role-feature-assignments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `identity-access.role-feature-assignments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `identity-access.role-feature-assignments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `identity-access.role-feature-assignments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `identity-access.role-feature-assignments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `identity-access.role-feature-assignments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `identity-access.role-feature-assignments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `identity-access.role-feature-assignments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `identity-access.role-feature-assignments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `identity-access.role-feature-assignments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `identity-access.role-feature-assignments.read` may show data, while command permissions must be separately configured per role.
