---
title: "Role Feature Assignments Feature Specification"
...
# Role Feature Assignments Feature Specification

## Purpose
This document defines the implementation scope for `role-feature-assignments` inside the `identity-access` module of the Unified Commerce platform.
...
|---|---|
| `users` | Source or supporting table for `role-feature-assignments` behavior. |
| `roles` | Source or supporting table for `role-feature-assignments` behavior. |
| `permissions` | Source or supporting table for `role-feature-assignments` behavior. |
| `role_permissions` | Source or supporting table for `role-feature-assignments` behavior. |
| `tenant_user_roles` | Source or supporting table for `role-feature-assignments` behavior. |
| `outlet_user_roles` | Source or supporting table for `role-feature-assignments` behavior. |
| `role_feature_assignments` | Source or supporting table for `role-feature-assignments` behavior. |
| `feature_flags` | Source or supporting table for `role-feature-assignments` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `identity-access.role-feature-assignments.read` | View data and search records. | Yes |
| `identity-access.role-feature-assignments.create` | Create new records or workflow drafts. | Yes |
| `identity-access.role-feature-assignments.update` | Modify allowed editable fields. | Yes |
| `identity-access.role-feature-assignments.approve` | Approve sensitive actions where applicable. | Yes |
| `identity-access.role-feature-assignments.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class RoleFeatureAssignmentsService : IRoleFeatureAssignmentsService
{
    private readonly IRoleFeatureAssignmentsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<RoleFeatureAssignmentsDto>> HandleAsync(RoleFeatureAssignmentsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "identity-access.role-feature-assignments.manage");
        ...
        return ApiResponse.Success(new RoleFeatureAssignmentsDto());
    }
}
```
...
```ts
export const rolefeatureassignmentsQueryKey = (tenantId: string, outletId?: string) =>
  ["identity-access", "role-feature-assignments", tenantId, outletId ?? "tenant"] as const;

export function useRoleFeatureAssignments(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: rolefeatureassignmentsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/identity-access/role-feature-assignments"),
    staleTime: 60_000,
  });
}
```
...
- Related module overview: [[../README|Identity Access Overview]]
