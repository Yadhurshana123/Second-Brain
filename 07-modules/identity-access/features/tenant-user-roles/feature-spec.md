---
title: "Tenant User Roles Feature Specification"
...
# Tenant User Roles Feature Specification

## Purpose
This document defines the implementation scope for `tenant-user-roles` inside the `identity-access` module of the Unified Commerce platform.
...
|---|---|
| `users` | Source or supporting table for `tenant-user-roles` behavior. |
| `roles` | Source or supporting table for `tenant-user-roles` behavior. |
| `permissions` | Source or supporting table for `tenant-user-roles` behavior. |
| `role_permissions` | Source or supporting table for `tenant-user-roles` behavior. |
| `tenant_user_roles` | Source or supporting table for `tenant-user-roles` behavior. |
| `outlet_user_roles` | Source or supporting table for `tenant-user-roles` behavior. |
| `role_feature_assignments` | Source or supporting table for `tenant-user-roles` behavior. |
| `feature_flags` | Source or supporting table for `tenant-user-roles` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `identity-access.tenant-user-roles.read` | View data and search records. | Yes |
| `identity-access.tenant-user-roles.create` | Create new records or workflow drafts. | Yes |
| `identity-access.tenant-user-roles.update` | Modify allowed editable fields. | Yes |
| `identity-access.tenant-user-roles.approve` | Approve sensitive actions where applicable. | Yes |
| `identity-access.tenant-user-roles.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class TenantUserRolesService : ITenantUserRolesService
{
    private readonly ITenantUserRolesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<TenantUserRolesDto>> HandleAsync(TenantUserRolesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "identity-access.tenant-user-roles.manage");
        ...
        return ApiResponse.Success(new TenantUserRolesDto());
    }
}
```
...
```ts
export const tenantuserrolesQueryKey = (tenantId: string, outletId?: string) =>
  ["identity-access", "tenant-user-roles", tenantId, outletId ?? "tenant"] as const;

export function useTenantUserRoles(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: tenantuserrolesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/identity-access/tenant-user-roles"),
    staleTime: 60_000,
  });
}
```
