---
title: "Tenant User Roles API Specification"
...
# Tenant User Roles API Specification

## API Ownership
The `tenant-user-roles` API belongs to the `identity-access` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/identity-access/tenant-user-roles` | `identity-access.tenant-user-roles.read` |
| Detail | GET | `/api/v1/identity-access/tenant-user-roles/{id}` | `identity-access.tenant-user-roles.read` |
| Create | POST | `/api/v1/identity-access/tenant-user-roles` | `identity-access.tenant-user-roles.create` |
| Update | PUT | `/api/v1/identity-access/tenant-user-roles/{id}` | `identity-access.tenant-user-roles.update` |
| State action | POST | `/api/v1/identity-access/tenant-user-roles/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/identity-access/tenant-user-roles
...
{
  "featureKey": "identity-access.tenant-user-roles",
...
- Access consideration 1: Permission `identity-access.tenant-user-roles.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `identity-access.tenant-user-roles.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `identity-access.tenant-user-roles.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `identity-access.tenant-user-roles.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `identity-access.tenant-user-roles.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `identity-access.tenant-user-roles.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `identity-access.tenant-user-roles.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `identity-access.tenant-user-roles.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `identity-access.tenant-user-roles.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `identity-access.tenant-user-roles.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `identity-access.tenant-user-roles.read` may show data, while command permissions must be separately configured per role.
