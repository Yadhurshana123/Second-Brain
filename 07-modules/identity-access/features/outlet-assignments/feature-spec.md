---
title: "Outlet Assignments Feature Specification"
...
# Outlet Assignments Feature Specification

## Purpose
This document defines the implementation scope for `outlet-assignments` inside the `identity-access` module of the Unified Commerce platform.
...
|---|---|
| `users` | Source or supporting table for `outlet-assignments` behavior. |
| `roles` | Source or supporting table for `outlet-assignments` behavior. |
| `permissions` | Source or supporting table for `outlet-assignments` behavior. |
| `role_permissions` | Source or supporting table for `outlet-assignments` behavior. |
| `tenant_user_roles` | Source or supporting table for `outlet-assignments` behavior. |
| `outlet_user_roles` | Source or supporting table for `outlet-assignments` behavior. |
| `role_feature_assignments` | Source or supporting table for `outlet-assignments` behavior. |
| `feature_flags` | Source or supporting table for `outlet-assignments` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `identity-access.outlet-assignments.read` | View data and search records. | Yes |
| `identity-access.outlet-assignments.create` | Create new records or workflow drafts. | Yes |
| `identity-access.outlet-assignments.update` | Modify allowed editable fields. | Yes |
| `identity-access.outlet-assignments.approve` | Approve sensitive actions where applicable. | Yes |
| `identity-access.outlet-assignments.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class OutletAssignmentsService : IOutletAssignmentsService
{
    private readonly IOutletAssignmentsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<OutletAssignmentsDto>> HandleAsync(OutletAssignmentsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "identity-access.outlet-assignments.manage");
        ...
        return ApiResponse.Success(new OutletAssignmentsDto());
    }
}
```
...
```ts
export const outletassignmentsQueryKey = (tenantId: string, outletId?: string) =>
  ["identity-access", "outlet-assignments", tenantId, outletId ?? "tenant"] as const;

export function useOutletAssignments(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: outletassignmentsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/identity-access/outlet-assignments"),
    staleTime: 60_000,
  });
}
```
