---
title: "Tenants Feature Specification"
...
## Purpose
This document defines the implementation scope for `tenants` inside the `tenant-management` module of the Unified Commerce platform.
...
|---|---|
| `tenants` | Source or supporting table for `tenants` behavior. |
| `outlets` | Source or supporting table for `tenants` behavior. |
| `outlet_addresses` | Source or supporting table for `tenants` behavior. |
| `document_sequences` | Source or supporting table for `tenants` behavior. |
| `tenant_settings` | Source or supporting table for `tenants` behavior. |
| `ui_themes` | Source or supporting table for `tenants` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `tenant-management.tenants.read` | View data and search records. | Yes |
| `tenant-management.tenants.create` | Create new records or workflow drafts. | Yes |
| `tenant-management.tenants.update` | Modify allowed editable fields. | Yes |
| `tenant-management.tenants.approve` | Approve sensitive actions where applicable. | Yes |
| `tenant-management.tenants.void` | Void/cancel/reverse where business rules allow. | Yes |
...
public sealed class TenantsService : ITenantsService
{
    ...
    public async Task<ApiResponse<TenantsDto>> HandleAsync(TenantsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "tenant-management.tenants.manage");
        ...
        return ApiResponse.Success(new TenantsDto());
    }
}
```
...
```ts
export const tenantsQueryKey = (tenantId: string, outletId?: string) =>
  ["tenant-management", "tenants", tenantId, outletId ?? "tenant"] as const;

export function useTenants(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: tenantsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/tenant-management/tenants"),
    staleTime: 60_000,
  });
}
```
