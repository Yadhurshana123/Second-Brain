---
title: "Document Sequences API Specification"
...
# Document Sequences API Specification

## API Ownership
The `document-sequences` API belongs to the `tenant-management` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/tenant-management/document-sequences` | `tenant-management.document-sequences.read` |
| Detail | GET | `/api/v1/tenant-management/document-sequences/{id}` | `tenant-management.document-sequences.read` |
| Create | POST | `/api/v1/tenant-management/document-sequences` | `tenant-management.document-sequences.create` |
| Update | PUT | `/api/v1/tenant-management/document-sequences/{id}` | `tenant-management.document-sequences.update` |
| State action | POST | `/api/v1/tenant-management/document-sequences/{id}/actions/{action}` | Action-specific permission |
...
- Access consideration 1: Permission `tenant-management.document-sequences.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `tenant-management.document-sequences.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `tenant-management.document-sequences.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `tenant-management.document-sequences.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `tenant-management.document-sequences.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `tenant-management.document-sequences.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `tenant-management.document-sequences.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `tenant-management.document-sequences.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `tenant-management.document-sequences.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `tenant-management.document-sequences.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `tenant-management.document-sequences.read` may show data, while command permissions must be separately configured per role.
