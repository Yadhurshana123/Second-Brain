---
title: "Outlets Feature Specification"
...
# Outlets Feature Specification

## Purpose
This document defines the implementation scope for `outlets` inside the `tenant-management` module of the Unified Commerce platform.
...
|---|---|
| `tenants` | Source or supporting table for `outlets` behavior. |
| `outlets` | Source or supporting table for `outlets` behavior. |
| `outlet_addresses` | Source or supporting table for `outlets` behavior. |
| `document_sequences` | Source or supporting table for `outlets` behavior. |
| `tenant_settings` | Source or supporting table for `outlets` behavior. |
| `ui_themes` | Source or supporting table for `outlets` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `tenant-management.outlets.read` | View data and search records. | Yes |
| `tenant-management.outlets.create` | Create new records or workflow drafts. | Yes |
| `tenant-management.outlets.update` | Modify allowed editable fields. | Yes |
| `tenant-management.outlets.approve` | Approve sensitive actions where applicable. | Yes |
| `tenant-management.outlets.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class OutletsService : IOutletsService
{
    private readonly IOutletsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<OutletsDto>> HandleAsync(OutletsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "tenant-management.outlets.manage");
        ...
        return ApiResponse.Success(new OutletsDto());
    }
}
```
...
```ts
export const outletsQueryKey = (tenantId: string, outletId?: string) =>
  ["tenant-management", "outlets", tenantId, outletId ?? "tenant"] as const;

export function useOutlets(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: outletsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/tenant-management/outlets"),
    staleTime: 60_000,
  });
}
```
