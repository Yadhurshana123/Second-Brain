---
title: "Outlets API Specification"
...
# Outlets API Specification

## API Ownership
The `outlets` API belongs to the `tenant-management` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/tenant-management/outlets` | `tenant-management.outlets.read` |
| Detail | GET | `/api/v1/tenant-management/outlets/{id}` | `tenant-management.outlets.read` |
| Create | POST | `/api/v1/tenant-management/outlets` | `tenant-management.outlets.create` |
| Update | PUT | `/api/v1/tenant-management/outlets/{id}` | `tenant-management.outlets.update` |
| State action | POST | `/api/v1/tenant-management/outlets/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/tenant-management/outlets
...
{
  "featureKey": "tenant-management.outlets",
...
- Access consideration 1: Permission `tenant-management.outlets.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `tenant-management.outlets.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `tenant-management.outlets.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `tenant-management.outlets.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `tenant-management.outlets.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `tenant-management.outlets.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `tenant-management.outlets.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `tenant-management.outlets.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `tenant-management.outlets.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `tenant-management.outlets.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `tenant-management.outlets.read` may show data, while command permissions must be separately configured per role.
