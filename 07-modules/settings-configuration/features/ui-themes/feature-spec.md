---
title: "Ui Themes Feature Specification"
...
# Ui Themes Feature Specification

## Purpose
This document defines the implementation scope for `ui-themes` inside the `settings-configuration` module of the Unified Commerce platform.
...
|---|---|
| `feature_flags` | Source or supporting table for `ui-themes` behavior. |
| `tenant_settings` | Source or supporting table for `ui-themes` behavior. |
| `ui_themes` | Source or supporting table for `ui-themes` behavior. |
| `tenant_feature_entitlements` | Source or supporting table for `ui-themes` behavior. |
| `role_feature_assignments` | Source or supporting table for `ui-themes` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `settings-configuration.ui-themes.read` | View data and search records. | Yes |
| `settings-configuration.ui-themes.create` | Create new records or workflow drafts. | Yes |
| `settings-configuration.ui-themes.update` | Modify allowed editable fields. | Yes |
| `settings-configuration.ui-themes.approve` | Approve sensitive actions where applicable. | Yes |
| `settings-configuration.ui-themes.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class UiThemesService : IUiThemesService
{
    private readonly IUiThemesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<UiThemesDto>> HandleAsync(UiThemesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "settings-configuration.ui-themes.manage");
        ...
        return ApiResponse.Success(new UiThemesDto());
    }
}
```
...
```ts
export const uithemesQueryKey = (tenantId: string, outletId?: string) =>
  ["settings-configuration", "ui-themes", tenantId, outletId ?? "tenant"] as const;

export function useUiThemes(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: uithemesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/settings-configuration/ui-themes"),
    staleTime: 60_000,
  });
}
```
