---
title: "Ui Themes API Specification"
...
# Ui Themes API Specification

## API Ownership
The `ui-themes` API belongs to the `settings-configuration` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/settings-configuration/ui-themes` | `settings-configuration.ui-themes.read` |
| Detail | GET | `/api/v1/settings-configuration/ui-themes/{id}` | `settings-configuration.ui-themes.read` |
| Create | POST | `/api/v1/settings-configuration/ui-themes` | `settings-configuration.ui-themes.create` |
| Update | PUT | `/api/v1/settings-configuration/ui-themes/{id}` | `settings-configuration.ui-themes.update` |
| State action | POST | `/api/v1/settings-configuration/ui-themes/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/settings-configuration/ui-themes
...
{
  "featureKey": "settings-configuration.ui-themes",
...
- Access consideration 1: Permission `settings-configuration.ui-themes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `settings-configuration.ui-themes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `settings-configuration.ui-themes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `settings-configuration.ui-themes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `settings-configuration.ui-themes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `settings-configuration.ui-themes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `settings-configuration.ui-themes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `settings-configuration.ui-themes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `settings-configuration.ui-themes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `settings-configuration.ui-themes.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `settings-configuration.ui-themes.read` may show data, while command permissions must be separately configured per role.
