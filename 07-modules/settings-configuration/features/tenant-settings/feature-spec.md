---
title: "Tenant Settings Feature Specification"
...
# Tenant Settings Feature Specification

## Purpose
This document defines the implementation scope for `tenant-settings` inside the `settings-configuration` module of the Unified Commerce platform.
...
|---|---|
| `feature_flags` | Source or supporting table for `tenant-settings` behavior. |
| `tenant_settings` | Source or supporting table for `tenant-settings` behavior. |
| `ui_themes` | Source or supporting table for `tenant-settings` behavior. |
| `tenant_feature_entitlements` | Source or supporting table for `tenant-settings` behavior. |
| `role_feature_assignments` | Source or supporting table for `tenant-settings` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `settings-configuration.tenant-settings.read` | View data and search records. | Yes |
| `settings-configuration.tenant-settings.create` | Create new records or workflow drafts. | Yes |
| `settings-configuration.tenant-settings.update` | Modify allowed editable fields. | Yes |
| `settings-configuration.tenant-settings.approve` | Approve sensitive actions where applicable. | Yes |
| `settings-configuration.tenant-settings.void` | Void/cancel/reverse where business rules allow. | Yes |
...
public sealed class TenantSettingsService : ITenantSettingsService
{
    ...
    public async Task<ApiResponse<TenantSettingsDto>> HandleAsync(TenantSettingsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "settings-configuration.tenant-settings.manage");
        ...
        return ApiResponse.Success(new TenantSettingsDto());
    }
}
```
...
```ts
export const tenantsettingsQueryKey = (tenantId: string, outletId?: string) =>
  ["settings-configuration", "tenant-settings", tenantId, outletId ?? "tenant"] as const;

export function useTenantSettings(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: tenantsettingsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/settings-configuration/tenant-settings"),
    staleTime: 60_000,
  });
}
```
