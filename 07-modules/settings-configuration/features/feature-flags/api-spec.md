---
title: "Feature Flags API Specification"
...
## API Ownership
The `feature-flags` API belongs to the `settings-configuration` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/settings-configuration/feature-flags` | `settings-configuration.feature-flags.read` |
| Detail | GET | `/api/v1/settings-configuration/feature-flags/{id}` | `settings-configuration.feature-flags.read` |
| Create | POST | `/api/v1/settings-configuration/feature-flags` | `settings-configuration.feature-flags.create` |
| Update | PUT | `/api/v1/settings-configuration/feature-flags/{id}` | `settings-configuration.feature-flags.update` |
| State action | POST | `/api/v1/settings-configuration/feature-flags/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/settings-configuration/feature-flags
...
{
  "featureKey": "settings-configuration.feature-flags",
...
- Access consideration 1: Permission `settings-configuration.feature-flags.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `settings-configuration.feature-flags.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `settings-configuration.feature-flags.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `settings-configuration.feature-flags.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `settings-configuration.feature-flags.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `settings-configuration.feature-flags.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `settings-configuration.feature-flags.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `settings-configuration.feature-flags.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `settings-configuration.feature-flags.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `settings-configuration.feature-flags.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `settings-configuration.feature-flags.read` may show data, while command permissions must be separately configured per role.
