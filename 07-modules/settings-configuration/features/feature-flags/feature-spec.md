---
title: "Feature Flags Feature Specification"
...
# Feature Flags Feature Specification

## Purpose
This document defines the implementation scope for `feature-flags` inside the `settings-configuration` module of the Unified Commerce platform.
...
|---|---|
| `feature_flags` | Source or supporting table for `feature-flags` behavior. |
| `tenant_settings` | Source or supporting table for `feature-flags` behavior. |
| `ui_themes` | Source or supporting table for `feature-flags` behavior. |
| `tenant_feature_entitlements` | Source or supporting table for `feature-flags` behavior. |
| `role_feature_assignments` | Source or supporting table for `feature-flags` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `settings-configuration.feature-flags.read` | View data and search records. | Yes |
| `settings-configuration.feature-flags.create` | Create new records or workflow drafts. | Yes |
| `settings-configuration.feature-flags.update` | Modify allowed editable fields. | Yes |
| `settings-configuration.feature-flags.approve` | Approve sensitive actions where applicable. | Yes |
| `settings-configuration.feature-flags.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class FeatureFlagsService : IFeatureFlagsService
{
    private readonly IFeatureFlagsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<FeatureFlagsDto>> HandleAsync(FeatureFlagsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "settings-configuration.feature-flags.manage");
        ...
        return ApiResponse.Success(new FeatureFlagsDto());
    }
}
```
...
```ts
export const featureflagsQueryKey = (tenantId: string, outletId?: string) =>
  ["settings-configuration", "feature-flags", tenantId, outletId ?? "tenant"] as const;

export function useFeatureFlags(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: featureflagsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/settings-configuration/feature-flags"),
    staleTime: 60_000,
  });
}
```
...
- Related module overview: [[../README|Settings Configuration Overview]]
