---
title: "Receipt Print Logs Feature Specification"
...
# Receipt Print Logs Feature Specification

## Purpose
This document defines the implementation scope for `receipt-print-logs` inside the `receipts` module of the Unified Commerce platform.
...
|---|---|
| `receipt_templates` | Source or supporting table for `receipt-print-logs` behavior. |
| `receipts` | Source or supporting table for `receipt-print-logs` behavior. |
| `receipt_print_logs` | Source or supporting table for `receipt-print-logs` behavior. |
| `document_sequences` | Source or supporting table for `receipt-print-logs` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `receipts.receipt-print-logs.read` | View data and search records. | Yes |
| `receipts.receipt-print-logs.create` | Create new records or workflow drafts. | Yes |
| `receipts.receipt-print-logs.update` | Modify allowed editable fields. | Yes |
| `receipts.receipt-print-logs.approve` | Approve sensitive actions where applicable. | Yes |
| `receipts.receipt-print-logs.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class ReceiptPrintLogsService : IReceiptPrintLogsService
{
    private readonly IReceiptPrintLogsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<ReceiptPrintLogsDto>> HandleAsync(ReceiptPrintLogsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "receipts.receipt-print-logs.manage");
        ...
        return ApiResponse.Success(new ReceiptPrintLogsDto());
    }
}
```
...
```ts
export const receiptprintlogsQueryKey = (tenantId: string, outletId?: string) =>
  ["receipts", "receipt-print-logs", tenantId, outletId ?? "tenant"] as const;

export function useReceiptPrintLogs(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: receiptprintlogsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/receipts/receipt-print-logs"),
    staleTime: 60_000,
  });
}
```
