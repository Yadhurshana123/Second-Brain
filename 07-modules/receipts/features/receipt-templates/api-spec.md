---
title: "Receipt Templates API Specification"
...
# Receipt Templates API Specification

## API Ownership
The `receipt-templates` API belongs to the `receipts` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/receipts/receipt-templates` | `receipts.receipt-templates.read` |
| Detail | GET | `/api/v1/receipts/receipt-templates/{id}` | `receipts.receipt-templates.read` |
| Create | POST | `/api/v1/receipts/receipt-templates` | `receipts.receipt-templates.create` |
| Update | PUT | `/api/v1/receipts/receipt-templates/{id}` | `receipts.receipt-templates.update` |
| State action | POST | `/api/v1/receipts/receipt-templates/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/receipts/receipt-templates
...
{
  "featureKey": "receipts.receipt-templates",
...
- Access consideration 1: Permission `receipts.receipt-templates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `receipts.receipt-templates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `receipts.receipt-templates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `receipts.receipt-templates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `receipts.receipt-templates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `receipts.receipt-templates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `receipts.receipt-templates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `receipts.receipt-templates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `receipts.receipt-templates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `receipts.receipt-templates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `receipts.receipt-templates.read` may show data, while command permissions must be separately configured per role.
