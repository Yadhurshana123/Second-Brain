---
title: "Receipt Templates Feature Specification"
...
## Purpose
This document defines the implementation scope for `receipt-templates` inside the `receipts` module of the Unified Commerce platform.
...
|---|---|
| `receipt_templates` | Source or supporting table for `receipt-templates` behavior. |
| `receipts` | Source or supporting table for `receipt-templates` behavior. |
| `receipt_print_logs` | Source or supporting table for `receipt-templates` behavior. |
| `document_sequences` | Source or supporting table for `receipt-templates` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `receipts.receipt-templates.read` | View data and search records. | Yes |
| `receipts.receipt-templates.create` | Create new records or workflow drafts. | Yes |
| `receipts.receipt-templates.update` | Modify allowed editable fields. | Yes |
| `receipts.receipt-templates.approve` | Approve sensitive actions where applicable. | Yes |
| `receipts.receipt-templates.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class ReceiptTemplatesService : IReceiptTemplatesService
{
    private readonly IReceiptTemplatesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<ReceiptTemplatesDto>> HandleAsync(ReceiptTemplatesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "receipts.receipt-templates.manage");
        ...
        return ApiResponse.Success(new ReceiptTemplatesDto());
    }
}
```
...
```ts
export const receipttemplatesQueryKey = (tenantId: string, outletId?: string) =>
  ["receipts", "receipt-templates", tenantId, outletId ?? "tenant"] as const;

export function useReceiptTemplates(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: receipttemplatesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/receipts/receipt-templates"),
    staleTime: 60_000,
  });
}
```
...
- Related module overview: [[../README|Receipts Overview]]
