---
title: "Receipts Feature Specification"
...
# Receipts Feature Specification

## Purpose
This document defines the implementation scope for `receipts` inside the `receipts` module of the Unified Commerce platform.
...
|---|---|
| `receipt_templates` | Source or supporting table for `receipts` behavior. |
| `receipts` | Source or supporting table for `receipts` behavior. |
| `receipt_print_logs` | Source or supporting table for `receipts` behavior. |
| `document_sequences` | Source or supporting table for `receipts` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `receipts.receipts.read` | View data and search records. | Yes |
| `receipts.receipts.create` | Create new records or workflow drafts. | Yes |
| `receipts.receipts.update` | Modify allowed editable fields. | Yes |
| `receipts.receipts.approve` | Approve sensitive actions where applicable. | Yes |
| `receipts.receipts.void` | Void/cancel/reverse where business rules allow. | Yes |
...
public sealed class ReceiptsService : IReceiptsService
{
    ...
    public async Task<ApiResponse<ReceiptsDto>> HandleAsync(ReceiptsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "receipts.receipts.manage");
        ...
        return ApiResponse.Success(new ReceiptsDto());
    }
}
```
...
```ts
export const receiptsQueryKey = (tenantId: string, outletId?: string) =>
  ["receipts", "receipts", tenantId, outletId ?? "tenant"] as const;

export function useReceipts(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: receiptsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/receipts/receipts"),
    staleTime: 60_000,
  });
}
```
