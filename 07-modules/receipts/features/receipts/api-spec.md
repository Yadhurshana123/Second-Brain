---
title: "Receipts API Specification"
...
# Receipts API Specification

## API Ownership
The `receipts` API belongs to the `receipts` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/receipts/receipts` | `receipts.receipts.read` |
| Detail | GET | `/api/v1/receipts/receipts/{id}` | `receipts.receipts.read` |
| Create | POST | `/api/v1/receipts/receipts` | `receipts.receipts.create` |
| Update | PUT | `/api/v1/receipts/receipts/{id}` | `receipts.receipts.update` |
| State action | POST | `/api/v1/receipts/receipts/{id}/actions/{action}` | Action-specific permission |
...
- Access consideration 1: Permission `receipts.receipts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `receipts.receipts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `receipts.receipts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `receipts.receipts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `receipts.receipts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `receipts.receipts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `receipts.receipts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `receipts.receipts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `receipts.receipts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `receipts.receipts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `receipts.receipts.read` may show data, while command permissions must be separately configured per role.
