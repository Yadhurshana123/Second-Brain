---
title: "Scanner Printer Configuration Feature Specification"
...
# Scanner Printer Configuration Feature Specification

## Purpose
This document defines the implementation scope for `scanner-printer-configuration` inside the `pos-devices-hardware` module of the Unified Commerce platform.
...
|---|---|
| `tills` | Source or supporting table for `scanner-printer-configuration` behavior. |
| `pos_devices` | Source or supporting table for `scanner-printer-configuration` behavior. |
| `receipt_templates` | Source or supporting table for `scanner-printer-configuration` behavior. |
| `receipt_print_logs` | Source or supporting table for `scanner-printer-configuration` behavior. |
| `offline_sync_batches` | Source or supporting table for `scanner-printer-configuration` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `pos-devices-hardware.scanner-printer-configuration.read` | View data and search records. | Yes |
| `pos-devices-hardware.scanner-printer-configuration.create` | Create new records or workflow drafts. | Yes |
| `pos-devices-hardware.scanner-printer-configuration.update` | Modify allowed editable fields. | Yes |
| `pos-devices-hardware.scanner-printer-configuration.approve` | Approve sensitive actions where applicable. | Yes |
| `pos-devices-hardware.scanner-printer-configuration.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class ScannerPrinterConfigurationService : IScannerPrinterConfigurationService
{
    private readonly IScannerPrinterConfigurationRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<ScannerPrinterConfigurationDto>> HandleAsync(ScannerPrinterConfigurationRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "pos-devices-hardware.scanner-printer-configuration.manage");
        ...
        return ApiResponse.Success(new ScannerPrinterConfigurationDto());
    }
}
```
...
```ts
export const scannerprinterconfigurationQueryKey = (tenantId: string, outletId?: string) =>
  ["pos-devices-hardware", "scanner-printer-configuration", tenantId, outletId ?? "tenant"] as const;

export function useScannerPrinterConfiguration(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: scannerprinterconfigurationQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/pos-devices-hardware/scanner-printer-configuration"),
    staleTime: 60_000,
  });
}
```
