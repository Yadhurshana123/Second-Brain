...
# Scanner Printer Configuration Feature History
...
## Related Documents
- Feature spec: [[feature-spec|Scanner Printer Configuration Feature Spec]]
- API spec: [[api-spec|Scanner Printer Configuration API Spec]]
...
- Implementation consideration 18: Keep `scanner-printer-configuration` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 19: Keep `scanner-printer-configuration` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 20: Keep `scanner-printer-configuration` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 21: Keep `scanner-printer-configuration` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 22: Keep `scanner-printer-configuration` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 23: Keep `scanner-printer-configuration` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 24: Keep `scanner-printer-configuration` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 25: Keep `scanner-printer-configuration` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 26: Keep `scanner-printer-configuration` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 27: Keep `scanner-printer-configuration` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 28: Keep `scanner-printer-configuration` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 29: Keep `scanner-printer-configuration` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Access consideration 30: Permission `pos-devices-hardware.scanner-printer-configuration.read` may show data, while command permissions must be separately configured per role.
- Access consideration 31: Permission `pos-devices-hardware.scanner-printer-configuration.read` may show data, while command permissions must be separately configured per role.
- Access consideration 32: Permission `pos-devices-hardware.scanner-printer-configuration.read` may show data, while command permissions must be separately configured per role.
- Access consideration 33: Permission `pos-devices-hardware.scanner-printer-configuration.read` may show data, while command permissions must be separately configured per role.
- Access consideration 34: Permission `pos-devices-hardware.scanner-printer-configuration.read` may show data, while command permissions must be separately configured per role.
- Access consideration 35: Permission `pos-devices-hardware.scanner-printer-configuration.read` may show data, while command permissions must be separately configured per role.
- Access consideration 36: Permission `pos-devices-hardware.scanner-printer-configuration.read` may show data, while command permissions must be separately configured per role.
- Access consideration 37: Permission `pos-devices-hardware.scanner-printer-configuration.read` may show data, while command permissions must be separately configured per role.
- Access consideration 38: Permission `pos-devices-hardware.scanner-printer-configuration.read` may show data, while command permissions must be separately configured per role.
- Access consideration 39: Permission `pos-devices-hardware.scanner-printer-configuration.read` may show data, while command permissions must be separately configured per role.
- Access consideration 40: Permission `pos-devices-hardware.scanner-printer-configuration.read` may show data, while command permissions must be separately configured per role.
- Access consideration 41: Permission `pos-devices-hardware.scanner-printer-configuration.read` may show data, while command permissions must be separately configured per role.
