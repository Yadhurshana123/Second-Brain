---
title: "Terminal Outlet Assignment Feature History"
...
# Terminal Outlet Assignment Feature History
...
## Related Documents
- Feature spec: [[feature-spec|Terminal Outlet Assignment Feature Spec]]
- API spec: [[api-spec|Terminal Outlet Assignment API Spec]]
...
- Implementation consideration 18: Keep `terminal-outlet-assignment` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 19: Keep `terminal-outlet-assignment` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 20: Keep `terminal-outlet-assignment` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 21: Keep `terminal-outlet-assignment` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 22: Keep `terminal-outlet-assignment` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 23: Keep `terminal-outlet-assignment` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 24: Keep `terminal-outlet-assignment` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 25: Keep `terminal-outlet-assignment` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 26: Keep `terminal-outlet-assignment` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 27: Keep `terminal-outlet-assignment` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 28: Keep `terminal-outlet-assignment` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Implementation consideration 29: Keep `terminal-outlet-assignment` tenant-scoped and never resolve records without `tenant_id` or a tenant-owned parent reference.
- Access consideration 30: Permission `pos-devices-hardware.terminal-outlet-assignment.read` may show data, while command permissions must be separately configured per role.
- Access consideration 31: Permission `pos-devices-hardware.terminal-outlet-assignment.read` may show data, while command permissions must be separately configured per role.
- Access consideration 32: Permission `pos-devices-hardware.terminal-outlet-assignment.read` may show data, while command permissions must be separately configured per role.
- Access consideration 33: Permission `pos-devices-hardware.terminal-outlet-assignment.read` may show data, while command permissions must be separately configured per role.
- Access consideration 34: Permission `pos-devices-hardware.terminal-outlet-assignment.read` may show data, while command permissions must be separately configured per role.
- Access consideration 35: Permission `pos-devices-hardware.terminal-outlet-assignment.read` may show data, while command permissions must be separately configured per role.
- Access consideration 36: Permission `pos-devices-hardware.terminal-outlet-assignment.read` may show data, while command permissions must be separately configured per role.
- Access consideration 37: Permission `pos-devices-hardware.terminal-outlet-assignment.read` may show data, while command permissions must be separately configured per role.
- Access consideration 38: Permission `pos-devices-hardware.terminal-outlet-assignment.read` may show data, while command permissions must be separately configured per role.
- Access consideration 39: Permission `pos-devices-hardware.terminal-outlet-assignment.read` may show data, while command permissions must be separately configured per role.
- Access consideration 40: Permission `pos-devices-hardware.terminal-outlet-assignment.read` may show data, while command permissions must be separately configured per role.
- Access consideration 41: Permission `pos-devices-hardware.terminal-outlet-assignment.read` may show data, while command permissions must be separately configured per role.
