---
title: "Pos Device Registration API Specification"
...
# Pos Device Registration API Specification

## API Ownership
The `pos-device-registration` API belongs to the `pos-devices-hardware` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/pos-devices-hardware/pos-device-registration` | `pos-devices-hardware.pos-device-registration.read` |
| Detail | GET | `/api/v1/pos-devices-hardware/pos-device-registration/{id}` | `pos-devices-hardware.pos-device-registration.read` |
| Create | POST | `/api/v1/pos-devices-hardware/pos-device-registration` | `pos-devices-hardware.pos-device-registration.create` |
| Update | PUT | `/api/v1/pos-devices-hardware/pos-device-registration/{id}` | `pos-devices-hardware.pos-device-registration.update` |
| State action | POST | `/api/v1/pos-devices-hardware/pos-device-registration/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/pos-devices-hardware/pos-device-registration
...
{
  "featureKey": "pos-devices-hardware.pos-device-registration",
...
- Access consideration 1: Permission `pos-devices-hardware.pos-device-registration.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `pos-devices-hardware.pos-device-registration.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `pos-devices-hardware.pos-device-registration.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `pos-devices-hardware.pos-device-registration.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `pos-devices-hardware.pos-device-registration.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `pos-devices-hardware.pos-device-registration.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `pos-devices-hardware.pos-device-registration.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `pos-devices-hardware.pos-device-registration.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `pos-devices-hardware.pos-device-registration.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `pos-devices-hardware.pos-device-registration.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `pos-devices-hardware.pos-device-registration.read` may show data, while command permissions must be separately configured per role.
