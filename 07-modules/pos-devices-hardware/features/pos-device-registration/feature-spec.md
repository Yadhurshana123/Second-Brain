---
title: "Pos Device Registration Feature Specification"
...
# Pos Device Registration Feature Specification

## Purpose
This document defines the implementation scope for `pos-device-registration` inside the `pos-devices-hardware` module of the Unified Commerce platform.
...
|---|---|
| `tills` | Source or supporting table for `pos-device-registration` behavior. |
| `pos_devices` | Source or supporting table for `pos-device-registration` behavior. |
| `receipt_templates` | Source or supporting table for `pos-device-registration` behavior. |
| `receipt_print_logs` | Source or supporting table for `pos-device-registration` behavior. |
| `offline_sync_batches` | Source or supporting table for `pos-device-registration` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `pos-devices-hardware.pos-device-registration.read` | View data and search records. | Yes |
| `pos-devices-hardware.pos-device-registration.create` | Create new records or workflow drafts. | Yes |
| `pos-devices-hardware.pos-device-registration.update` | Modify allowed editable fields. | Yes |
| `pos-devices-hardware.pos-device-registration.approve` | Approve sensitive actions where applicable. | Yes |
| `pos-devices-hardware.pos-device-registration.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class PosDeviceRegistrationService : IPosDeviceRegistrationService
{
    private readonly IPosDeviceRegistrationRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<PosDeviceRegistrationDto>> HandleAsync(PosDeviceRegistrationRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "pos-devices-hardware.pos-device-registration.manage");
        ...
        return ApiResponse.Success(new PosDeviceRegistrationDto());
    }
}
```
...
```ts
export const posdeviceregistrationQueryKey = (tenantId: string, outletId?: string) =>
  ["pos-devices-hardware", "pos-device-registration", tenantId, outletId ?? "tenant"] as const;

export function usePosDeviceRegistration(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: posdeviceregistrationQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/pos-devices-hardware/pos-device-registration"),
    staleTime: 60_000,
  });
}
```
