---
title: "Tills Feature Specification"
...
# Tills Feature Specification

## Purpose
This document defines the implementation scope for `tills` inside the `pos-devices-hardware` module of the Unified Commerce platform.
...
|---|---|
| `tills` | Source or supporting table for `tills` behavior. |
| `pos_devices` | Source or supporting table for `tills` behavior. |
| `receipt_templates` | Source or supporting table for `tills` behavior. |
| `receipt_print_logs` | Source or supporting table for `tills` behavior. |
| `offline_sync_batches` | Source or supporting table for `tills` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `pos-devices-hardware.tills.read` | View data and search records. | Yes |
| `pos-devices-hardware.tills.create` | Create new records or workflow drafts. | Yes |
| `pos-devices-hardware.tills.update` | Modify allowed editable fields. | Yes |
| `pos-devices-hardware.tills.approve` | Approve sensitive actions where applicable. | Yes |
| `pos-devices-hardware.tills.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class TillsService : ITillsService
{
    private readonly ITillsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<TillsDto>> HandleAsync(TillsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "pos-devices-hardware.tills.manage");
        ...
        return ApiResponse.Success(new TillsDto());
    }
}
```
...
```ts
export const tillsQueryKey = (tenantId: string, outletId?: string) =>
  ["pos-devices-hardware", "tills", tenantId, outletId ?? "tenant"] as const;

export function useTills(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: tillsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/pos-devices-hardware/tills"),
    staleTime: 60_000,
  });
}
```
