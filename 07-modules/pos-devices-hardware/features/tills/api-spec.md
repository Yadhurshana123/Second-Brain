...
# Tills API Specification

## API Ownership
The `tills` API belongs to the `pos-devices-hardware` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/pos-devices-hardware/tills` | `pos-devices-hardware.tills.read` |
| Detail | GET | `/api/v1/pos-devices-hardware/tills/{id}` | `pos-devices-hardware.tills.read` |
| Create | POST | `/api/v1/pos-devices-hardware/tills` | `pos-devices-hardware.tills.create` |
| Update | PUT | `/api/v1/pos-devices-hardware/tills/{id}` | `pos-devices-hardware.tills.update` |
| State action | POST | `/api/v1/pos-devices-hardware/tills/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/pos-devices-hardware/tills
...
{
  "featureKey": "pos-devices-hardware.tills",
...
- Access consideration 1: Permission `pos-devices-hardware.tills.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `pos-devices-hardware.tills.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `pos-devices-hardware.tills.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `pos-devices-hardware.tills.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `pos-devices-hardware.tills.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `pos-devices-hardware.tills.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `pos-devices-hardware.tills.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `pos-devices-hardware.tills.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `pos-devices-hardware.tills.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `pos-devices-hardware.tills.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `pos-devices-hardware.tills.read` may show data, while command permissions must be separately configured per role.
