---
title: "Tax Rates Feature Specification"
...
# Tax Rates Feature Specification

## Purpose
This document defines the implementation scope for `tax-rates` inside the `tax` module of the Unified Commerce platform.
...
|---|---|
| `tax_classes` | Source or supporting table for `tax-rates` behavior. |
| `tax_rates` | Source or supporting table for `tax-rates` behavior. |
| `tax_class_rates` | Source or supporting table for `tax-rates` behavior. |
| `products` | Source or supporting table for `tax-rates` behavior. |
| `sale_lines` | Source or supporting table for `tax-rates` behavior. |
| `order_items` | Source or supporting table for `tax-rates` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `tax.tax-rates.read` | View data and search records. | Yes |
| `tax.tax-rates.create` | Create new records or workflow drafts. | Yes |
| `tax.tax-rates.update` | Modify allowed editable fields. | Yes |
| `tax.tax-rates.approve` | Approve sensitive actions where applicable. | Yes |
| `tax.tax-rates.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class TaxRatesService : ITaxRatesService
{
    private readonly ITaxRatesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<TaxRatesDto>> HandleAsync(TaxRatesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "tax.tax-rates.manage");
        ...
        return ApiResponse.Success(new TaxRatesDto());
    }
}
```
...
```ts
export const taxratesQueryKey = (tenantId: string, outletId?: string) =>
  ["tax", "tax-rates", tenantId, outletId ?? "tenant"] as const;

export function useTaxRates(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: taxratesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/tax/tax-rates"),
    staleTime: 60_000,
  });
}
```
