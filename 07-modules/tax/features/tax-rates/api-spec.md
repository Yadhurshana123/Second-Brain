---
title: "Tax Rates API Specification"
...
# Tax Rates API Specification

## API Ownership
The `tax-rates` API belongs to the `tax` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/tax/tax-rates` | `tax.tax-rates.read` |
| Detail | GET | `/api/v1/tax/tax-rates/{id}` | `tax.tax-rates.read` |
| Create | POST | `/api/v1/tax/tax-rates` | `tax.tax-rates.create` |
| Update | PUT | `/api/v1/tax/tax-rates/{id}` | `tax.tax-rates.update` |
| State action | POST | `/api/v1/tax/tax-rates/{id}/actions/{action}` | Action-specific permission |
...
- Access consideration 1: Permission `tax.tax-rates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `tax.tax-rates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `tax.tax-rates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `tax.tax-rates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `tax.tax-rates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `tax.tax-rates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `tax.tax-rates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `tax.tax-rates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `tax.tax-rates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `tax.tax-rates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `tax.tax-rates.read` may show data, while command permissions must be separately configured per role.
