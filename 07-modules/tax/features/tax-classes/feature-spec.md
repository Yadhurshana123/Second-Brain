---
title: "Tax Classes Feature Specification"
...
# Tax Classes Feature Specification

## Purpose
This document defines the implementation scope for `tax-classes` inside the `tax` module of the Unified Commerce platform.
...
|---|---|
| `tax_classes` | Source or supporting table for `tax-classes` behavior. |
| `tax_rates` | Source or supporting table for `tax-classes` behavior. |
| `tax_class_rates` | Source or supporting table for `tax-classes` behavior. |
| `products` | Source or supporting table for `tax-classes` behavior. |
| `sale_lines` | Source or supporting table for `tax-classes` behavior. |
| `order_items` | Source or supporting table for `tax-classes` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `tax.tax-classes.read` | View data and search records. | Yes |
| `tax.tax-classes.create` | Create new records or workflow drafts. | Yes |
| `tax.tax-classes.update` | Modify allowed editable fields. | Yes |
| `tax.tax-classes.approve` | Approve sensitive actions where applicable. | Yes |
| `tax.tax-classes.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class TaxClassesService : ITaxClassesService
{
    private readonly ITaxClassesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<TaxClassesDto>> HandleAsync(TaxClassesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "tax.tax-classes.manage");
        ...
        return ApiResponse.Success(new TaxClassesDto());
    }
}
```
...
```ts
export const taxclassesQueryKey = (tenantId: string, outletId?: string) =>
  ["tax", "tax-classes", tenantId, outletId ?? "tenant"] as const;

export function useTaxClasses(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: taxclassesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/tax/tax-classes"),
    staleTime: 60_000,
  });
}
```
