---
title: "Tax Class Rates Feature Specification"
...
# Tax Class Rates Feature Specification

## Purpose
This document defines the implementation scope for `tax-class-rates` inside the `tax` module of the Unified Commerce platform.
...
|---|---|
| `tax_classes` | Source or supporting table for `tax-class-rates` behavior. |
| `tax_rates` | Source or supporting table for `tax-class-rates` behavior. |
| `tax_class_rates` | Source or supporting table for `tax-class-rates` behavior. |
| `products` | Source or supporting table for `tax-class-rates` behavior. |
| `sale_lines` | Source or supporting table for `tax-class-rates` behavior. |
| `order_items` | Source or supporting table for `tax-class-rates` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `tax.tax-class-rates.read` | View data and search records. | Yes |
| `tax.tax-class-rates.create` | Create new records or workflow drafts. | Yes |
| `tax.tax-class-rates.update` | Modify allowed editable fields. | Yes |
| `tax.tax-class-rates.approve` | Approve sensitive actions where applicable. | Yes |
| `tax.tax-class-rates.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class TaxClassRatesService : ITaxClassRatesService
{
    private readonly ITaxClassRatesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<TaxClassRatesDto>> HandleAsync(TaxClassRatesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "tax.tax-class-rates.manage");
        ...
        return ApiResponse.Success(new TaxClassRatesDto());
    }
}
```
...
```ts
export const taxclassratesQueryKey = (tenantId: string, outletId?: string) =>
  ["tax", "tax-class-rates", tenantId, outletId ?? "tenant"] as const;

export function useTaxClassRates(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: taxclassratesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/tax/tax-class-rates"),
    staleTime: 60_000,
  });
}
```
...
- Related module overview: [[../README|Tax Overview]]
