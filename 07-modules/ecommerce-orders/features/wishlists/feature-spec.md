---
title: "Wishlists Feature Specification"
...
# Wishlists Feature Specification

## Purpose
This document defines the implementation scope for `wishlists` inside the `ecommerce-orders` module of the Unified Commerce platform.
...
|---|---|
| `customers` | Source or supporting table for `wishlists` behavior. |
| `carts` | Source or supporting table for `wishlists` behavior. |
| `cart_items` | Source or supporting table for `wishlists` behavior. |
| `orders` | Source or supporting table for `wishlists` behavior. |
| `order_items` | Source or supporting table for `wishlists` behavior. |
| `order_addresses` | Source or supporting table for `wishlists` behavior. |
| `order_status_history` | Source or supporting table for `wishlists` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `ecommerce-orders.wishlists.read` | View data and search records. | Yes |
| `ecommerce-orders.wishlists.create` | Create new records or workflow drafts. | Yes |
| `ecommerce-orders.wishlists.update` | Modify allowed editable fields. | Yes |
| `ecommerce-orders.wishlists.approve` | Approve sensitive actions where applicable. | Yes |
| `ecommerce-orders.wishlists.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class WishlistsService : IWishlistsService
{
    private readonly IWishlistsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<WishlistsDto>> HandleAsync(WishlistsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "ecommerce-orders.wishlists.manage");
        ...
        return ApiResponse.Success(new WishlistsDto());
    }
}
```
...
```ts
export const wishlistsQueryKey = (tenantId: string, outletId?: string) =>
  ["ecommerce-orders", "wishlists", tenantId, outletId ?? "tenant"] as const;

export function useWishlists(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: wishlistsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/ecommerce-orders/wishlists"),
    staleTime: 60_000,
  });
}
```
