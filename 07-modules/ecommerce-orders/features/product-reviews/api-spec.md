---
title: "Product Reviews API Specification"
...
# Product Reviews API Specification

## API Ownership
The `product-reviews` API belongs to the `ecommerce-orders` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/ecommerce-orders/product-reviews` | `ecommerce-orders.product-reviews.read` |
| Detail | GET | `/api/v1/ecommerce-orders/product-reviews/{id}` | `ecommerce-orders.product-reviews.read` |
| Create | POST | `/api/v1/ecommerce-orders/product-reviews` | `ecommerce-orders.product-reviews.create` |
| Update | PUT | `/api/v1/ecommerce-orders/product-reviews/{id}` | `ecommerce-orders.product-reviews.update` |
| State action | POST | `/api/v1/ecommerce-orders/product-reviews/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/ecommerce-orders/product-reviews
...
{
  "featureKey": "ecommerce-orders.product-reviews",
...
- Access consideration 1: Permission `ecommerce-orders.product-reviews.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `ecommerce-orders.product-reviews.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `ecommerce-orders.product-reviews.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `ecommerce-orders.product-reviews.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `ecommerce-orders.product-reviews.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `ecommerce-orders.product-reviews.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `ecommerce-orders.product-reviews.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `ecommerce-orders.product-reviews.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `ecommerce-orders.product-reviews.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `ecommerce-orders.product-reviews.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `ecommerce-orders.product-reviews.read` may show data, while command permissions must be separately configured per role.
