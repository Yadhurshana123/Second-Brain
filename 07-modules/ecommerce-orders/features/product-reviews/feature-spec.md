---
title: "Product Reviews Feature Specification"
...
## Purpose
This document defines the implementation scope for `product-reviews` inside the `ecommerce-orders` module of the Unified Commerce platform.
...
|---|---|
| `customers` | Source or supporting table for `product-reviews` behavior. |
| `carts` | Source or supporting table for `product-reviews` behavior. |
| `cart_items` | Source or supporting table for `product-reviews` behavior. |
| `orders` | Source or supporting table for `product-reviews` behavior. |
| `order_items` | Source or supporting table for `product-reviews` behavior. |
| `order_addresses` | Source or supporting table for `product-reviews` behavior. |
| `order_status_history` | Source or supporting table for `product-reviews` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `ecommerce-orders.product-reviews.read` | View data and search records. | Yes |
| `ecommerce-orders.product-reviews.create` | Create new records or workflow drafts. | Yes |
| `ecommerce-orders.product-reviews.update` | Modify allowed editable fields. | Yes |
| `ecommerce-orders.product-reviews.approve` | Approve sensitive actions where applicable. | Yes |
| `ecommerce-orders.product-reviews.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class ProductReviewsService : IProductReviewsService
{
    private readonly IProductReviewsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<ProductReviewsDto>> HandleAsync(ProductReviewsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "ecommerce-orders.product-reviews.manage");
        ...
        return ApiResponse.Success(new ProductReviewsDto());
    }
}
```
...
```ts
export const productreviewsQueryKey = (tenantId: string, outletId?: string) =>
  ["ecommerce-orders", "product-reviews", tenantId, outletId ?? "tenant"] as const;

export function useProductReviews(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: productreviewsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/ecommerce-orders/product-reviews"),
    staleTime: 60_000,
  });
}
```
