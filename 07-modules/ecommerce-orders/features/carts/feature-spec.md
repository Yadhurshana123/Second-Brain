---
title: "Carts Feature Specification"
...
# Carts Feature Specification

## Purpose
This document defines the implementation scope for `carts` inside the `ecommerce-orders` module of the Unified Commerce platform.
...
|---|---|
| `customers` | Source or supporting table for `carts` behavior. |
| `carts` | Source or supporting table for `carts` behavior. |
| `cart_items` | Source or supporting table for `carts` behavior. |
| `orders` | Source or supporting table for `carts` behavior. |
| `order_items` | Source or supporting table for `carts` behavior. |
| `order_addresses` | Source or supporting table for `carts` behavior. |
| `order_status_history` | Source or supporting table for `carts` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `ecommerce-orders.carts.read` | View data and search records. | Yes |
| `ecommerce-orders.carts.create` | Create new records or workflow drafts. | Yes |
| `ecommerce-orders.carts.update` | Modify allowed editable fields. | Yes |
| `ecommerce-orders.carts.approve` | Approve sensitive actions where applicable. | Yes |
| `ecommerce-orders.carts.void` | Void/cancel/reverse where business rules allow. | Yes |
...
public sealed class CartsService : ICartsService
{
    ...
    public async Task<ApiResponse<CartsDto>> HandleAsync(CartsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "ecommerce-orders.carts.manage");
        ...
        return ApiResponse.Success(new CartsDto());
    }
}
```
...
```ts
export const cartsQueryKey = (tenantId: string, outletId?: string) =>
  ["ecommerce-orders", "carts", tenantId, outletId ?? "tenant"] as const;

export function useCarts(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: cartsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/ecommerce-orders/carts"),
    staleTime: 60_000,
  });
}
```
