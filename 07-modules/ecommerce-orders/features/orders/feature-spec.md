---
title: "Orders Feature Specification"
...
## Purpose
This document defines the implementation scope for `orders` inside the `ecommerce-orders` module of the Unified Commerce platform.
...
|---|---|
| `customers` | Source or supporting table for `orders` behavior. |
| `carts` | Source or supporting table for `orders` behavior. |
| `cart_items` | Source or supporting table for `orders` behavior. |
| `orders` | Source or supporting table for `orders` behavior. |
| `order_items` | Source or supporting table for `orders` behavior. |
| `order_addresses` | Source or supporting table for `orders` behavior. |
| `order_status_history` | Source or supporting table for `orders` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `ecommerce-orders.orders.read` | View data and search records. | Yes |
| `ecommerce-orders.orders.create` | Create new records or workflow drafts. | Yes |
| `ecommerce-orders.orders.update` | Modify allowed editable fields. | Yes |
| `ecommerce-orders.orders.approve` | Approve sensitive actions where applicable. | Yes |
| `ecommerce-orders.orders.void` | Void/cancel/reverse where business rules allow. | Yes |
...
public sealed class OrdersService : IOrdersService
{
    ...
    public async Task<ApiResponse<OrdersDto>> HandleAsync(OrdersRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "ecommerce-orders.orders.manage");
        ...
        return ApiResponse.Success(new OrdersDto());
    }
}
```
...
```ts
export const ordersQueryKey = (tenantId: string, outletId?: string) =>
  ["ecommerce-orders", "orders", tenantId, outletId ?? "tenant"] as const;

export function useOrders(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: ordersQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/ecommerce-orders/orders"),
    staleTime: 60_000,
  });
}
```
