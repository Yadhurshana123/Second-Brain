---
title: "Order Addresses API Specification"
...
# Order Addresses API Specification

## API Ownership
The `order-addresses` API belongs to the `ecommerce-orders` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/ecommerce-orders/order-addresses` | `ecommerce-orders.order-addresses.read` |
| Detail | GET | `/api/v1/ecommerce-orders/order-addresses/{id}` | `ecommerce-orders.order-addresses.read` |
| Create | POST | `/api/v1/ecommerce-orders/order-addresses` | `ecommerce-orders.order-addresses.create` |
| Update | PUT | `/api/v1/ecommerce-orders/order-addresses/{id}` | `ecommerce-orders.order-addresses.update` |
| State action | POST | `/api/v1/ecommerce-orders/order-addresses/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/ecommerce-orders/order-addresses
...
{
  "featureKey": "ecommerce-orders.order-addresses",
...
- Access consideration 1: Permission `ecommerce-orders.order-addresses.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `ecommerce-orders.order-addresses.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `ecommerce-orders.order-addresses.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `ecommerce-orders.order-addresses.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `ecommerce-orders.order-addresses.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `ecommerce-orders.order-addresses.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `ecommerce-orders.order-addresses.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `ecommerce-orders.order-addresses.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `ecommerce-orders.order-addresses.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `ecommerce-orders.order-addresses.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `ecommerce-orders.order-addresses.read` may show data, while command permissions must be separately configured per role.
