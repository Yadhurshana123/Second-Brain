---
title: "Order Addresses Feature Specification"
...
# Order Addresses Feature Specification

## Purpose
This document defines the implementation scope for `order-addresses` inside the `ecommerce-orders` module of the Unified Commerce platform.
...
|---|---|
| `customers` | Source or supporting table for `order-addresses` behavior. |
| `carts` | Source or supporting table for `order-addresses` behavior. |
| `cart_items` | Source or supporting table for `order-addresses` behavior. |
| `orders` | Source or supporting table for `order-addresses` behavior. |
| `order_items` | Source or supporting table for `order-addresses` behavior. |
| `order_addresses` | Source or supporting table for `order-addresses` behavior. |
| `order_status_history` | Source or supporting table for `order-addresses` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `ecommerce-orders.order-addresses.read` | View data and search records. | Yes |
| `ecommerce-orders.order-addresses.create` | Create new records or workflow drafts. | Yes |
| `ecommerce-orders.order-addresses.update` | Modify allowed editable fields. | Yes |
| `ecommerce-orders.order-addresses.approve` | Approve sensitive actions where applicable. | Yes |
| `ecommerce-orders.order-addresses.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class OrderAddressesService : IOrderAddressesService
{
    private readonly IOrderAddressesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<OrderAddressesDto>> HandleAsync(OrderAddressesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "ecommerce-orders.order-addresses.manage");
        ...
        return ApiResponse.Success(new OrderAddressesDto());
    }
}
```
...
```ts
export const orderaddressesQueryKey = (tenantId: string, outletId?: string) =>
  ["ecommerce-orders", "order-addresses", tenantId, outletId ?? "tenant"] as const;

export function useOrderAddresses(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: orderaddressesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/ecommerce-orders/order-addresses"),
    staleTime: 60_000,
  });
}
```
