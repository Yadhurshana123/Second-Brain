---
title: "Cart Items Feature Specification"
...
# Cart Items Feature Specification

## Purpose
This document defines the implementation scope for `cart-items` inside the `ecommerce-orders` module of the Unified Commerce platform.
...
|---|---|
| `customers` | Source or supporting table for `cart-items` behavior. |
| `carts` | Source or supporting table for `cart-items` behavior. |
| `cart_items` | Source or supporting table for `cart-items` behavior. |
| `orders` | Source or supporting table for `cart-items` behavior. |
| `order_items` | Source or supporting table for `cart-items` behavior. |
| `order_addresses` | Source or supporting table for `cart-items` behavior. |
| `order_status_history` | Source or supporting table for `cart-items` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `ecommerce-orders.cart-items.read` | View data and search records. | Yes |
| `ecommerce-orders.cart-items.create` | Create new records or workflow drafts. | Yes |
| `ecommerce-orders.cart-items.update` | Modify allowed editable fields. | Yes |
| `ecommerce-orders.cart-items.approve` | Approve sensitive actions where applicable. | Yes |
| `ecommerce-orders.cart-items.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class CartItemsService : ICartItemsService
{
    private readonly ICartItemsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<CartItemsDto>> HandleAsync(CartItemsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "ecommerce-orders.cart-items.manage");
        ...
        return ApiResponse.Success(new CartItemsDto());
    }
}
```
...
```ts
export const cartitemsQueryKey = (tenantId: string, outletId?: string) =>
  ["ecommerce-orders", "cart-items", tenantId, outletId ?? "tenant"] as const;

export function useCartItems(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: cartitemsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/ecommerce-orders/cart-items"),
    staleTime: 60_000,
  });
}
```
