---
title: "Delivery Items Feature Specification"
...
# Delivery Items Feature Specification

## Purpose
This document defines the implementation scope for `delivery-items` inside the `fulfillment-logistics` module of the Unified Commerce platform.
...
|---|---|
| `delivery_methods` | Source or supporting table for `delivery-items` behavior. |
| `delivery_zones` | Source or supporting table for `delivery-items` behavior. |
| `delivery_zone_rates` | Source or supporting table for `delivery-items` behavior. |
| `deliveries` | Source or supporting table for `delivery-items` behavior. |
| `delivery_items` | Source or supporting table for `delivery-items` behavior. |
| `delivery_tracking` | Source or supporting table for `delivery-items` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `fulfillment-logistics.delivery-items.read` | View data and search records. | Yes |
| `fulfillment-logistics.delivery-items.create` | Create new records or workflow drafts. | Yes |
| `fulfillment-logistics.delivery-items.update` | Modify allowed editable fields. | Yes |
| `fulfillment-logistics.delivery-items.approve` | Approve sensitive actions where applicable. | Yes |
| `fulfillment-logistics.delivery-items.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class DeliveryItemsService : IDeliveryItemsService
{
    private readonly IDeliveryItemsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<DeliveryItemsDto>> HandleAsync(DeliveryItemsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "fulfillment-logistics.delivery-items.manage");
        ...
        return ApiResponse.Success(new DeliveryItemsDto());
    }
}
```
...
```ts
export const deliveryitemsQueryKey = (tenantId: string, outletId?: string) =>
  ["fulfillment-logistics", "delivery-items", tenantId, outletId ?? "tenant"] as const;

export function useDeliveryItems(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: deliveryitemsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/fulfillment-logistics/delivery-items"),
    staleTime: 60_000,
  });
}
```
