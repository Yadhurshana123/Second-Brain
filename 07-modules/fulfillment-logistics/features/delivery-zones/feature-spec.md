---
title: "Delivery Zones Feature Specification"
...
# Delivery Zones Feature Specification

## Purpose
This document defines the implementation scope for `delivery-zones` inside the `fulfillment-logistics` module of the Unified Commerce platform.
...
|---|---|
| `delivery_methods` | Source or supporting table for `delivery-zones` behavior. |
| `delivery_zones` | Source or supporting table for `delivery-zones` behavior. |
| `delivery_zone_rates` | Source or supporting table for `delivery-zones` behavior. |
| `deliveries` | Source or supporting table for `delivery-zones` behavior. |
| `delivery_items` | Source or supporting table for `delivery-zones` behavior. |
| `delivery_tracking` | Source or supporting table for `delivery-zones` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `fulfillment-logistics.delivery-zones.read` | View data and search records. | Yes |
| `fulfillment-logistics.delivery-zones.create` | Create new records or workflow drafts. | Yes |
| `fulfillment-logistics.delivery-zones.update` | Modify allowed editable fields. | Yes |
| `fulfillment-logistics.delivery-zones.approve` | Approve sensitive actions where applicable. | Yes |
| `fulfillment-logistics.delivery-zones.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class DeliveryZonesService : IDeliveryZonesService
{
    private readonly IDeliveryZonesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<DeliveryZonesDto>> HandleAsync(DeliveryZonesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "fulfillment-logistics.delivery-zones.manage");
        ...
        return ApiResponse.Success(new DeliveryZonesDto());
    }
}
```
...
```ts
export const deliveryzonesQueryKey = (tenantId: string, outletId?: string) =>
  ["fulfillment-logistics", "delivery-zones", tenantId, outletId ?? "tenant"] as const;

export function useDeliveryZones(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: deliveryzonesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/fulfillment-logistics/delivery-zones"),
    staleTime: 60_000,
  });
}
```
