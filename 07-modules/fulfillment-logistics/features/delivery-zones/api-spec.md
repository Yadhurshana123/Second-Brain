---
title: "Delivery Zones API Specification"
...
# Delivery Zones API Specification

## API Ownership
The `delivery-zones` API belongs to the `fulfillment-logistics` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/fulfillment-logistics/delivery-zones` | `fulfillment-logistics.delivery-zones.read` |
| Detail | GET | `/api/v1/fulfillment-logistics/delivery-zones/{id}` | `fulfillment-logistics.delivery-zones.read` |
| Create | POST | `/api/v1/fulfillment-logistics/delivery-zones` | `fulfillment-logistics.delivery-zones.create` |
| Update | PUT | `/api/v1/fulfillment-logistics/delivery-zones/{id}` | `fulfillment-logistics.delivery-zones.update` |
| State action | POST | `/api/v1/fulfillment-logistics/delivery-zones/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/fulfillment-logistics/delivery-zones
...
{
  "featureKey": "fulfillment-logistics.delivery-zones",
...
- Access consideration 1: Permission `fulfillment-logistics.delivery-zones.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `fulfillment-logistics.delivery-zones.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `fulfillment-logistics.delivery-zones.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `fulfillment-logistics.delivery-zones.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `fulfillment-logistics.delivery-zones.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `fulfillment-logistics.delivery-zones.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `fulfillment-logistics.delivery-zones.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `fulfillment-logistics.delivery-zones.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `fulfillment-logistics.delivery-zones.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `fulfillment-logistics.delivery-zones.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `fulfillment-logistics.delivery-zones.read` may show data, while command permissions must be separately configured per role.
