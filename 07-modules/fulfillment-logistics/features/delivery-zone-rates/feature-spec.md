---
title: "Delivery Zone Rates Feature Specification"
...
# Delivery Zone Rates Feature Specification

## Purpose
This document defines the implementation scope for `delivery-zone-rates` inside the `fulfillment-logistics` module of the Unified Commerce platform.
...
|---|---|
| `delivery_methods` | Source or supporting table for `delivery-zone-rates` behavior. |
| `delivery_zones` | Source or supporting table for `delivery-zone-rates` behavior. |
| `delivery_zone_rates` | Source or supporting table for `delivery-zone-rates` behavior. |
| `deliveries` | Source or supporting table for `delivery-zone-rates` behavior. |
| `delivery_items` | Source or supporting table for `delivery-zone-rates` behavior. |
| `delivery_tracking` | Source or supporting table for `delivery-zone-rates` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `fulfillment-logistics.delivery-zone-rates.read` | View data and search records. | Yes |
| `fulfillment-logistics.delivery-zone-rates.create` | Create new records or workflow drafts. | Yes |
| `fulfillment-logistics.delivery-zone-rates.update` | Modify allowed editable fields. | Yes |
| `fulfillment-logistics.delivery-zone-rates.approve` | Approve sensitive actions where applicable. | Yes |
| `fulfillment-logistics.delivery-zone-rates.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class DeliveryZoneRatesService : IDeliveryZoneRatesService
{
    private readonly IDeliveryZoneRatesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<DeliveryZoneRatesDto>> HandleAsync(DeliveryZoneRatesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "fulfillment-logistics.delivery-zone-rates.manage");
        ...
        return ApiResponse.Success(new DeliveryZoneRatesDto());
    }
}
```
...
```ts
export const deliveryzoneratesQueryKey = (tenantId: string, outletId?: string) =>
  ["fulfillment-logistics", "delivery-zone-rates", tenantId, outletId ?? "tenant"] as const;

export function useDeliveryZoneRates(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: deliveryzoneratesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/fulfillment-logistics/delivery-zone-rates"),
    staleTime: 60_000,
  });
}
```
...
- Related module overview: [[../README|Fulfillment Logistics Overview]]
