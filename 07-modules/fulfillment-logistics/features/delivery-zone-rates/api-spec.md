---
title: "Delivery Zone Rates API Specification"
...
# Delivery Zone Rates API Specification

## API Ownership
The `delivery-zone-rates` API belongs to the `fulfillment-logistics` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/fulfillment-logistics/delivery-zone-rates` | `fulfillment-logistics.delivery-zone-rates.read` |
| Detail | GET | `/api/v1/fulfillment-logistics/delivery-zone-rates/{id}` | `fulfillment-logistics.delivery-zone-rates.read` |
| Create | POST | `/api/v1/fulfillment-logistics/delivery-zone-rates` | `fulfillment-logistics.delivery-zone-rates.create` |
| Update | PUT | `/api/v1/fulfillment-logistics/delivery-zone-rates/{id}` | `fulfillment-logistics.delivery-zone-rates.update` |
| State action | POST | `/api/v1/fulfillment-logistics/delivery-zone-rates/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/fulfillment-logistics/delivery-zone-rates
...
{
  "featureKey": "fulfillment-logistics.delivery-zone-rates",
...
- Access consideration 1: Permission `fulfillment-logistics.delivery-zone-rates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `fulfillment-logistics.delivery-zone-rates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `fulfillment-logistics.delivery-zone-rates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `fulfillment-logistics.delivery-zone-rates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `fulfillment-logistics.delivery-zone-rates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `fulfillment-logistics.delivery-zone-rates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `fulfillment-logistics.delivery-zone-rates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `fulfillment-logistics.delivery-zone-rates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `fulfillment-logistics.delivery-zone-rates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `fulfillment-logistics.delivery-zone-rates.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `fulfillment-logistics.delivery-zone-rates.read` may show data, while command permissions must be separately configured per role.
