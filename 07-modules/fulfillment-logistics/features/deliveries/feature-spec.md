---
title: "Deliveries Feature Specification"
...
# Deliveries Feature Specification

## Purpose
This document defines the implementation scope for `deliveries` inside the `fulfillment-logistics` module of the Unified Commerce platform.
...
|---|---|
| `delivery_methods` | Source or supporting table for `deliveries` behavior. |
| `delivery_zones` | Source or supporting table for `deliveries` behavior. |
| `delivery_zone_rates` | Source or supporting table for `deliveries` behavior. |
| `deliveries` | Source or supporting table for `deliveries` behavior. |
| `delivery_items` | Source or supporting table for `deliveries` behavior. |
| `delivery_tracking` | Source or supporting table for `deliveries` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `fulfillment-logistics.deliveries.read` | View data and search records. | Yes |
| `fulfillment-logistics.deliveries.create` | Create new records or workflow drafts. | Yes |
| `fulfillment-logistics.deliveries.update` | Modify allowed editable fields. | Yes |
| `fulfillment-logistics.deliveries.approve` | Approve sensitive actions where applicable. | Yes |
| `fulfillment-logistics.deliveries.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class DeliveriesService : IDeliveriesService
{
    private readonly IDeliveriesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<DeliveriesDto>> HandleAsync(DeliveriesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "fulfillment-logistics.deliveries.manage");
        ...
        return ApiResponse.Success(new DeliveriesDto());
    }
}
```
...
```ts
export const deliveriesQueryKey = (tenantId: string, outletId?: string) =>
  ["fulfillment-logistics", "deliveries", tenantId, outletId ?? "tenant"] as const;

export function useDeliveries(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: deliveriesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/fulfillment-logistics/deliveries"),
    staleTime: 60_000,
  });
}
```
