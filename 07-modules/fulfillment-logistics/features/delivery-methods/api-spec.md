---
title: "Delivery Methods API Specification"
...
# Delivery Methods API Specification

## API Ownership
The `delivery-methods` API belongs to the `fulfillment-logistics` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/fulfillment-logistics/delivery-methods` | `fulfillment-logistics.delivery-methods.read` |
| Detail | GET | `/api/v1/fulfillment-logistics/delivery-methods/{id}` | `fulfillment-logistics.delivery-methods.read` |
| Create | POST | `/api/v1/fulfillment-logistics/delivery-methods` | `fulfillment-logistics.delivery-methods.create` |
| Update | PUT | `/api/v1/fulfillment-logistics/delivery-methods/{id}` | `fulfillment-logistics.delivery-methods.update` |
| State action | POST | `/api/v1/fulfillment-logistics/delivery-methods/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/fulfillment-logistics/delivery-methods
...
{
  "featureKey": "fulfillment-logistics.delivery-methods",
...
- Access consideration 1: Permission `fulfillment-logistics.delivery-methods.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `fulfillment-logistics.delivery-methods.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `fulfillment-logistics.delivery-methods.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `fulfillment-logistics.delivery-methods.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `fulfillment-logistics.delivery-methods.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `fulfillment-logistics.delivery-methods.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `fulfillment-logistics.delivery-methods.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `fulfillment-logistics.delivery-methods.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `fulfillment-logistics.delivery-methods.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `fulfillment-logistics.delivery-methods.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `fulfillment-logistics.delivery-methods.read` may show data, while command permissions must be separately configured per role.
