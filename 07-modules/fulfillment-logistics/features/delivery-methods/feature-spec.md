---
title: "Delivery Methods Feature Specification"
...
# Delivery Methods Feature Specification

## Purpose
This document defines the implementation scope for `delivery-methods` inside the `fulfillment-logistics` module of the Unified Commerce platform.
...
|---|---|
| `delivery_methods` | Source or supporting table for `delivery-methods` behavior. |
| `delivery_zones` | Source or supporting table for `delivery-methods` behavior. |
| `delivery_zone_rates` | Source or supporting table for `delivery-methods` behavior. |
| `deliveries` | Source or supporting table for `delivery-methods` behavior. |
| `delivery_items` | Source or supporting table for `delivery-methods` behavior. |
| `delivery_tracking` | Source or supporting table for `delivery-methods` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `fulfillment-logistics.delivery-methods.read` | View data and search records. | Yes |
| `fulfillment-logistics.delivery-methods.create` | Create new records or workflow drafts. | Yes |
| `fulfillment-logistics.delivery-methods.update` | Modify allowed editable fields. | Yes |
| `fulfillment-logistics.delivery-methods.approve` | Approve sensitive actions where applicable. | Yes |
| `fulfillment-logistics.delivery-methods.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class DeliveryMethodsService : IDeliveryMethodsService
{
    private readonly IDeliveryMethodsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<DeliveryMethodsDto>> HandleAsync(DeliveryMethodsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "fulfillment-logistics.delivery-methods.manage");
        ...
        return ApiResponse.Success(new DeliveryMethodsDto());
    }
}
```
...
```ts
export const deliverymethodsQueryKey = (tenantId: string, outletId?: string) =>
  ["fulfillment-logistics", "delivery-methods", tenantId, outletId ?? "tenant"] as const;

export function useDeliveryMethods(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: deliverymethodsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/fulfillment-logistics/delivery-methods"),
    staleTime: 60_000,
  });
}
```
