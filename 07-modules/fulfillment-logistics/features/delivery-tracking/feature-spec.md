---
title: "Delivery Tracking Feature Specification"
...
# Delivery Tracking Feature Specification

## Purpose
This document defines the implementation scope for `delivery-tracking` inside the `fulfillment-logistics` module of the Unified Commerce platform.
...
|---|---|
| `delivery_methods` | Source or supporting table for `delivery-tracking` behavior. |
| `delivery_zones` | Source or supporting table for `delivery-tracking` behavior. |
| `delivery_zone_rates` | Source or supporting table for `delivery-tracking` behavior. |
| `deliveries` | Source or supporting table for `delivery-tracking` behavior. |
| `delivery_items` | Source or supporting table for `delivery-tracking` behavior. |
| `delivery_tracking` | Source or supporting table for `delivery-tracking` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `fulfillment-logistics.delivery-tracking.read` | View data and search records. | Yes |
| `fulfillment-logistics.delivery-tracking.create` | Create new records or workflow drafts. | Yes |
| `fulfillment-logistics.delivery-tracking.update` | Modify allowed editable fields. | Yes |
| `fulfillment-logistics.delivery-tracking.approve` | Approve sensitive actions where applicable. | Yes |
| `fulfillment-logistics.delivery-tracking.void` | Void/cancel/reverse where business rules allow. | Yes |
...
public sealed class DeliveryTrackingService : IDeliveryTrackingService
{
    ...
    public async Task<ApiResponse<DeliveryTrackingDto>> HandleAsync(DeliveryTrackingRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "fulfillment-logistics.delivery-tracking.manage");
        ...
        return ApiResponse.Success(new DeliveryTrackingDto());
    }
}
```
...
```ts
export const deliverytrackingQueryKey = (tenantId: string, outletId?: string) =>
  ["fulfillment-logistics", "delivery-tracking", tenantId, outletId ?? "tenant"] as const;

export function useDeliveryTracking(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: deliverytrackingQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/fulfillment-logistics/delivery-tracking"),
    staleTime: 60_000,
  });
}
```
