---
title: "Order Workflow Module Overview"
...
# Order Workflow Module Overview

## Module Purpose
The `order-workflow` module groups related Unified Commerce capabilities and documents how they are implemented across database, backend, API, frontend, security, and cache/storage layers.
...
|---|---|---|---|
| `fulfillment-status-workflow` | [[features/fulfillment-status-workflow/feature-spec|Spec]] | [[features/fulfillment-status-workflow/api-spec|API]] | [[features/fulfillment-status-workflow/feature-history|History]] |
| `order-status-history` | [[features/order-status-history/feature-spec|Spec]] | [[features/order-status-history/api-spec|API]] | [[features/order-status-history/feature-history|History]] |
| `order-status-transitions` | [[features/order-status-transitions/feature-spec|Spec]] | [[features/order-status-transitions/api-spec|API]] | [[features/order-status-transitions/feature-history|History]] |
| `payment-status-workflow` | [[features/payment-status-workflow/feature-spec|Spec]] | [[features/payment-status-workflow/api-spec|API]] | [[features/payment-status-workflow/feature-history|History]] |

## Database Tables Commonly Used
| Table | Purpose in module |
|---|---|
| `orders` | Supports `order-workflow` module workflows and tenant-scoped persistence. |
| `order_status_transitions` | Supports `order-workflow` module workflows and tenant-scoped persistence. |
| `order_status_history` | Supports `order-workflow` module workflows and tenant-scoped persistence. |
| `deliveries` | Supports `order-workflow` module workflows and tenant-scoped persistence. |
| `refunds` | Supports `order-workflow` module workflows and tenant-scoped persistence. |
...
|---|---|---|
| API | `POS.API/Modules/OrderWorkflow` | Controllers, request models, response models. |
| Application | `POS.Application/Modules/OrderWorkflow` | Services, validators, interfaces, and `Dtos/`. |
| Domain | `POS.Domain/Modules/OrderWorkflow` | Pure rules and entities where needed. |
| Infrastructure | `POS.Infrastructure/Repositories/OrderWorkflow` | EF Core repositories and persistence logic. |
...
- Access consideration 32: Permission `order-workflow.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 33: Permission `order-workflow.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 34: Permission `order-workflow.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 35: Permission `order-workflow.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 36: Permission `order-workflow.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 37: Permission `order-workflow.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 38: Permission `order-workflow.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 39: Permission `order-workflow.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 40: Permission `order-workflow.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 41: Permission `order-workflow.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 42: Permission `order-workflow.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 43: Permission `order-workflow.module.read` may show data, while command permissions must be separately configured per role.
