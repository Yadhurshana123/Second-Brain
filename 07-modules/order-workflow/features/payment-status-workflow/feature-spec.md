---
title: "Payment Status Workflow Feature Specification"
...
# Payment Status Workflow Feature Specification

## Purpose
This document defines the implementation scope for `payment-status-workflow` inside the `order-workflow` module of the Unified Commerce platform.
...
|---|---|
| `orders` | Source or supporting table for `payment-status-workflow` behavior. |
| `order_status_transitions` | Source or supporting table for `payment-status-workflow` behavior. |
| `order_status_history` | Source or supporting table for `payment-status-workflow` behavior. |
| `deliveries` | Source or supporting table for `payment-status-workflow` behavior. |
| `refunds` | Source or supporting table for `payment-status-workflow` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `order-workflow.payment-status-workflow.read` | View data and search records. | Yes |
| `order-workflow.payment-status-workflow.create` | Create new records or workflow drafts. | Yes |
| `order-workflow.payment-status-workflow.update` | Modify allowed editable fields. | Yes |
| `order-workflow.payment-status-workflow.approve` | Approve sensitive actions where applicable. | Yes |
| `order-workflow.payment-status-workflow.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class PaymentStatusWorkflowService : IPaymentStatusWorkflowService
{
    private readonly IPaymentStatusWorkflowRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<PaymentStatusWorkflowDto>> HandleAsync(PaymentStatusWorkflowRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "order-workflow.payment-status-workflow.manage");
        ...
        return ApiResponse.Success(new PaymentStatusWorkflowDto());
    }
}
```
...
```ts
export const paymentstatusworkflowQueryKey = (tenantId: string, outletId?: string) =>
  ["order-workflow", "payment-status-workflow", tenantId, outletId ?? "tenant"] as const;

export function usePaymentStatusWorkflow(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: paymentstatusworkflowQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/order-workflow/payment-status-workflow"),
    staleTime: 60_000,
  });
}
```
