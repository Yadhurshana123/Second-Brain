---
title: "Order Status Transitions Feature Specification"
...
# Order Status Transitions Feature Specification

## Purpose
This document defines the implementation scope for `order-status-transitions` inside the `order-workflow` module of the Unified Commerce platform.
...
|---|---|
| `orders` | Source or supporting table for `order-status-transitions` behavior. |
| `order_status_transitions` | Source or supporting table for `order-status-transitions` behavior. |
| `order_status_history` | Source or supporting table for `order-status-transitions` behavior. |
| `deliveries` | Source or supporting table for `order-status-transitions` behavior. |
| `refunds` | Source or supporting table for `order-status-transitions` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `order-workflow.order-status-transitions.read` | View data and search records. | Yes |
| `order-workflow.order-status-transitions.create` | Create new records or workflow drafts. | Yes |
| `order-workflow.order-status-transitions.update` | Modify allowed editable fields. | Yes |
| `order-workflow.order-status-transitions.approve` | Approve sensitive actions where applicable. | Yes |
| `order-workflow.order-status-transitions.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class OrderStatusTransitionsService : IOrderStatusTransitionsService
{
    private readonly IOrderStatusTransitionsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<OrderStatusTransitionsDto>> HandleAsync(OrderStatusTransitionsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "order-workflow.order-status-transitions.manage");
        ...
        return ApiResponse.Success(new OrderStatusTransitionsDto());
    }
}
```
...
```ts
export const orderstatustransitionsQueryKey = (tenantId: string, outletId?: string) =>
  ["order-workflow", "order-status-transitions", tenantId, outletId ?? "tenant"] as const;

export function useOrderStatusTransitions(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: orderstatustransitionsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/order-workflow/order-status-transitions"),
    staleTime: 60_000,
  });
}
```
