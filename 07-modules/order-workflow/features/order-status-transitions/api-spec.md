---
title: "Order Status Transitions API Specification"
...
# Order Status Transitions API Specification

## API Ownership
The `order-status-transitions` API belongs to the `order-workflow` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/order-workflow/order-status-transitions` | `order-workflow.order-status-transitions.read` |
| Detail | GET | `/api/v1/order-workflow/order-status-transitions/{id}` | `order-workflow.order-status-transitions.read` |
| Create | POST | `/api/v1/order-workflow/order-status-transitions` | `order-workflow.order-status-transitions.create` |
| Update | PUT | `/api/v1/order-workflow/order-status-transitions/{id}` | `order-workflow.order-status-transitions.update` |
| State action | POST | `/api/v1/order-workflow/order-status-transitions/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/order-workflow/order-status-transitions
...
{
  "featureKey": "order-workflow.order-status-transitions",
...
- Access consideration 1: Permission `order-workflow.order-status-transitions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `order-workflow.order-status-transitions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `order-workflow.order-status-transitions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `order-workflow.order-status-transitions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `order-workflow.order-status-transitions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `order-workflow.order-status-transitions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `order-workflow.order-status-transitions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `order-workflow.order-status-transitions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `order-workflow.order-status-transitions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `order-workflow.order-status-transitions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `order-workflow.order-status-transitions.read` may show data, while command permissions must be separately configured per role.
