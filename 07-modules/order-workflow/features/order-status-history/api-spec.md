---
title: "Order Status History API Specification"
...
# Order Status History API Specification

## API Ownership
The `order-status-history` API belongs to the `order-workflow` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/order-workflow/order-status-history` | `order-workflow.order-status-history.read` |
| Detail | GET | `/api/v1/order-workflow/order-status-history/{id}` | `order-workflow.order-status-history.read` |
| Create | POST | `/api/v1/order-workflow/order-status-history` | `order-workflow.order-status-history.create` |
| Update | PUT | `/api/v1/order-workflow/order-status-history/{id}` | `order-workflow.order-status-history.update` |
| State action | POST | `/api/v1/order-workflow/order-status-history/{id}/actions/{action}` | Action-specific permission |
...
- Access consideration 1: Permission `order-workflow.order-status-history.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `order-workflow.order-status-history.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `order-workflow.order-status-history.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `order-workflow.order-status-history.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `order-workflow.order-status-history.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `order-workflow.order-status-history.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `order-workflow.order-status-history.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `order-workflow.order-status-history.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `order-workflow.order-status-history.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `order-workflow.order-status-history.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `order-workflow.order-status-history.read` may show data, while command permissions must be separately configured per role.
