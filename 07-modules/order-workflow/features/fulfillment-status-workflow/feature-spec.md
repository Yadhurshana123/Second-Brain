---
title: "Fulfillment Status Workflow Feature Specification"
...
# Fulfillment Status Workflow Feature Specification

## Purpose
This document defines the implementation scope for `fulfillment-status-workflow` inside the `order-workflow` module of the Unified Commerce platform.
...
|---|---|
| `orders` | Source or supporting table for `fulfillment-status-workflow` behavior. |
| `order_status_transitions` | Source or supporting table for `fulfillment-status-workflow` behavior. |
| `order_status_history` | Source or supporting table for `fulfillment-status-workflow` behavior. |
| `deliveries` | Source or supporting table for `fulfillment-status-workflow` behavior. |
| `refunds` | Source or supporting table for `fulfillment-status-workflow` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `order-workflow.fulfillment-status-workflow.read` | View data and search records. | Yes |
| `order-workflow.fulfillment-status-workflow.create` | Create new records or workflow drafts. | Yes |
| `order-workflow.fulfillment-status-workflow.update` | Modify allowed editable fields. | Yes |
| `order-workflow.fulfillment-status-workflow.approve` | Approve sensitive actions where applicable. | Yes |
| `order-workflow.fulfillment-status-workflow.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class FulfillmentStatusWorkflowService : IFulfillmentStatusWorkflowService
{
    private readonly IFulfillmentStatusWorkflowRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<FulfillmentStatusWorkflowDto>> HandleAsync(FulfillmentStatusWorkflowRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "order-workflow.fulfillment-status-workflow.manage");
        ...
        return ApiResponse.Success(new FulfillmentStatusWorkflowDto());
    }
}
```
...
```ts
export const fulfillmentstatusworkflowQueryKey = (tenantId: string, outletId?: string) =>
  ["order-workflow", "fulfillment-status-workflow", tenantId, outletId ?? "tenant"] as const;

export function useFulfillmentStatusWorkflow(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: fulfillmentstatusworkflowQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/order-workflow/fulfillment-status-workflow"),
    staleTime: 60_000,
  });
}
```
