---
title: "Daily Inventory Reporting Feature Specification"
...
# Daily Inventory Reporting Feature Specification

## Purpose
This document defines the implementation scope for `daily-inventory-reporting` inside the `reporting` module of the Unified Commerce platform.
...
|---|---|
| `daily_sales_summaries` | Source or supporting table for `daily-inventory-reporting` behavior. |
| `daily_payment_summaries` | Source or supporting table for `daily-inventory-reporting` behavior. |
| `daily_inventory_summaries` | Source or supporting table for `daily-inventory-reporting` behavior. |
| `daily_discount_return_summaries` | Source or supporting table for `daily-inventory-reporting` behavior. |
| `audit_logs` | Source or supporting table for `daily-inventory-reporting` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `reporting.daily-inventory-reporting.read` | View data and search records. | Yes |
| `reporting.daily-inventory-reporting.create` | Create new records or workflow drafts. | Yes |
| `reporting.daily-inventory-reporting.update` | Modify allowed editable fields. | Yes |
| `reporting.daily-inventory-reporting.approve` | Approve sensitive actions where applicable. | Yes |
| `reporting.daily-inventory-reporting.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class DailyInventoryReportingService : IDailyInventoryReportingService
{
    private readonly IDailyInventoryReportingRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<DailyInventoryReportingDto>> HandleAsync(DailyInventoryReportingRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "reporting.daily-inventory-reporting.manage");
        ...
        return ApiResponse.Success(new DailyInventoryReportingDto());
    }
}
```
...
```ts
export const dailyinventoryreportingQueryKey = (tenantId: string, outletId?: string) =>
  ["reporting", "daily-inventory-reporting", tenantId, outletId ?? "tenant"] as const;

export function useDailyInventoryReporting(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: dailyinventoryreportingQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/reporting/daily-inventory-reporting"),
    staleTime: 60_000,
  });
}
```
