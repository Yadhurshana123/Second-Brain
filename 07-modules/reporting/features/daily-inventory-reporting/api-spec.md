---
title: "Daily Inventory Reporting API Specification"
...
# Daily Inventory Reporting API Specification

## API Ownership
The `daily-inventory-reporting` API belongs to the `reporting` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/reporting/daily-inventory-reporting` | `reporting.daily-inventory-reporting.read` |
| Detail | GET | `/api/v1/reporting/daily-inventory-reporting/{id}` | `reporting.daily-inventory-reporting.read` |
| Create | POST | `/api/v1/reporting/daily-inventory-reporting` | `reporting.daily-inventory-reporting.create` |
| Update | PUT | `/api/v1/reporting/daily-inventory-reporting/{id}` | `reporting.daily-inventory-reporting.update` |
| State action | POST | `/api/v1/reporting/daily-inventory-reporting/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/reporting/daily-inventory-reporting
...
{
  "featureKey": "reporting.daily-inventory-reporting",
...
- Access consideration 1: Permission `reporting.daily-inventory-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `reporting.daily-inventory-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `reporting.daily-inventory-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `reporting.daily-inventory-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `reporting.daily-inventory-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `reporting.daily-inventory-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `reporting.daily-inventory-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `reporting.daily-inventory-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `reporting.daily-inventory-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `reporting.daily-inventory-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `reporting.daily-inventory-reporting.read` may show data, while command permissions must be separately configured per role.
