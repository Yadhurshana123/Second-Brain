---
title: "Daily Discount Return Reporting Feature Specification"
...
# Daily Discount Return Reporting Feature Specification

## Purpose
This document defines the implementation scope for `daily-discount-return-reporting` inside the `reporting` module of the Unified Commerce platform.
...
|---|---|
| `daily_sales_summaries` | Source or supporting table for `daily-discount-return-reporting` behavior. |
| `daily_payment_summaries` | Source or supporting table for `daily-discount-return-reporting` behavior. |
| `daily_inventory_summaries` | Source or supporting table for `daily-discount-return-reporting` behavior. |
| `daily_discount_return_summaries` | Source or supporting table for `daily-discount-return-reporting` behavior. |
| `audit_logs` | Source or supporting table for `daily-discount-return-reporting` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `reporting.daily-discount-return-reporting.read` | View data and search records. | Yes |
| `reporting.daily-discount-return-reporting.create` | Create new records or workflow drafts. | Yes |
| `reporting.daily-discount-return-reporting.update` | Modify allowed editable fields. | Yes |
| `reporting.daily-discount-return-reporting.approve` | Approve sensitive actions where applicable. | Yes |
| `reporting.daily-discount-return-reporting.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class DailyDiscountReturnReportingService : IDailyDiscountReturnReportingService
{
    private readonly IDailyDiscountReturnReportingRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<DailyDiscountReturnReportingDto>> HandleAsync(DailyDiscountReturnReportingRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "reporting.daily-discount-return-reporting.manage");
        ...
        return ApiResponse.Success(new DailyDiscountReturnReportingDto());
    }
}
```
...
```ts
export const dailydiscountreturnreportingQueryKey = (tenantId: string, outletId?: string) =>
  ["reporting", "daily-discount-return-reporting", tenantId, outletId ?? "tenant"] as const;

export function useDailyDiscountReturnReporting(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: dailydiscountreturnreportingQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/reporting/daily-discount-return-reporting"),
    staleTime: 60_000,
  });
}
```
...
- Related module overview: [[../README|Reporting Overview]]
