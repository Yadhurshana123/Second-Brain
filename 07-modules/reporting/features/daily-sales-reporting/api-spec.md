---
title: "Daily Sales Reporting API Specification"
...
# Daily Sales Reporting API Specification

## API Ownership
The `daily-sales-reporting` API belongs to the `reporting` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/reporting/daily-sales-reporting` | `reporting.daily-sales-reporting.read` |
| Detail | GET | `/api/v1/reporting/daily-sales-reporting/{id}` | `reporting.daily-sales-reporting.read` |
| Create | POST | `/api/v1/reporting/daily-sales-reporting` | `reporting.daily-sales-reporting.create` |
| Update | PUT | `/api/v1/reporting/daily-sales-reporting/{id}` | `reporting.daily-sales-reporting.update` |
| State action | POST | `/api/v1/reporting/daily-sales-reporting/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/reporting/daily-sales-reporting
...
{
  "featureKey": "reporting.daily-sales-reporting",
...
- Access consideration 1: Permission `reporting.daily-sales-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `reporting.daily-sales-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `reporting.daily-sales-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `reporting.daily-sales-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `reporting.daily-sales-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `reporting.daily-sales-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `reporting.daily-sales-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `reporting.daily-sales-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `reporting.daily-sales-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `reporting.daily-sales-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `reporting.daily-sales-reporting.read` may show data, while command permissions must be separately configured per role.
