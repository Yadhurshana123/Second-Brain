---
title: "Daily Sales Reporting Feature Specification"
...
# Daily Sales Reporting Feature Specification

## Purpose
This document defines the implementation scope for `daily-sales-reporting` inside the `reporting` module of the Unified Commerce platform.
...
|---|---|
| `daily_sales_summaries` | Source or supporting table for `daily-sales-reporting` behavior. |
| `daily_payment_summaries` | Source or supporting table for `daily-sales-reporting` behavior. |
| `daily_inventory_summaries` | Source or supporting table for `daily-sales-reporting` behavior. |
| `daily_discount_return_summaries` | Source or supporting table for `daily-sales-reporting` behavior. |
| `audit_logs` | Source or supporting table for `daily-sales-reporting` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `reporting.daily-sales-reporting.read` | View data and search records. | Yes |
| `reporting.daily-sales-reporting.create` | Create new records or workflow drafts. | Yes |
| `reporting.daily-sales-reporting.update` | Modify allowed editable fields. | Yes |
| `reporting.daily-sales-reporting.approve` | Approve sensitive actions where applicable. | Yes |
| `reporting.daily-sales-reporting.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class DailySalesReportingService : IDailySalesReportingService
{
    private readonly IDailySalesReportingRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<DailySalesReportingDto>> HandleAsync(DailySalesReportingRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "reporting.daily-sales-reporting.manage");
        ...
        return ApiResponse.Success(new DailySalesReportingDto());
    }
}
```
...
```ts
export const dailysalesreportingQueryKey = (tenantId: string, outletId?: string) =>
  ["reporting", "daily-sales-reporting", tenantId, outletId ?? "tenant"] as const;

export function useDailySalesReporting(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: dailysalesreportingQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/reporting/daily-sales-reporting"),
    staleTime: 60_000,
  });
}
```
