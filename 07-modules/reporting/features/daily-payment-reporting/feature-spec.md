---
title: "Daily Payment Reporting Feature Specification"
...
# Daily Payment Reporting Feature Specification

## Purpose
This document defines the implementation scope for `daily-payment-reporting` inside the `reporting` module of the Unified Commerce platform.
...
|---|---|
| `daily_sales_summaries` | Source or supporting table for `daily-payment-reporting` behavior. |
| `daily_payment_summaries` | Source or supporting table for `daily-payment-reporting` behavior. |
| `daily_inventory_summaries` | Source or supporting table for `daily-payment-reporting` behavior. |
| `daily_discount_return_summaries` | Source or supporting table for `daily-payment-reporting` behavior. |
| `audit_logs` | Source or supporting table for `daily-payment-reporting` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `reporting.daily-payment-reporting.read` | View data and search records. | Yes |
| `reporting.daily-payment-reporting.create` | Create new records or workflow drafts. | Yes |
| `reporting.daily-payment-reporting.update` | Modify allowed editable fields. | Yes |
| `reporting.daily-payment-reporting.approve` | Approve sensitive actions where applicable. | Yes |
| `reporting.daily-payment-reporting.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class DailyPaymentReportingService : IDailyPaymentReportingService
{
    private readonly IDailyPaymentReportingRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<DailyPaymentReportingDto>> HandleAsync(DailyPaymentReportingRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "reporting.daily-payment-reporting.manage");
        ...
        return ApiResponse.Success(new DailyPaymentReportingDto());
    }
}
```
...
```ts
export const dailypaymentreportingQueryKey = (tenantId: string, outletId?: string) =>
  ["reporting", "daily-payment-reporting", tenantId, outletId ?? "tenant"] as const;

export function useDailyPaymentReporting(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: dailypaymentreportingQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/reporting/daily-payment-reporting"),
    staleTime: 60_000,
  });
}
```
