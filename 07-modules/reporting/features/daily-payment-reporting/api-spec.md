---
title: "Daily Payment Reporting API Specification"
...
# Daily Payment Reporting API Specification

## API Ownership
The `daily-payment-reporting` API belongs to the `reporting` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/reporting/daily-payment-reporting` | `reporting.daily-payment-reporting.read` |
| Detail | GET | `/api/v1/reporting/daily-payment-reporting/{id}` | `reporting.daily-payment-reporting.read` |
| Create | POST | `/api/v1/reporting/daily-payment-reporting` | `reporting.daily-payment-reporting.create` |
| Update | PUT | `/api/v1/reporting/daily-payment-reporting/{id}` | `reporting.daily-payment-reporting.update` |
| State action | POST | `/api/v1/reporting/daily-payment-reporting/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/reporting/daily-payment-reporting
...
{
  "featureKey": "reporting.daily-payment-reporting",
...
- Access consideration 1: Permission `reporting.daily-payment-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `reporting.daily-payment-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `reporting.daily-payment-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `reporting.daily-payment-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `reporting.daily-payment-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `reporting.daily-payment-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `reporting.daily-payment-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `reporting.daily-payment-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `reporting.daily-payment-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `reporting.daily-payment-reporting.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `reporting.daily-payment-reporting.read` may show data, while command permissions must be separately configured per role.
