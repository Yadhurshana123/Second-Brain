---
title: "Customer Auth Accounts API Specification"
...
# Customer Auth Accounts API Specification

## API Ownership
The `customer-auth-accounts` API belongs to the `customers` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/customers/customer-auth-accounts` | `customers.customer-auth-accounts.read` |
| Detail | GET | `/api/v1/customers/customer-auth-accounts/{id}` | `customers.customer-auth-accounts.read` |
| Create | POST | `/api/v1/customers/customer-auth-accounts` | `customers.customer-auth-accounts.create` |
| Update | PUT | `/api/v1/customers/customer-auth-accounts/{id}` | `customers.customer-auth-accounts.update` |
| State action | POST | `/api/v1/customers/customer-auth-accounts/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/customers/customer-auth-accounts
...
{
  "featureKey": "customers.customer-auth-accounts",
...
- Access consideration 1: Permission `customers.customer-auth-accounts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `customers.customer-auth-accounts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `customers.customer-auth-accounts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `customers.customer-auth-accounts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `customers.customer-auth-accounts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `customers.customer-auth-accounts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `customers.customer-auth-accounts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `customers.customer-auth-accounts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `customers.customer-auth-accounts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `customers.customer-auth-accounts.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `customers.customer-auth-accounts.read` may show data, while command permissions must be separately configured per role.
