---
title: "Customer Auth Accounts Feature Specification"
...
# Customer Auth Accounts Feature Specification

## Purpose
This document defines the implementation scope for `customer-auth-accounts` inside the `customers` module of the Unified Commerce platform.
...
|---|---|
| `customers` | Source or supporting table for `customer-auth-accounts` behavior. |
| `customer_auth_accounts` | Source or supporting table for `customer-auth-accounts` behavior. |
| `customer_auth_identities` | Source or supporting table for `customer-auth-accounts` behavior. |
| `customer_addresses` | Source or supporting table for `customer-auth-accounts` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `customers.customer-auth-accounts.read` | View data and search records. | Yes |
| `customers.customer-auth-accounts.create` | Create new records or workflow drafts. | Yes |
| `customers.customer-auth-accounts.update` | Modify allowed editable fields. | Yes |
| `customers.customer-auth-accounts.approve` | Approve sensitive actions where applicable. | Yes |
| `customers.customer-auth-accounts.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class CustomerAuthAccountsService : ICustomerAuthAccountsService
{
    private readonly ICustomerAuthAccountsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<CustomerAuthAccountsDto>> HandleAsync(CustomerAuthAccountsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "customers.customer-auth-accounts.manage");
        ...
        return ApiResponse.Success(new CustomerAuthAccountsDto());
    }
}
```
...
```ts
export const customerauthaccountsQueryKey = (tenantId: string, outletId?: string) =>
  ["customers", "customer-auth-accounts", tenantId, outletId ?? "tenant"] as const;

export function useCustomerAuthAccounts(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: customerauthaccountsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/customers/customer-auth-accounts"),
    staleTime: 60_000,
  });
}
```
