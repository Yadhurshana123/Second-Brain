---
title: "Customer Auth Identities API Specification"
...
# Customer Auth Identities API Specification

## API Ownership
The `customer-auth-identities` API belongs to the `customers` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/customers/customer-auth-identities` | `customers.customer-auth-identities.read` |
| Detail | GET | `/api/v1/customers/customer-auth-identities/{id}` | `customers.customer-auth-identities.read` |
| Create | POST | `/api/v1/customers/customer-auth-identities` | `customers.customer-auth-identities.create` |
| Update | PUT | `/api/v1/customers/customer-auth-identities/{id}` | `customers.customer-auth-identities.update` |
| State action | POST | `/api/v1/customers/customer-auth-identities/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/customers/customer-auth-identities
...
{
  "featureKey": "customers.customer-auth-identities",
...
- Access consideration 1: Permission `customers.customer-auth-identities.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `customers.customer-auth-identities.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `customers.customer-auth-identities.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `customers.customer-auth-identities.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `customers.customer-auth-identities.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `customers.customer-auth-identities.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `customers.customer-auth-identities.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `customers.customer-auth-identities.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `customers.customer-auth-identities.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `customers.customer-auth-identities.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `customers.customer-auth-identities.read` may show data, while command permissions must be separately configured per role.
