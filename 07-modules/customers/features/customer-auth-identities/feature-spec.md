---
title: "Customer Auth Identities Feature Specification"
...
# Customer Auth Identities Feature Specification

## Purpose
This document defines the implementation scope for `customer-auth-identities` inside the `customers` module of the Unified Commerce platform.
...
|---|---|
| `customers` | Source or supporting table for `customer-auth-identities` behavior. |
| `customer_auth_accounts` | Source or supporting table for `customer-auth-identities` behavior. |
| `customer_auth_identities` | Source or supporting table for `customer-auth-identities` behavior. |
| `customer_addresses` | Source or supporting table for `customer-auth-identities` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `customers.customer-auth-identities.read` | View data and search records. | Yes |
| `customers.customer-auth-identities.create` | Create new records or workflow drafts. | Yes |
| `customers.customer-auth-identities.update` | Modify allowed editable fields. | Yes |
| `customers.customer-auth-identities.approve` | Approve sensitive actions where applicable. | Yes |
| `customers.customer-auth-identities.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class CustomerAuthIdentitiesService : ICustomerAuthIdentitiesService
{
    private readonly ICustomerAuthIdentitiesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<CustomerAuthIdentitiesDto>> HandleAsync(CustomerAuthIdentitiesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "customers.customer-auth-identities.manage");
        ...
        return ApiResponse.Success(new CustomerAuthIdentitiesDto());
    }
}
```
...
```ts
export const customerauthidentitiesQueryKey = (tenantId: string, outletId?: string) =>
  ["customers", "customer-auth-identities", tenantId, outletId ?? "tenant"] as const;

export function useCustomerAuthIdentities(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: customerauthidentitiesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/customers/customer-auth-identities"),
    staleTime: 60_000,
  });
}
```
