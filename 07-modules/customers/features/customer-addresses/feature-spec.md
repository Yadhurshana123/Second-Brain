---
title: "Customer Addresses Feature Specification"
...
# Customer Addresses Feature Specification

## Purpose
This document defines the implementation scope for `customer-addresses` inside the `customers` module of the Unified Commerce platform.
...
|---|---|
| `customers` | Source or supporting table for `customer-addresses` behavior. |
| `customer_auth_accounts` | Source or supporting table for `customer-addresses` behavior. |
| `customer_auth_identities` | Source or supporting table for `customer-addresses` behavior. |
| `customer_addresses` | Source or supporting table for `customer-addresses` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `customers.customer-addresses.read` | View data and search records. | Yes |
| `customers.customer-addresses.create` | Create new records or workflow drafts. | Yes |
| `customers.customer-addresses.update` | Modify allowed editable fields. | Yes |
| `customers.customer-addresses.approve` | Approve sensitive actions where applicable. | Yes |
| `customers.customer-addresses.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class CustomerAddressesService : ICustomerAddressesService
{
    private readonly ICustomerAddressesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<CustomerAddressesDto>> HandleAsync(CustomerAddressesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "customers.customer-addresses.manage");
        ...
        return ApiResponse.Success(new CustomerAddressesDto());
    }
}
```
...
```ts
export const customeraddressesQueryKey = (tenantId: string, outletId?: string) =>
  ["customers", "customer-addresses", tenantId, outletId ?? "tenant"] as const;

export function useCustomerAddresses(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: customeraddressesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/customers/customer-addresses"),
    staleTime: 60_000,
  });
}
```
