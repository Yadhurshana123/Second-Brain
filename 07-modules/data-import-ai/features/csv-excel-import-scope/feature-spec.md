---
title: "Csv Excel Import Scope Feature Specification"
...
# Csv Excel Import Scope Feature Specification

## Purpose
This document defines the implementation scope for `csv-excel-import-scope` inside the `data-import-ai` module of the Unified Commerce platform.
...
|---|---|
| `products` | Source or supporting table for `csv-excel-import-scope` behavior. |
| `product_variants` | Source or supporting table for `csv-excel-import-scope` behavior. |
| `customers` | Source or supporting table for `csv-excel-import-scope` behavior. |
| `suppliers` | Source or supporting table for `csv-excel-import-scope` behavior. |
| `inventory_balances` | Source or supporting table for `csv-excel-import-scope` behavior. |
| `audit_logs` | Source or supporting table for `csv-excel-import-scope` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `data-import-ai.csv-excel-import-scope.read` | View data and search records. | Yes |
| `data-import-ai.csv-excel-import-scope.create` | Create new records or workflow drafts. | Yes |
| `data-import-ai.csv-excel-import-scope.update` | Modify allowed editable fields. | Yes |
| `data-import-ai.csv-excel-import-scope.approve` | Approve sensitive actions where applicable. | Yes |
| `data-import-ai.csv-excel-import-scope.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class CsvExcelImportScopeService : ICsvExcelImportScopeService
{
    private readonly ICsvExcelImportScopeRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<CsvExcelImportScopeDto>> HandleAsync(CsvExcelImportScopeRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "data-import-ai.csv-excel-import-scope.manage");
        ...
        return ApiResponse.Success(new CsvExcelImportScopeDto());
    }
}
```
...
```ts
export const csvexcelimportscopeQueryKey = (tenantId: string, outletId?: string) =>
  ["data-import-ai", "csv-excel-import-scope", tenantId, outletId ?? "tenant"] as const;

export function useCsvExcelImportScope(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: csvexcelimportscopeQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/data-import-ai/csv-excel-import-scope"),
    staleTime: 60_000,
  });
}
```
