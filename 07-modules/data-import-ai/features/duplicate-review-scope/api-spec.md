---
title: "Duplicate Review Scope API Specification"
...
# Duplicate Review Scope API Specification

## API Ownership
The `duplicate-review-scope` API belongs to the `data-import-ai` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/data-import-ai/duplicate-review-scope` | `data-import-ai.duplicate-review-scope.read` |
| Detail | GET | `/api/v1/data-import-ai/duplicate-review-scope/{id}` | `data-import-ai.duplicate-review-scope.read` |
| Create | POST | `/api/v1/data-import-ai/duplicate-review-scope` | `data-import-ai.duplicate-review-scope.create` |
| Update | PUT | `/api/v1/data-import-ai/duplicate-review-scope/{id}` | `data-import-ai.duplicate-review-scope.update` |
| State action | POST | `/api/v1/data-import-ai/duplicate-review-scope/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/data-import-ai/duplicate-review-scope
...
{
  "featureKey": "data-import-ai.duplicate-review-scope",
...
- Access consideration 1: Permission `data-import-ai.duplicate-review-scope.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `data-import-ai.duplicate-review-scope.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `data-import-ai.duplicate-review-scope.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `data-import-ai.duplicate-review-scope.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `data-import-ai.duplicate-review-scope.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `data-import-ai.duplicate-review-scope.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `data-import-ai.duplicate-review-scope.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `data-import-ai.duplicate-review-scope.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `data-import-ai.duplicate-review-scope.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `data-import-ai.duplicate-review-scope.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `data-import-ai.duplicate-review-scope.read` may show data, while command permissions must be separately configured per role.
