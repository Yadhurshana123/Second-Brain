---
title: "Duplicate Review Scope Feature Specification"
...
# Duplicate Review Scope Feature Specification

## Purpose
This document defines the implementation scope for `duplicate-review-scope` inside the `data-import-ai` module of the Unified Commerce platform.
...
|---|---|
| `products` | Source or supporting table for `duplicate-review-scope` behavior. |
| `product_variants` | Source or supporting table for `duplicate-review-scope` behavior. |
| `customers` | Source or supporting table for `duplicate-review-scope` behavior. |
| `suppliers` | Source or supporting table for `duplicate-review-scope` behavior. |
| `inventory_balances` | Source or supporting table for `duplicate-review-scope` behavior. |
| `audit_logs` | Source or supporting table for `duplicate-review-scope` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `data-import-ai.duplicate-review-scope.read` | View data and search records. | Yes |
| `data-import-ai.duplicate-review-scope.create` | Create new records or workflow drafts. | Yes |
| `data-import-ai.duplicate-review-scope.update` | Modify allowed editable fields. | Yes |
| `data-import-ai.duplicate-review-scope.approve` | Approve sensitive actions where applicable. | Yes |
| `data-import-ai.duplicate-review-scope.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class DuplicateReviewScopeService : IDuplicateReviewScopeService
{
    private readonly IDuplicateReviewScopeRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<DuplicateReviewScopeDto>> HandleAsync(DuplicateReviewScopeRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "data-import-ai.duplicate-review-scope.manage");
        ...
        return ApiResponse.Success(new DuplicateReviewScopeDto());
    }
}
```
...
```ts
export const duplicatereviewscopeQueryKey = (tenantId: string, outletId?: string) =>
  ["data-import-ai", "duplicate-review-scope", tenantId, outletId ?? "tenant"] as const;

export function useDuplicateReviewScope(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: duplicatereviewscopeQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/data-import-ai/duplicate-review-scope"),
    staleTime: 60_000,
  });
}
```
