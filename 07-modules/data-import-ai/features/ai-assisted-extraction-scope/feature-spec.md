---
title: "Ai Assisted Extraction Scope Feature Specification"
...
# Ai Assisted Extraction Scope Feature Specification

## Purpose
This document defines the implementation scope for `ai-assisted-extraction-scope` inside the `data-import-ai` module of the Unified Commerce platform.
...
|---|---|
| `products` | Source or supporting table for `ai-assisted-extraction-scope` behavior. |
| `product_variants` | Source or supporting table for `ai-assisted-extraction-scope` behavior. |
| `customers` | Source or supporting table for `ai-assisted-extraction-scope` behavior. |
| `suppliers` | Source or supporting table for `ai-assisted-extraction-scope` behavior. |
| `inventory_balances` | Source or supporting table for `ai-assisted-extraction-scope` behavior. |
| `audit_logs` | Source or supporting table for `ai-assisted-extraction-scope` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `data-import-ai.ai-assisted-extraction-scope.read` | View data and search records. | Yes |
| `data-import-ai.ai-assisted-extraction-scope.create` | Create new records or workflow drafts. | Yes |
| `data-import-ai.ai-assisted-extraction-scope.update` | Modify allowed editable fields. | Yes |
| `data-import-ai.ai-assisted-extraction-scope.approve` | Approve sensitive actions where applicable. | Yes |
| `data-import-ai.ai-assisted-extraction-scope.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class AiAssistedExtractionScopeService : IAiAssistedExtractionScopeService
{
    private readonly IAiAssistedExtractionScopeRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<AiAssistedExtractionScopeDto>> HandleAsync(AiAssistedExtractionScopeRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "data-import-ai.ai-assisted-extraction-scope.manage");
        ...
        return ApiResponse.Success(new AiAssistedExtractionScopeDto());
    }
}
```
...
```ts
export const aiassistedextractionscopeQueryKey = (tenantId: string, outletId?: string) =>
  ["data-import-ai", "ai-assisted-extraction-scope", tenantId, outletId ?? "tenant"] as const;

export function useAiAssistedExtractionScope(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: aiassistedextractionscopeQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/data-import-ai/ai-assisted-extraction-scope"),
    staleTime: 60_000,
  });
}
```
...
- Related module overview: [[../README|Data Import Ai Overview]]
