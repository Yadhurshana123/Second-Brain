---
title: "Ai Assisted Extraction Scope API Specification"
...
# Ai Assisted Extraction Scope API Specification

## API Ownership
The `ai-assisted-extraction-scope` API belongs to the `data-import-ai` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/data-import-ai/ai-assisted-extraction-scope` | `data-import-ai.ai-assisted-extraction-scope.read` |
| Detail | GET | `/api/v1/data-import-ai/ai-assisted-extraction-scope/{id}` | `data-import-ai.ai-assisted-extraction-scope.read` |
| Create | POST | `/api/v1/data-import-ai/ai-assisted-extraction-scope` | `data-import-ai.ai-assisted-extraction-scope.create` |
| Update | PUT | `/api/v1/data-import-ai/ai-assisted-extraction-scope/{id}` | `data-import-ai.ai-assisted-extraction-scope.update` |
| State action | POST | `/api/v1/data-import-ai/ai-assisted-extraction-scope/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/data-import-ai/ai-assisted-extraction-scope
...
{
  "featureKey": "data-import-ai.ai-assisted-extraction-scope",
...
- Access consideration 1: Permission `data-import-ai.ai-assisted-extraction-scope.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `data-import-ai.ai-assisted-extraction-scope.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `data-import-ai.ai-assisted-extraction-scope.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `data-import-ai.ai-assisted-extraction-scope.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `data-import-ai.ai-assisted-extraction-scope.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `data-import-ai.ai-assisted-extraction-scope.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `data-import-ai.ai-assisted-extraction-scope.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `data-import-ai.ai-assisted-extraction-scope.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `data-import-ai.ai-assisted-extraction-scope.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `data-import-ai.ai-assisted-extraction-scope.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `data-import-ai.ai-assisted-extraction-scope.read` may show data, while command permissions must be separately configured per role.
