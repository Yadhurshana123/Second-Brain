---
title: "Tenant Lifecycle API Specification"
...
# Tenant Lifecycle API Specification

## API Ownership
The `tenant-lifecycle` API belongs to the `platform-administration` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/platform-administration/tenant-lifecycle` | `platform-administration.tenant-lifecycle.read` |
| Detail | GET | `/api/v1/platform-administration/tenant-lifecycle/{id}` | `platform-administration.tenant-lifecycle.read` |
| Create | POST | `/api/v1/platform-administration/tenant-lifecycle` | `platform-administration.tenant-lifecycle.create` |
| Update | PUT | `/api/v1/platform-administration/tenant-lifecycle/{id}` | `platform-administration.tenant-lifecycle.update` |
| State action | POST | `/api/v1/platform-administration/tenant-lifecycle/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/platform-administration/tenant-lifecycle
...
{
  "featureKey": "platform-administration.tenant-lifecycle",
...
- Access consideration 1: Permission `platform-administration.tenant-lifecycle.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `platform-administration.tenant-lifecycle.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `platform-administration.tenant-lifecycle.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `platform-administration.tenant-lifecycle.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `platform-administration.tenant-lifecycle.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `platform-administration.tenant-lifecycle.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `platform-administration.tenant-lifecycle.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `platform-administration.tenant-lifecycle.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `platform-administration.tenant-lifecycle.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `platform-administration.tenant-lifecycle.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `platform-administration.tenant-lifecycle.read` may show data, while command permissions must be separately configured per role.
