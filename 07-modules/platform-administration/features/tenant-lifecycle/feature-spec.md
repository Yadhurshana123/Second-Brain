---
title: "Tenant Lifecycle Feature Specification"
...
# Tenant Lifecycle Feature Specification

## Purpose
This document defines the implementation scope for `tenant-lifecycle` inside the `platform-administration` module of the Unified Commerce platform.
...
|---|---|
| `platform_users` | Source or supporting table for `tenant-lifecycle` behavior. |
| `platform_features` | Source or supporting table for `tenant-lifecycle` behavior. |
| `tenant_feature_entitlements` | Source or supporting table for `tenant-lifecycle` behavior. |
| `audit_logs` | Source or supporting table for `tenant-lifecycle` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `platform-administration.tenant-lifecycle.read` | View data and search records. | Yes |
| `platform-administration.tenant-lifecycle.create` | Create new records or workflow drafts. | Yes |
| `platform-administration.tenant-lifecycle.update` | Modify allowed editable fields. | Yes |
| `platform-administration.tenant-lifecycle.approve` | Approve sensitive actions where applicable. | Yes |
| `platform-administration.tenant-lifecycle.void` | Void/cancel/reverse where business rules allow. | Yes |
...
public sealed class TenantLifecycleService : ITenantLifecycleService
{
    ...
    public async Task<ApiResponse<TenantLifecycleDto>> HandleAsync(TenantLifecycleRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "platform-administration.tenant-lifecycle.manage");
        ...
        return ApiResponse.Success(new TenantLifecycleDto());
    }
}
```
...
```ts
export const tenantlifecycleQueryKey = (tenantId: string, outletId?: string) =>
  ["platform-administration", "tenant-lifecycle", tenantId, outletId ?? "tenant"] as const;

export function useTenantLifecycle(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: tenantlifecycleQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/platform-administration/tenant-lifecycle"),
    staleTime: 60_000,
  });
}
```
