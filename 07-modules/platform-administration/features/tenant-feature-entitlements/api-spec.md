---
title: "Tenant Feature Entitlements API Specification"
...
# Tenant Feature Entitlements API Specification

## API Ownership
The `tenant-feature-entitlements` API belongs to the `platform-administration` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/platform-administration/tenant-feature-entitlements` | `platform-administration.tenant-feature-entitlements.read` |
| Detail | GET | `/api/v1/platform-administration/tenant-feature-entitlements/{id}` | `platform-administration.tenant-feature-entitlements.read` |
| Create | POST | `/api/v1/platform-administration/tenant-feature-entitlements` | `platform-administration.tenant-feature-entitlements.create` |
| Update | PUT | `/api/v1/platform-administration/tenant-feature-entitlements/{id}` | `platform-administration.tenant-feature-entitlements.update` |
| State action | POST | `/api/v1/platform-administration/tenant-feature-entitlements/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/platform-administration/tenant-feature-entitlements
...
{
  "featureKey": "platform-administration.tenant-feature-entitlements",
...
- Access consideration 1: Permission `platform-administration.tenant-feature-entitlements.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `platform-administration.tenant-feature-entitlements.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `platform-administration.tenant-feature-entitlements.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `platform-administration.tenant-feature-entitlements.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `platform-administration.tenant-feature-entitlements.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `platform-administration.tenant-feature-entitlements.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `platform-administration.tenant-feature-entitlements.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `platform-administration.tenant-feature-entitlements.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `platform-administration.tenant-feature-entitlements.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `platform-administration.tenant-feature-entitlements.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `platform-administration.tenant-feature-entitlements.read` may show data, while command permissions must be separately configured per role.
