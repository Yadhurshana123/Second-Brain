---
title: "Tenant Feature Entitlements Feature Specification"
...
# Tenant Feature Entitlements Feature Specification

## Purpose
This document defines the implementation scope for `tenant-feature-entitlements` inside the `platform-administration` module of the Unified Commerce platform.
...
|---|---|
| `platform_users` | Source or supporting table for `tenant-feature-entitlements` behavior. |
| `platform_features` | Source or supporting table for `tenant-feature-entitlements` behavior. |
| `tenant_feature_entitlements` | Source or supporting table for `tenant-feature-entitlements` behavior. |
| `audit_logs` | Source or supporting table for `tenant-feature-entitlements` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `platform-administration.tenant-feature-entitlements.read` | View data and search records. | Yes |
| `platform-administration.tenant-feature-entitlements.create` | Create new records or workflow drafts. | Yes |
| `platform-administration.tenant-feature-entitlements.update` | Modify allowed editable fields. | Yes |
| `platform-administration.tenant-feature-entitlements.approve` | Approve sensitive actions where applicable. | Yes |
| `platform-administration.tenant-feature-entitlements.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class TenantFeatureEntitlementsService : ITenantFeatureEntitlementsService
{
    private readonly ITenantFeatureEntitlementsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<TenantFeatureEntitlementsDto>> HandleAsync(TenantFeatureEntitlementsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "platform-administration.tenant-feature-entitlements.manage");
        ...
        return ApiResponse.Success(new TenantFeatureEntitlementsDto());
    }
}
```
...
```ts
export const tenantfeatureentitlementsQueryKey = (tenantId: string, outletId?: string) =>
  ["platform-administration", "tenant-feature-entitlements", tenantId, outletId ?? "tenant"] as const;

export function useTenantFeatureEntitlements(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: tenantfeatureentitlementsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/platform-administration/tenant-feature-entitlements"),
    staleTime: 60_000,
  });
}
```
...
- Related module overview: [[../README|Platform Administration Overview]]
