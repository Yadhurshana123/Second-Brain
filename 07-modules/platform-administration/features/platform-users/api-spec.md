---
title: "Platform Users API Specification"
...
# Platform Users API Specification

## API Ownership
The `platform-users` API belongs to the `platform-administration` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/platform-administration/platform-users` | `platform-administration.platform-users.read` |
| Detail | GET | `/api/v1/platform-administration/platform-users/{id}` | `platform-administration.platform-users.read` |
| Create | POST | `/api/v1/platform-administration/platform-users` | `platform-administration.platform-users.create` |
| Update | PUT | `/api/v1/platform-administration/platform-users/{id}` | `platform-administration.platform-users.update` |
| State action | POST | `/api/v1/platform-administration/platform-users/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/platform-administration/platform-users
...
{
  "featureKey": "platform-administration.platform-users",
...
- Access consideration 1: Permission `platform-administration.platform-users.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `platform-administration.platform-users.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `platform-administration.platform-users.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `platform-administration.platform-users.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `platform-administration.platform-users.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `platform-administration.platform-users.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `platform-administration.platform-users.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `platform-administration.platform-users.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `platform-administration.platform-users.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `platform-administration.platform-users.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `platform-administration.platform-users.read` may show data, while command permissions must be separately configured per role.
