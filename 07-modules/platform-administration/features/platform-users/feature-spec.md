---
title: "Platform Users Feature Specification"
...
# Platform Users Feature Specification

## Purpose
This document defines the implementation scope for `platform-users` inside the `platform-administration` module of the Unified Commerce platform.
...
|---|---|
| `platform_users` | Source or supporting table for `platform-users` behavior. |
| `platform_features` | Source or supporting table for `platform-users` behavior. |
| `tenant_feature_entitlements` | Source or supporting table for `platform-users` behavior. |
| `audit_logs` | Source or supporting table for `platform-users` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `platform-administration.platform-users.read` | View data and search records. | Yes |
| `platform-administration.platform-users.create` | Create new records or workflow drafts. | Yes |
| `platform-administration.platform-users.update` | Modify allowed editable fields. | Yes |
| `platform-administration.platform-users.approve` | Approve sensitive actions where applicable. | Yes |
| `platform-administration.platform-users.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class PlatformUsersService : IPlatformUsersService
{
    private readonly IPlatformUsersRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<PlatformUsersDto>> HandleAsync(PlatformUsersRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "platform-administration.platform-users.manage");
        ...
        return ApiResponse.Success(new PlatformUsersDto());
    }
}
```
...
```ts
export const platformusersQueryKey = (tenantId: string, outletId?: string) =>
  ["platform-administration", "platform-users", tenantId, outletId ?? "tenant"] as const;

export function usePlatformUsers(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: platformusersQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/platform-administration/platform-users"),
    staleTime: 60_000,
  });
}
```
