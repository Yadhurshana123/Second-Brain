...
## Module Purpose
The `platform-administration` module groups related Unified Commerce capabilities and documents how they are implemented across database, backend, API, frontend, security, and cache/storage layers.
...
|---|---|---|---|
| `platform-users` | [[features/platform-users/feature-spec|Spec]] | [[features/platform-users/api-spec|API]] | [[features/platform-users/feature-history|History]] |
| `tenant-feature-entitlements` | [[features/tenant-feature-entitlements/feature-spec|Spec]] | [[features/tenant-feature-entitlements/api-spec|API]] | [[features/tenant-feature-entitlements/feature-history|History]] |
| `tenant-lifecycle` | [[features/tenant-lifecycle/feature-spec|Spec]] | [[features/tenant-lifecycle/api-spec|API]] | [[features/tenant-lifecycle/feature-history|History]] |

## Database Tables Commonly Used
| Table | Purpose in module |
|---|---|
| `platform_users` | Supports `platform-administration` module workflows and tenant-scoped persistence. |
| `platform_features` | Supports `platform-administration` module workflows and tenant-scoped persistence. |
| `tenant_feature_entitlements` | Supports `platform-administration` module workflows and tenant-scoped persistence. |
| `audit_logs` | Supports `platform-administration` module workflows and tenant-scoped persistence. |
...
|---|---|---|
| API | `POS.API/Modules/PlatformAdministration` | Controllers, request models, response models. |
| Application | `POS.Application/Modules/PlatformAdministration` | Services, validators, interfaces, and `Dtos/`. |
| Domain | `POS.Domain/Modules/PlatformAdministration` | Pure rules and entities where needed. |
| Infrastructure | `POS.Infrastructure/Repositories/PlatformAdministration` | EF Core repositories and persistence logic. |
...
- Access consideration 34: Permission `platform-administration.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 35: Permission `platform-administration.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 36: Permission `platform-administration.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 37: Permission `platform-administration.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 38: Permission `platform-administration.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 39: Permission `platform-administration.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 40: Permission `platform-administration.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 41: Permission `platform-administration.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 42: Permission `platform-administration.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 43: Permission `platform-administration.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 44: Permission `platform-administration.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 45: Permission `platform-administration.module.read` may show data, while command permissions must be separately configured per role.
