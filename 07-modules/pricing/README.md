...
## Module Purpose
The `pricing` module groups related Unified Commerce capabilities and documents how they are implemented across database, backend, API, frontend, security, and cache/storage layers.
...
|---|---|---|---|
| `outlet-price-overrides` | [[features/outlet-price-overrides/feature-spec|Spec]] | [[features/outlet-price-overrides/api-spec|API]] | [[features/outlet-price-overrides/feature-history|History]] |
| `price-list-items` | [[features/price-list-items/feature-spec|Spec]] | [[features/price-list-items/api-spec|API]] | [[features/price-list-items/feature-history|History]] |
| `price-lists` | [[features/price-lists/feature-spec|Spec]] | [[features/price-lists/api-spec|API]] | [[features/price-lists/feature-history|History]] |

## Database Tables Commonly Used
| Table | Purpose in module |
|---|---|
| `price_lists` | Supports `pricing` module workflows and tenant-scoped persistence. |
| `price_list_items` | Supports `pricing` module workflows and tenant-scoped persistence. |
| `discount_policies` | Supports `pricing` module workflows and tenant-scoped persistence. |
| `discount_applications` | Supports `pricing` module workflows and tenant-scoped persistence. |
| `coupons` | Supports `pricing` module workflows and tenant-scoped persistence. |
...
|---|---|---|
| API | `POS.API/Modules/Pricing` | Controllers, request models, response models. |
| Application | `POS.Application/Modules/Pricing` | Services, validators, interfaces, and `Dtos/`. |
| Domain | `POS.Domain/Modules/Pricing` | Pure rules and entities where needed. |
| Infrastructure | `POS.Infrastructure/Repositories/Pricing` | EF Core repositories and persistence logic. |
...
- Access consideration 33: Permission `pricing.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 34: Permission `pricing.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 35: Permission `pricing.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 36: Permission `pricing.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 37: Permission `pricing.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 38: Permission `pricing.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 39: Permission `pricing.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 40: Permission `pricing.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 41: Permission `pricing.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 42: Permission `pricing.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 43: Permission `pricing.module.read` may show data, while command permissions must be separately configured per role.
- Access consideration 44: Permission `pricing.module.read` may show data, while command permissions must be separately configured per role.
