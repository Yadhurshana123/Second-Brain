---
title: "Price List Items Feature Specification"
...
# Price List Items Feature Specification

## Purpose
This document defines the implementation scope for `price-list-items` inside the `pricing` module of the Unified Commerce platform.
...
|---|---|
| `price_lists` | Source or supporting table for `price-list-items` behavior. |
| `price_list_items` | Source or supporting table for `price-list-items` behavior. |
| `discount_policies` | Source or supporting table for `price-list-items` behavior. |
| `discount_applications` | Source or supporting table for `price-list-items` behavior. |
| `coupons` | Source or supporting table for `price-list-items` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `pricing.price-list-items.read` | View data and search records. | Yes |
| `pricing.price-list-items.create` | Create new records or workflow drafts. | Yes |
| `pricing.price-list-items.update` | Modify allowed editable fields. | Yes |
| `pricing.price-list-items.approve` | Approve sensitive actions where applicable. | Yes |
| `pricing.price-list-items.void` | Void/cancel/reverse where business rules allow. | Yes |
...
public sealed class PriceListItemsService : IPriceListItemsService
{
    ...
    public async Task<ApiResponse<PriceListItemsDto>> HandleAsync(PriceListItemsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "pricing.price-list-items.manage");
        ...
        return ApiResponse.Success(new PriceListItemsDto());
    }
}
```
...
```ts
export const pricelistitemsQueryKey = (tenantId: string, outletId?: string) =>
  ["pricing", "price-list-items", tenantId, outletId ?? "tenant"] as const;

export function usePriceListItems(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: pricelistitemsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/pricing/price-list-items"),
    staleTime: 60_000,
  });
}
```
