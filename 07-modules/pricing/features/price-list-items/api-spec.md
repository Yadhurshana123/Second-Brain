---
title: "Price List Items API Specification"
...
# Price List Items API Specification

## API Ownership
The `price-list-items` API belongs to the `pricing` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/pricing/price-list-items` | `pricing.price-list-items.read` |
| Detail | GET | `/api/v1/pricing/price-list-items/{id}` | `pricing.price-list-items.read` |
| Create | POST | `/api/v1/pricing/price-list-items` | `pricing.price-list-items.create` |
| Update | PUT | `/api/v1/pricing/price-list-items/{id}` | `pricing.price-list-items.update` |
| State action | POST | `/api/v1/pricing/price-list-items/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/pricing/price-list-items
...
{
  "featureKey": "pricing.price-list-items",
...
- Access consideration 1: Permission `pricing.price-list-items.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `pricing.price-list-items.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `pricing.price-list-items.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `pricing.price-list-items.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `pricing.price-list-items.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `pricing.price-list-items.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `pricing.price-list-items.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `pricing.price-list-items.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `pricing.price-list-items.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `pricing.price-list-items.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `pricing.price-list-items.read` may show data, while command permissions must be separately configured per role.
