---
title: "Price Lists Feature Specification"
...
# Price Lists Feature Specification

## Purpose
This document defines the implementation scope for `price-lists` inside the `pricing` module of the Unified Commerce platform.
...
|---|---|
| `price_lists` | Source or supporting table for `price-lists` behavior. |
| `price_list_items` | Source or supporting table for `price-lists` behavior. |
| `discount_policies` | Source or supporting table for `price-lists` behavior. |
| `discount_applications` | Source or supporting table for `price-lists` behavior. |
| `coupons` | Source or supporting table for `price-lists` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `pricing.price-lists.read` | View data and search records. | Yes |
| `pricing.price-lists.create` | Create new records or workflow drafts. | Yes |
| `pricing.price-lists.update` | Modify allowed editable fields. | Yes |
| `pricing.price-lists.approve` | Approve sensitive actions where applicable. | Yes |
| `pricing.price-lists.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class PriceListsService : IPriceListsService
{
    private readonly IPriceListsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<PriceListsDto>> HandleAsync(PriceListsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "pricing.price-lists.manage");
        ...
        return ApiResponse.Success(new PriceListsDto());
    }
}
```
...
```ts
export const pricelistsQueryKey = (tenantId: string, outletId?: string) =>
  ["pricing", "price-lists", tenantId, outletId ?? "tenant"] as const;

export function usePriceLists(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: pricelistsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/pricing/price-lists"),
    staleTime: 60_000,
  });
}
```
