---
title: "Outlet Price Overrides Feature Specification"
...
# Outlet Price Overrides Feature Specification

## Purpose
This document defines the implementation scope for `outlet-price-overrides` inside the `pricing` module of the Unified Commerce platform.
...
|---|---|
| `price_lists` | Source or supporting table for `outlet-price-overrides` behavior. |
| `price_list_items` | Source or supporting table for `outlet-price-overrides` behavior. |
| `discount_policies` | Source or supporting table for `outlet-price-overrides` behavior. |
| `discount_applications` | Source or supporting table for `outlet-price-overrides` behavior. |
| `coupons` | Source or supporting table for `outlet-price-overrides` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `pricing.outlet-price-overrides.read` | View data and search records. | Yes |
| `pricing.outlet-price-overrides.create` | Create new records or workflow drafts. | Yes |
| `pricing.outlet-price-overrides.update` | Modify allowed editable fields. | Yes |
| `pricing.outlet-price-overrides.approve` | Approve sensitive actions where applicable. | Yes |
| `pricing.outlet-price-overrides.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class OutletPriceOverridesService : IOutletPriceOverridesService
{
    private readonly IOutletPriceOverridesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<OutletPriceOverridesDto>> HandleAsync(OutletPriceOverridesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "pricing.outlet-price-overrides.manage");
        ...
        return ApiResponse.Success(new OutletPriceOverridesDto());
    }
}
```
...
```ts
export const outletpriceoverridesQueryKey = (tenantId: string, outletId?: string) =>
  ["pricing", "outlet-price-overrides", tenantId, outletId ?? "tenant"] as const;

export function useOutletPriceOverrides(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: outletpriceoverridesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/pricing/outlet-price-overrides"),
    staleTime: 60_000,
  });
}
```
...
- Related module overview: [[../README|Pricing Overview]]
