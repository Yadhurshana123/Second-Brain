---
title: "Payments Feature Specification"
...
## Purpose
This document defines the implementation scope for `payments` inside the `payments` module of the Unified Commerce platform.
...
|---|---|
| `payment_method_types` | Source or supporting table for `payments` behavior. |
| `tenant_payment_methods` | Source or supporting table for `payments` behavior. |
| `payment_provider_configs` | Source or supporting table for `payments` behavior. |
| `payments` | Source or supporting table for `payments` behavior. |
| `payment_transactions` | Source or supporting table for `payments` behavior. |
| `sale_payment_allocations` | Source or supporting table for `payments` behavior. |
| `order_payment_allocations` | Source or supporting table for `payments` behavior. |
| `refunds` | Source or supporting table for `payments` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `payments.payments.read` | View data and search records. | Yes |
| `payments.payments.create` | Create new records or workflow drafts. | Yes |
| `payments.payments.update` | Modify allowed editable fields. | Yes |
| `payments.payments.approve` | Approve sensitive actions where applicable. | Yes |
| `payments.payments.void` | Void/cancel/reverse where business rules allow. | Yes |
...
public sealed class PaymentsService : IPaymentsService
{
    ...
    public async Task<ApiResponse<PaymentsDto>> HandleAsync(PaymentsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "payments.payments.manage");
        ...
        return ApiResponse.Success(new PaymentsDto());
    }
}
```
...
```ts
export const paymentsQueryKey = (tenantId: string, outletId?: string) =>
  ["payments", "payments", tenantId, outletId ?? "tenant"] as const;

export function usePayments(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: paymentsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/payments/payments"),
    staleTime: 60_000,
  });
}
```
