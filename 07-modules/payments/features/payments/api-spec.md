...
## API Ownership
The `payments` API belongs to the `payments` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/payments/payments` | `payments.payments.read` |
| Detail | GET | `/api/v1/payments/payments/{id}` | `payments.payments.read` |
| Create | POST | `/api/v1/payments/payments` | `payments.payments.create` |
| Update | PUT | `/api/v1/payments/payments/{id}` | `payments.payments.update` |
| State action | POST | `/api/v1/payments/payments/{id}/actions/{action}` | Action-specific permission |
...
- Access consideration 1: Permission `payments.payments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `payments.payments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `payments.payments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `payments.payments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `payments.payments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `payments.payments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `payments.payments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `payments.payments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `payments.payments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `payments.payments.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `payments.payments.read` may show data, while command permissions must be separately configured per role.
