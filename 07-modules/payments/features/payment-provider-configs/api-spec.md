---
title: "Payment Provider Configs API Specification"
...
# Payment Provider Configs API Specification

## API Ownership
The `payment-provider-configs` API belongs to the `payments` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/payments/payment-provider-configs` | `payments.payment-provider-configs.read` |
| Detail | GET | `/api/v1/payments/payment-provider-configs/{id}` | `payments.payment-provider-configs.read` |
| Create | POST | `/api/v1/payments/payment-provider-configs` | `payments.payment-provider-configs.create` |
| Update | PUT | `/api/v1/payments/payment-provider-configs/{id}` | `payments.payment-provider-configs.update` |
| State action | POST | `/api/v1/payments/payment-provider-configs/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/payments/payment-provider-configs
...
{
  "featureKey": "payments.payment-provider-configs",
...
- Access consideration 1: Permission `payments.payment-provider-configs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `payments.payment-provider-configs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `payments.payment-provider-configs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `payments.payment-provider-configs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `payments.payment-provider-configs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `payments.payment-provider-configs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `payments.payment-provider-configs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `payments.payment-provider-configs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `payments.payment-provider-configs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `payments.payment-provider-configs.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `payments.payment-provider-configs.read` may show data, while command permissions must be separately configured per role.
