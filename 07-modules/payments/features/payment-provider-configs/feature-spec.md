---
title: "Payment Provider Configs Feature Specification"
...
# Payment Provider Configs Feature Specification

## Purpose
This document defines the implementation scope for `payment-provider-configs` inside the `payments` module of the Unified Commerce platform.
...
|---|---|
| `payment_method_types` | Source or supporting table for `payment-provider-configs` behavior. |
| `tenant_payment_methods` | Source or supporting table for `payment-provider-configs` behavior. |
| `payment_provider_configs` | Source or supporting table for `payment-provider-configs` behavior. |
| `payments` | Source or supporting table for `payment-provider-configs` behavior. |
| `payment_transactions` | Source or supporting table for `payment-provider-configs` behavior. |
| `sale_payment_allocations` | Source or supporting table for `payment-provider-configs` behavior. |
| `order_payment_allocations` | Source or supporting table for `payment-provider-configs` behavior. |
| `refunds` | Source or supporting table for `payment-provider-configs` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `payments.payment-provider-configs.read` | View data and search records. | Yes |
| `payments.payment-provider-configs.create` | Create new records or workflow drafts. | Yes |
| `payments.payment-provider-configs.update` | Modify allowed editable fields. | Yes |
| `payments.payment-provider-configs.approve` | Approve sensitive actions where applicable. | Yes |
| `payments.payment-provider-configs.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class PaymentProviderConfigsService : IPaymentProviderConfigsService
{
    private readonly IPaymentProviderConfigsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<PaymentProviderConfigsDto>> HandleAsync(PaymentProviderConfigsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "payments.payment-provider-configs.manage");
        ...
        return ApiResponse.Success(new PaymentProviderConfigsDto());
    }
}
```
...
```ts
export const paymentproviderconfigsQueryKey = (tenantId: string, outletId?: string) =>
  ["payments", "payment-provider-configs", tenantId, outletId ?? "tenant"] as const;

export function usePaymentProviderConfigs(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: paymentproviderconfigsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/payments/payment-provider-configs"),
    staleTime: 60_000,
  });
}
```
...
- Related module overview: [[../README|Payments Overview]]
