---
title: "Tenant Payment Methods Feature Specification"
...
# Tenant Payment Methods Feature Specification

## Purpose
This document defines the implementation scope for `tenant-payment-methods` inside the `payments` module of the Unified Commerce platform.
...
|---|---|
| `payment_method_types` | Source or supporting table for `tenant-payment-methods` behavior. |
| `tenant_payment_methods` | Source or supporting table for `tenant-payment-methods` behavior. |
| `payment_provider_configs` | Source or supporting table for `tenant-payment-methods` behavior. |
| `payments` | Source or supporting table for `tenant-payment-methods` behavior. |
| `payment_transactions` | Source or supporting table for `tenant-payment-methods` behavior. |
| `sale_payment_allocations` | Source or supporting table for `tenant-payment-methods` behavior. |
| `order_payment_allocations` | Source or supporting table for `tenant-payment-methods` behavior. |
| `refunds` | Source or supporting table for `tenant-payment-methods` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `payments.tenant-payment-methods.read` | View data and search records. | Yes |
| `payments.tenant-payment-methods.create` | Create new records or workflow drafts. | Yes |
| `payments.tenant-payment-methods.update` | Modify allowed editable fields. | Yes |
| `payments.tenant-payment-methods.approve` | Approve sensitive actions where applicable. | Yes |
| `payments.tenant-payment-methods.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class TenantPaymentMethodsService : ITenantPaymentMethodsService
{
    private readonly ITenantPaymentMethodsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<TenantPaymentMethodsDto>> HandleAsync(TenantPaymentMethodsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "payments.tenant-payment-methods.manage");
        ...
        return ApiResponse.Success(new TenantPaymentMethodsDto());
    }
}
```
...
```ts
export const tenantpaymentmethodsQueryKey = (tenantId: string, outletId?: string) =>
  ["payments", "tenant-payment-methods", tenantId, outletId ?? "tenant"] as const;

export function useTenantPaymentMethods(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: tenantpaymentmethodsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/payments/tenant-payment-methods"),
    staleTime: 60_000,
  });
}
```
