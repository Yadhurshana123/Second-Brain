---
title: "Tenant Payment Methods API Specification"
...
# Tenant Payment Methods API Specification

## API Ownership
The `tenant-payment-methods` API belongs to the `payments` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/payments/tenant-payment-methods` | `payments.tenant-payment-methods.read` |
| Detail | GET | `/api/v1/payments/tenant-payment-methods/{id}` | `payments.tenant-payment-methods.read` |
| Create | POST | `/api/v1/payments/tenant-payment-methods` | `payments.tenant-payment-methods.create` |
| Update | PUT | `/api/v1/payments/tenant-payment-methods/{id}` | `payments.tenant-payment-methods.update` |
| State action | POST | `/api/v1/payments/tenant-payment-methods/{id}/actions/{action}` | Action-specific permission |
...
## Request Example
```http
POST /api/v1/payments/tenant-payment-methods
...
{
  "featureKey": "payments.tenant-payment-methods",
...
- Access consideration 1: Permission `payments.tenant-payment-methods.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `payments.tenant-payment-methods.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `payments.tenant-payment-methods.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `payments.tenant-payment-methods.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `payments.tenant-payment-methods.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `payments.tenant-payment-methods.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `payments.tenant-payment-methods.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `payments.tenant-payment-methods.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `payments.tenant-payment-methods.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `payments.tenant-payment-methods.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `payments.tenant-payment-methods.read` may show data, while command permissions must be separately configured per role.
