---
title: "Payment Method Types Feature Specification"
...
# Payment Method Types Feature Specification

## Purpose
This document defines the implementation scope for `payment-method-types` inside the `payments` module of the Unified Commerce platform.
...
|---|---|
| `payment_method_types` | Source or supporting table for `payment-method-types` behavior. |
| `tenant_payment_methods` | Source or supporting table for `payment-method-types` behavior. |
| `payment_provider_configs` | Source or supporting table for `payment-method-types` behavior. |
| `payments` | Source or supporting table for `payment-method-types` behavior. |
| `payment_transactions` | Source or supporting table for `payment-method-types` behavior. |
| `sale_payment_allocations` | Source or supporting table for `payment-method-types` behavior. |
| `order_payment_allocations` | Source or supporting table for `payment-method-types` behavior. |
| `refunds` | Source or supporting table for `payment-method-types` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `payments.payment-method-types.read` | View data and search records. | Yes |
| `payments.payment-method-types.create` | Create new records or workflow drafts. | Yes |
| `payments.payment-method-types.update` | Modify allowed editable fields. | Yes |
| `payments.payment-method-types.approve` | Approve sensitive actions where applicable. | Yes |
| `payments.payment-method-types.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class PaymentMethodTypesService : IPaymentMethodTypesService
{
    private readonly IPaymentMethodTypesRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<PaymentMethodTypesDto>> HandleAsync(PaymentMethodTypesRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "payments.payment-method-types.manage");
        ...
        return ApiResponse.Success(new PaymentMethodTypesDto());
    }
}
```
...
```ts
export const paymentmethodtypesQueryKey = (tenantId: string, outletId?: string) =>
  ["payments", "payment-method-types", tenantId, outletId ?? "tenant"] as const;

export function usePaymentMethodTypes(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: paymentmethodtypesQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/payments/payment-method-types"),
    staleTime: 60_000,
  });
}
```
