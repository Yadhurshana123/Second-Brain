---
title: "Payment Transactions Feature Specification"
...
# Payment Transactions Feature Specification

## Purpose
This document defines the implementation scope for `payment-transactions` inside the `payments` module of the Unified Commerce platform.
...
|---|---|
| `payment_method_types` | Source or supporting table for `payment-transactions` behavior. |
| `tenant_payment_methods` | Source or supporting table for `payment-transactions` behavior. |
| `payment_provider_configs` | Source or supporting table for `payment-transactions` behavior. |
| `payments` | Source or supporting table for `payment-transactions` behavior. |
| `payment_transactions` | Source or supporting table for `payment-transactions` behavior. |
| `sale_payment_allocations` | Source or supporting table for `payment-transactions` behavior. |
| `order_payment_allocations` | Source or supporting table for `payment-transactions` behavior. |
| `refunds` | Source or supporting table for `payment-transactions` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `payments.payment-transactions.read` | View data and search records. | Yes |
| `payments.payment-transactions.create` | Create new records or workflow drafts. | Yes |
| `payments.payment-transactions.update` | Modify allowed editable fields. | Yes |
| `payments.payment-transactions.approve` | Approve sensitive actions where applicable. | Yes |
| `payments.payment-transactions.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class PaymentTransactionsService : IPaymentTransactionsService
{
    private readonly IPaymentTransactionsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<PaymentTransactionsDto>> HandleAsync(PaymentTransactionsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "payments.payment-transactions.manage");
        ...
        return ApiResponse.Success(new PaymentTransactionsDto());
    }
}
```
...
```ts
export const paymenttransactionsQueryKey = (tenantId: string, outletId?: string) =>
  ["payments", "payment-transactions", tenantId, outletId ?? "tenant"] as const;

export function usePaymentTransactions(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: paymenttransactionsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/payments/payment-transactions"),
    staleTime: 60_000,
  });
}
```
