---
title: "Payment Transactions API Specification"
...
## API Ownership
The `payment-transactions` API belongs to the `payments` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/payments/payment-transactions` | `payments.payment-transactions.read` |
| Detail | GET | `/api/v1/payments/payment-transactions/{id}` | `payments.payment-transactions.read` |
| Create | POST | `/api/v1/payments/payment-transactions` | `payments.payment-transactions.create` |
| Update | PUT | `/api/v1/payments/payment-transactions/{id}` | `payments.payment-transactions.update` |
| State action | POST | `/api/v1/payments/payment-transactions/{id}/actions/{action}` | Action-specific permission |
...
{
  "featureKey": "payments.payment-transactions",
...
- Access consideration 1: Permission `payments.payment-transactions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `payments.payment-transactions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `payments.payment-transactions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `payments.payment-transactions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `payments.payment-transactions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `payments.payment-transactions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `payments.payment-transactions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `payments.payment-transactions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `payments.payment-transactions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `payments.payment-transactions.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `payments.payment-transactions.read` may show data, while command permissions must be separately configured per role.
