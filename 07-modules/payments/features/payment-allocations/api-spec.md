---
title: "Payment Allocations API Specification"
...
# Payment Allocations API Specification

## API Ownership
The `payment-allocations` API belongs to the `payments` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/payments/payment-allocations` | `payments.payment-allocations.read` |
| Detail | GET | `/api/v1/payments/payment-allocations/{id}` | `payments.payment-allocations.read` |
| Create | POST | `/api/v1/payments/payment-allocations` | `payments.payment-allocations.create` |
| Update | PUT | `/api/v1/payments/payment-allocations/{id}` | `payments.payment-allocations.update` |
| State action | POST | `/api/v1/payments/payment-allocations/{id}/actions/{action}` | Action-specific permission |
...
{
  "featureKey": "payments.payment-allocations",
...
- Access consideration 1: Permission `payments.payment-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `payments.payment-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `payments.payment-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `payments.payment-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `payments.payment-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `payments.payment-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `payments.payment-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `payments.payment-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `payments.payment-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `payments.payment-allocations.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `payments.payment-allocations.read` may show data, while command permissions must be separately configured per role.
