---
title: "Payment Allocations Feature Specification"
...
# Payment Allocations Feature Specification

## Purpose
This document defines the implementation scope for `payment-allocations` inside the `payments` module of the Unified Commerce platform.
...
|---|---|
| `payment_method_types` | Source or supporting table for `payment-allocations` behavior. |
| `tenant_payment_methods` | Source or supporting table for `payment-allocations` behavior. |
| `payment_provider_configs` | Source or supporting table for `payment-allocations` behavior. |
| `payments` | Source or supporting table for `payment-allocations` behavior. |
| `payment_transactions` | Source or supporting table for `payment-allocations` behavior. |
| `sale_payment_allocations` | Source or supporting table for `payment-allocations` behavior. |
| `order_payment_allocations` | Source or supporting table for `payment-allocations` behavior. |
| `refunds` | Source or supporting table for `payment-allocations` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `payments.payment-allocations.read` | View data and search records. | Yes |
| `payments.payment-allocations.create` | Create new records or workflow drafts. | Yes |
| `payments.payment-allocations.update` | Modify allowed editable fields. | Yes |
| `payments.payment-allocations.approve` | Approve sensitive actions where applicable. | Yes |
| `payments.payment-allocations.void` | Void/cancel/reverse where business rules allow. | Yes |
...
```csharp
public sealed class PaymentAllocationsService : IPaymentAllocationsService
{
    private readonly IPaymentAllocationsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<PaymentAllocationsDto>> HandleAsync(PaymentAllocationsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "payments.payment-allocations.manage");
        ...
        return ApiResponse.Success(new PaymentAllocationsDto());
    }
}
```
...
```ts
export const paymentallocationsQueryKey = (tenantId: string, outletId?: string) =>
  ["payments", "payment-allocations", tenantId, outletId ?? "tenant"] as const;

export function usePaymentAllocations(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: paymentallocationsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/payments/payment-allocations"),
    staleTime: 60_000,
  });
}
```
