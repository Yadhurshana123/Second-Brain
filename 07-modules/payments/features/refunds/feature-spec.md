---
title: "Refunds Feature Specification"
...
# Refunds Feature Specification

## Purpose
This document defines the implementation scope for `refunds` inside the `payments` module of the Unified Commerce platform.
...
|---|---|
| `payment_method_types` | Source or supporting table for `refunds` behavior. |
| `tenant_payment_methods` | Source or supporting table for `refunds` behavior. |
| `payment_provider_configs` | Source or supporting table for `refunds` behavior. |
| `payments` | Source or supporting table for `refunds` behavior. |
| `payment_transactions` | Source or supporting table for `refunds` behavior. |
| `sale_payment_allocations` | Source or supporting table for `refunds` behavior. |
| `order_payment_allocations` | Source or supporting table for `refunds` behavior. |
| `refunds` | Source or supporting table for `refunds` behavior. |

## Permission Model
| Permission | Meaning | Configurable by tenant? |
|---|---|---|
| `payments.refunds.read` | View data and search records. | Yes |
| `payments.refunds.create` | Create new records or workflow drafts. | Yes |
| `payments.refunds.update` | Modify allowed editable fields. | Yes |
| `payments.refunds.approve` | Approve sensitive actions where applicable. | Yes |
| `payments.refunds.void` | Void/cancel/reverse where business rules allow. | Yes |
...
{
    private readonly IRefundsRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAccessDecisionService _access;

    public async Task<ApiResponse<RefundsDto>> HandleAsync(RefundsRequest request, UserContext actor)
    {
        await _access.RequireAsync(actor, "payments.refunds.manage");
        ...
        return ApiResponse.Success(new RefundsDto());
    }
}
```
...
```ts
export const refundsQueryKey = (tenantId: string, outletId?: string) =>
  ["payments", "refunds", tenantId, outletId ?? "tenant"] as const;

export function useRefunds(tenantId: string, outletId?: string) {
  return useQuery({
    queryKey: refundsQueryKey(tenantId, outletId),
    queryFn: () => api.get("/api/v1/payments/refunds"),
    staleTime: 60_000,
  });
}
```
