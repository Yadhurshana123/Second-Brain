---
title: "Refunds API Specification"
...
# Refunds API Specification

## API Ownership
The `refunds` API belongs to the `payments` module and must be implemented through ASP.NET Core controllers, application services, validators, repositories, and Unit of Work.
...
|---|---|---|---|
| Search/list | GET | `/api/v1/payments/refunds` | `payments.refunds.read` |
| Detail | GET | `/api/v1/payments/refunds/{id}` | `payments.refunds.read` |
| Create | POST | `/api/v1/payments/refunds` | `payments.refunds.create` |
| Update | PUT | `/api/v1/payments/refunds/{id}` | `payments.refunds.update` |
| State action | POST | `/api/v1/payments/refunds/{id}/actions/{action}` | Action-specific permission |
...
- Access consideration 1: Permission `payments.refunds.read` may show data, while command permissions must be separately configured per role.
- Access consideration 2: Permission `payments.refunds.read` may show data, while command permissions must be separately configured per role.
- Access consideration 3: Permission `payments.refunds.read` may show data, while command permissions must be separately configured per role.
- Access consideration 4: Permission `payments.refunds.read` may show data, while command permissions must be separately configured per role.
- Access consideration 5: Permission `payments.refunds.read` may show data, while command permissions must be separately configured per role.
- Access consideration 6: Permission `payments.refunds.read` may show data, while command permissions must be separately configured per role.
- Access consideration 7: Permission `payments.refunds.read` may show data, while command permissions must be separately configured per role.
- Access consideration 8: Permission `payments.refunds.read` may show data, while command permissions must be separately configured per role.
- Access consideration 9: Permission `payments.refunds.read` may show data, while command permissions must be separately configured per role.
- Access consideration 10: Permission `payments.refunds.read` may show data, while command permissions must be separately configured per role.
- Access consideration 11: Permission `payments.refunds.read` may show data, while command permissions must be separately configured per role.
