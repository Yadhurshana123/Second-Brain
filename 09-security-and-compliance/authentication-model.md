---
title: Authentication Model with JWT Security
...
document_id: SEC-AUTHN
last_updated: 2026-05-11
tags: [authentication, jwt, identity]
---

# Authentication Model with JWT Security

## Purpose
This document defines platform user, tenant staff, outlet staff, and customer authentication using secure credentials, account status checks, JWT issuance, refresh-session behavior, and tenant-aware claims.
...
|---|---|
| `auth.login` | Must be checked by backend services before the corresponding operation is executed. |
| `auth.refresh` | Must be checked by backend services before the corresponding operation is executed. |
| `auth.logout` | Must be checked by backend services before the corresponding operation is executed. |
| `auth.mfa.verify` | Must be checked by backend services before the corresponding operation is executed. |
...
|---|---|---|
| `POST` | `/api/v1/auth/login` | Issue JWT after identity validation. |
...
sequenceDiagram
  ...
  Client->>AuthAPI: credentials + tenant/outlet hint
  AuthAPI->>AuthService: validate identity/status
  AuthService->>TokenService: create JWT claims
  TokenService-->>Client: access token + refresh metadata
```
