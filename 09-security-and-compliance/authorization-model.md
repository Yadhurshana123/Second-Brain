...
document_id: SEC-AUTHZ
last_updated: 2026-05-11
tags: [authorization, rbac, features]
---

# Authorization Model

## Purpose
This document defines backend-enforced authorization using tenant feature entitlements, role feature assignments, role permissions, outlet role assignments, runtime feature flags, and action-level permission codes.
...
|---|---|
| `rbac.role.manage` | Must be checked by backend services before the corresponding operation is executed. |
| `rbac.permission.assign` | Must be checked by backend services before the corresponding operation is executed. |
| `feature.entitlement.read` | Must be checked by backend services before the corresponding operation is executed. |
| `feature.flag.manage` | Must be checked by backend services before the corresponding operation is executed. |
...
|---|---|---|
| `GET` | `/api/v1/access/effective-permissions` | Read actor effective rights. |
| `POST` | `/api/v1/roles/{id}/permissions` | Assign permission to role. |

```http
GET /api/v1/access/effective-permissions HTTP/1.1
...
flowchart LR
  ...
  R --> F[Role feature assignments]
  F --> T[Tenant entitlement]
  P --> A[Allowed action]
  T --> A
```
