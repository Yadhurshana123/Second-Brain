...
document_id: SEC-PAYMENT
last_updated: 2026-05-11
tags: [payment, refund, gateway]
---

# Payment Security Rules

## Purpose
This document defines payment and refund security controls for cash, card, QR, wallet, bank transfer, gateway-backed methods, secret references, allocations, idempotency, and audit.
...
|---|---|
| `payment_method_types` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `tenant_payment_methods` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `payment_provider_configs` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `payments` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `payment_transactions` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `sale_payment_allocations` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `order_payment_allocations` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
...
|---|---|
| `payment.capture` | Must be checked by backend services before the corresponding operation is executed. |
| `payment.refund` | Must be checked by backend services before the corresponding operation is executed. |
| `payment.method.manage` | Must be checked by backend services before the corresponding operation is executed. |
| `payment.gateway.configure` | Must be checked by backend services before the corresponding operation is executed. |
...
|---|---|---|
| `POST` | `/api/v1/payments` | Record or initiate payment. |
| `POST` | `/api/v1/refunds` | Create controlled refund. |
...
```mermaid
flowchart TD
  A[Payment request] --> B[Permission + feature check]
  B --> C[Idempotency check]
  C --> D[Provider/manual validation]
  D --> E[payments row]
  E --> F[allocation/refund/audit]
```
