...
document_id: SEC-AUDIT
last_updated: 2026-05-11
tags: [audit, traceability, controls]
---

# Audit Requirements

## Purpose
This document defines when the system must create immutable audit records for platform, tenant, outlet, POS, e-commerce, payment, stock, security, and configuration actions.
...
| `audit.log.export` | Must be checked by backend services before the corresponding operation is executed. |
| `platform.audit.read` | Must be checked by backend services before the corresponding operation is executed. |
| `tenant.audit.read` | Must be checked by backend services before the corresponding operation is executed. |
...
|---|---|---|
| `GET` | `/api/v1/audit-logs` | Filter audit events. |
| `POST` | `/api/v1/audit-logs/export` | Export permitted audit result. |
...
sequenceDiagram
  ...
  participant Service
  participant Audit
  UI->>API: Sensitive action
  API->>Service: Validate tenant, permission, feature
  Service->>Audit: Append audit_logs row
  Audit-->>Service: immutable event id
```
