...
## Purpose
This document provides a practical security review structure for code reviews, feature acceptance, API review, frontend review, database review, and release readiness.
...
|---|---|
| `all tenant-owned tables` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `access-control tables` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `audit tables` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `offline sync tables` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `payment tables` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `session tables` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |

## Required permissions and feature gates
| Permission / gate | Usage rule |
|---|---|
| `security.review.perform` | Must be checked by backend services before the corresponding operation is executed. |
| `security.review.approve` | Must be checked by backend services before the corresponding operation is executed. |
| `release.security.signoff` | Must be checked by backend services before the corresponding operation is executed. |
...
|---|---|---|
| `GET` | `/api/v1/security/review-scope` | Retrieve review scope. |
| `POST` | `/api/v1/security/review-results` | Store review result if enabled. |

```http
GET /api/v1/security/review-scope HTTP/1.1
...
```mermaid
flowchart TD
  A[Feature ready] --> B[API security review]
  B --> C[Backend authority review]
  C --> D[Data isolation review]
  D --> E[Frontend leakage review]
  E --> F[Approval or remediation]
```
...
- Implementation consideration 1: keep tenant_id, actor identity, permission code, feature key, and request correlation visible in logs for this control area.
- Implementation consideration 2: keep tenant_id, actor identity, permission code, feature key, and request correlation visible in logs for this control area.
