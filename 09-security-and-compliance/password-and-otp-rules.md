...
document_id: SEC-PASSWORD-OTP
last_updated: 2026-05-11
tags: [password, otp, mfa]
---

# Password and OTP Rules

## Purpose
This document defines password hashing, password reset, OTP issuance, OTP verification, rate limiting, resend limits, abuse blocking, and retention for tenant users and customer auth accounts.
...
| `otp_channels` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `otp_purposes` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
...
|---|---|
| `auth.password.change` | Must be checked by backend services before the corresponding operation is executed. |
| `auth.password.reset` | Must be checked by backend services before the corresponding operation is executed. |
| `auth.otp.issue` | Must be checked by backend services before the corresponding operation is executed. |
| `auth.otp.verify` | Must be checked by backend services before the corresponding operation is executed. |
...
sequenceDiagram
  ...
  participant AuthAPI
  participant OTP
  User->>AuthAPI: request OTP
  AuthAPI->>OTP: create hashed OTP record
  User->>AuthAPI: submit OTP
  AuthAPI->>OTP: validate attempts, expiry, blocked_until
```
