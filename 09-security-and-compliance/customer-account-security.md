...
document_id: SEC-CUSTOMER
last_updated: 2026-05-11
tags: [customer, account, ecommerce]
---

# Customer Account Security

## Purpose
This document defines security controls for tenant-scoped customer profiles, customer auth accounts, local/social identities, guest checkout, address handling, OTP verification, and customer order access.
...
|---|---|
| `customers` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `customer_auth_accounts` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `customer_auth_identities` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `otp_verifications` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `customer_addresses` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `carts` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `orders` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `order_addresses` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |

## Required permissions and feature gates
| Permission / gate | Usage rule |
|---|---|
| `customer.account.read` | Must be checked by backend services before the corresponding operation is executed. |
| `customer.account.update` | Must be checked by backend services before the corresponding operation is executed. |
| `customer.identity.manage` | Must be checked by backend services before the corresponding operation is executed. |
| `customer.order.read` | Must be checked by backend services before the corresponding operation is executed. |
...
|---|---|---|
| `POST` | `/api/v1/customers/register` | Create tenant customer account. |
| `GET` | `/api/v1/customers/me/orders` | Read own tenant order history. |

```http
POST /api/v1/customers/register HTTP/1.1
...
```mermaid
flowchart TD
  A[Customer login/register] --> B[Tenant customer profile]
  B --> C[Auth account]
  C --> D[Identity provider]
  B --> E[Orders and addresses]
  E --> F[Tenant-only visibility]
```
