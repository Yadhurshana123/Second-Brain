...
document_id: SEC-DATA-ISO
last_updated: 2026-05-11
tags: [tenant, isolation, data]
---

# Data Isolation Controls

## Purpose
This document defines mandatory tenant isolation and outlet scoping controls for every platform, POS, e-commerce, stock, payment, reporting, offline sync, and audit operation.
...
| `products` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `product_variants` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `inventory_balances` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
...
|---|---|
| `tenant.data.read` | Must be checked by backend services before the corresponding operation is executed. |
| `tenant.data.write` | Must be checked by backend services before the corresponding operation is executed. |
| `outlet.data.read` | Must be checked by backend services before the corresponding operation is executed. |
| `outlet.data.write` | Must be checked by backend services before the corresponding operation is executed. |
...
|---|---|---|
| `GET` | `/api/v1/tenants/{tenantId}/reports/sales` | Read tenant-scoped reports. |
| `POST` | `/api/v1/outlets/{outletId}/sales` | Create outlet-scoped sale. |

```http
GET /api/v1/tenants/{tenantId}/reports/sales HTTP/1.1
...
```mermaid
flowchart TD
  A[JWT tenant claim] --> B[API tenant context]
  B --> C[Repository tenant filter]
  C --> D[(Tenant-owned rows)]
  B --> E[Outlet scope when required]
  E --> D
```
