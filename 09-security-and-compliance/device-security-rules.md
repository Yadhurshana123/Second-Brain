...
document_id: SEC-DEVICE
last_updated: 2026-05-11
tags: [device, pos, till]
---

# Device Security Rules

## Purpose
This document defines POS device, till, terminal, scanner, printer, and cash drawer security controls so device identity cannot cross tenant or outlet boundaries.
...
|---|---|
| `device.register` | Must be checked by backend services before the corresponding operation is executed. |
| `device.block` | Must be checked by backend services before the corresponding operation is executed. |
...
| `receipt.print` | Must be checked by backend services before the corresponding operation is executed. |
...
|---|---|---|
| `POST` | `/api/v1/pos-devices/register` | Register POS terminal. |
| `POST` | `/api/v1/till-sessions/open` | Open till session for device. |

```http
POST /api/v1/pos-devices/register HTTP/1.1
...
```mermaid
flowchart LR
  D[POS device] --> O[Assigned outlet]
  O --> T[Till]
  T --> S[Till session]
  S --> P[POS sale]
  D --> R[Receipt printer context]
```
