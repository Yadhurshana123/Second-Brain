...
## Purpose
This document defines protection rules for offline POS data stored in browser/app IndexedDB through core/offline and synced through server-controlled offline sync queues.
...
|---|---|
| `offline_sync_batches` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `offline_sync_items` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `offline_sale_sync_queue` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `offline_payment_sync_queue` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `offline_sync_conflicts` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `offline_sync_audit_logs` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `sales` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `payments` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |
| `receipts` | Must be tenant-aware, access-controlled, audited, or protected according to this document. |

## Required permissions and feature gates
| Permission / gate | Usage rule |
|---|---|
| `offline.pos.use` | Must be checked by backend services before the corresponding operation is executed. |
| `offline.sync.submit` | Must be checked by backend services before the corresponding operation is executed. |
| `offline.conflict.resolve` | Must be checked by backend services before the corresponding operation is executed. |
| `offline.audit.read` | Must be checked by backend services before the corresponding operation is executed. |
...
|---|---|---|
| `POST` | `/api/v1/offline-sync/batches` | Submit offline sync batch. |
| `GET` | `/api/v1/offline-sync/conflicts` | Read sync conflicts. |

```http
POST /api/v1/offline-sync/batches HTTP/1.1
...
sequenceDiagram
  ...
  POS->>SyncAPI: push queued items
  SyncAPI->>DB: validate and accept/reject
  DB-->>POS: conflict or accepted status
```
