...
folder: 05-backend
document_type: backend-authentication-authorization
...
Purpose: Defines JWT authentication, tenant context resolution, platform user separation and configurable authorization enforcement.
...
Related reading: [[feature-access-handling]], [[validation-rules]], [[exception-handling]]
...
| Identity type | Table | JWT context |
|---|---|---|
| Platform user | `platform_users` | Platform admin operations only |
| Tenant staff | `users` | Tenant, optional outlet, role and permission context |
| Customer | `customers`, `customer_auth_accounts` | E-Commerce customer scope |
| POS device | `pos_devices` | Device context for terminal/session/sync |
| System/API | service account or system actor | Background jobs and integrations |
...
- JWT must carry only trusted identity and context claims; permissions may be resolved server-side to avoid stale token access.
- Platform users and tenant users must not be mixed in the same authorization branch.
- Password hashes are stored only in `platform_users`, `users` or customer auth identity rows as applicable.

## 1. JWT security model

JWT validation must verify issuer, audience, signing key and lifetime.
JWT should identify actor type, subject id and active tenant context where applicable.
High-risk authorization data should be loaded server-side so role changes can take effect without waiting for long token expiry.
...
## 3. Platform vs tenant identities
...
- Use short-lived access tokens and secure refresh token storage when refresh flow is implemented.
- Reject tokens for inactive, suspended or deleted actors.
