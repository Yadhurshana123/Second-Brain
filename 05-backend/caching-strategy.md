...
folder: 05-backend
document_type: backend-caching-strategy
...
Purpose: Defines safe backend caching for access context, configuration, reference data and POS cache preparation without making cache a source of truth.
...
Related reading: [[feature-access-handling]], [[authentication-authorization]], [[offline-sync-backend-rules]]
...
flowchart LR
    ...
    Cache -- yes --> Value[Use short-lived value]
    Cache -- no --> Repo[Load from database]
    Repo --> CacheSet[Set expiry]
    CacheSet --> Value
```

## System-Specific Responsibilities

| Cache candidate | Allowed? | Expiry rule |
|---|---|---|
| Permission catalog | Yes | Medium TTL; invalidate on deployment/seed change |
| Tenant access context | Yes | Short TTL; invalidate on role/feature updates |
| Feature flags | Yes | Short TTL; invalidate on configuration update |
| Product search projection | Yes | Short TTL; tenant scoped |
| Payment totals | No | Source of truth must be database |
| Stock balance for writes | No | Use transactional database state |
...
## 1. Allowed cache data

Cache access contexts only with short TTL and clear invalidation triggers.
Never use cache as source of truth for stock deduction or payment capture.
Tenant settings and feature flags may be cached, but writes must invalidate related keys.

## 2. Forbidden cache data
...
```csharp
var cacheKey = CacheKeys.TenantAccessContext(context.TenantId, context.UserId);
var accessContext = await _cache.GetOrCreateAsync(cacheKey, async entry =>
{
    entry.AbsoluteExpirationRelativeToNow = TimeSpan.FromMinutes(5);
    return await _accessRepository.LoadAccessContextAsync(context.TenantId, context.UserId);
});
```
...
- Use cache tags or key conventions that include tenant id.
- Invalidate access cache after role, permission or feature assignment changes.
