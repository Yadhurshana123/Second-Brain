...
title: Mapping Rules
folder: 05-backend
document_type: backend-mapping-rules
...
Purpose: Defines safe mapping between API requests, DTOs, domain entities and database entities without leaking persistence models to the API.
...
Related reading: [[dto-handling]], [[repository-layer-rules]], [[service-layer-rules]]
...
flowchart TD
    ApiRequest --> AppDto
    AppDto --> DomainFactory
    DomainEntity --> Repository
    Repository --> Projection
    Projection --> ResponseDto
```

## System-Specific Responsibilities

| Mapping | Allowed direction | Rule |
|---|---|---|
| API request to DTO | Controller or mapper | Never expose DB entity |
| DTO to domain | Application service/factory | Validate before creation |
| Domain to DTO | Application mapper | Shape only allowed fields |
| Entity to projection | Repository query | Read-only lists only |
| Snapshot mapping | Service | Freeze price/tax/receipt context |
...
public static ProductDto From(Product product) => new()
{
    ...
    IsSellablePos = product.IsSellablePos,
